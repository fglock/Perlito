...
sub is { my $got = $_[0]; my $expected = $_[1]; my $desc = $_[2]; my $todo = $_[3]; my $depends = $_[4]; ((my  $test = undef) = ($got eq $expected)); Perlito::Test::proclaim($test, 'is! ' . $desc, $todo, $got, $expected, $depends) };
sub is_deeply { my $got = $_[0]; my $expected = $_[1]; my $desc = $_[2]; my $todo = $_[3]; my $depends = $_[4]; ((my  $got_perl = undef) = Main::perl($got, )); ((my  $expected_perl = undef) = Main::perl($expected, )); ((my  $test = undef) = (($got_perl eq $expected_perl))); Perlito::Test::proclaim($test, 'is deeply! ' . $desc, $todo, $got_perl, $expected_perl, $depends) };
sub isnt { my $got = $_[0]; my $expected = $_[1]; my $desc = $_[2]; my $todo = $_[3]; my $depends = $_[4]; ((my  $test = undef) = !Main::bool((($got eq $expected)))); Perlito::Test::proclaim($test, 'isnt! ' . $desc, $todo, $got, $expected, $depends, do { (my  $Hash_a = {}); ($Hash_a->{'negate'} = 1); $Hash_a }) };
...
sub test_ends { if (Main::bool((!Main::bool($testing_started)))) { return () } ; if (Main::bool((!Main::bool($num_of_tests_planned)))) { Main::say('1..' . $num_of_tests_run) } ; if (Main::bool((($num_of_tests_planned != $num_of_tests_run)))) { Main::say('# Looks like you planned ' . $num_of_tests_planned . ' tests, but ran ' . $num_of_tests_run) } ; if (Main::bool(($num_of_tests_failed))) { Main::say('# Looks like you failed ' . $num_of_tests_failed . ' tests of ' . $num_of_tests_run) } ; ($num_of_tests_run = 0); ($num_of_tests_failed = 0); ($num_of_tests_planned = 0); ($testing_started = 0) }
}
