...
sub name { $_[0]->{name} };
sub emit { my $self = $_[0]; ((my  $table = undef) = { ('$' => '$'), ('@' => '$List_'), ('%' => '$Hash_'), ('&' => '$Code_') }); (Main::bool((($self->{twigil} eq '.'))) ? ('$self.{' . $self->{name} . '}') : ((Main::bool((($self->{name} eq '/'))) ? ($table->{$self->{sigil}} . 'MATCH') : ($table->{$self->{sigil}} . $self->{name})))) }
}
...
sub emit { my $self = $_[0]; if (Main::bool(Main::isa($self->{parameters}, 'Lit::Array'))) { ((my  $a = undef) = $self->{parameters}->array()); ((my  $str = undef) = 'do { '); ((my  $i = undef) = 0); for my $var ( @{($a || []) || []} ) { ((my  $bind = undef) = Bind->new(('parameters' => $var), ('arguments' => Index->new(('obj' => $self->{arguments}), ('index_exp' => Val::Int->new(('int' => $i))))))); ($str = $str . ' ' . $bind->emit(("" . '; '))); ($i = ($i + 1)) }; return($str . $self->{parameters}->emit(("" . ' }'))) } ; if (Main::bool(Main::isa($self->{parameters}, 'Lit::Hash'))) { ((my  $a = undef) = $self->{parameters}->hash()); ((my  $b = undef) = $self->{arguments}->hash()); ((my  $str = undef) = 'do { '); ((my  $i = undef) = 0); (my  $arg = undef); for my $var ( @{($a || []) || []} ) { ($arg = Val::Undef->new()); for my $var2 ( @{($b || []) || []} ) { if (Main::bool((($var2->[0])->buf() eq ($var->[0])->buf()))) { ($arg = $var2->[1]) }  }; ((my  $bind = undef) = Bind->new(('parameters' => $var->[1]), ('arguments' => $arg))); ($str = $str . ' ' . $bind->emit(("" . '; '))); ($i = ($i + 1)) }; return($str . $self->{parameters}->emit(("" . ' }'))) } ; if (Main::bool(Main::isa($self->{parameters}, 'Lit::Object'))) { ((my  $class = undef) = $self->{parameters}->class()); ((my  $a = undef) = $self->{parameters}->fields()); ((my  $b = undef) = $self->{arguments}); ((my  $str = undef) = 'do { '); ((my  $i = undef) = 0); (my  $arg = undef); for my $var ( @{($a || []) || []} ) { ((my  $bind = undef) = Bind->new(('parameters' => $var->[1]), ('arguments' => Call->new(('invocant' => $b), ('method' => ($var->[0])->buf()), ('arguments' => []), ('hyper' => 0))))); ($str = $str . ' ' . $bind->emit(("" . '; '))); ($i = ($i + 1)) }; return($str . $self->{parameters}->emit(("" . ' }'))) } ; $self->{parameters}->emit(("" . ' = ') . $self->{arguments}->emit()) }
}
...
sub arguments { $_[0]->{arguments} };
sub emit { my $self = $_[0]; ((my  $invocant = undef) = $self->{invocant}->emit()); if (Main::bool(($invocant eq 'self'))) { ($invocant = '$self') } ; if (Main::bool(((((((($self->{method} eq 'perl')) || (($self->{method} eq 'yaml'))) || (($self->{method} eq 'say'))) || (($self->{method} eq 'join'))) || (($self->{method} eq 'chars'))) || (($self->{method} eq 'isa'))))) { if (Main::bool(($self->{hyper}))) { return('[ map { &Main::' . $self->{method} . '( $_, ' . ', ' . Main::join(([ map { $_->emit() } @{ $self->{arguments} } ]), ', ') . ')' . ' } @( ' . $invocant . ' ) ]') } else { return('&Main::' . $self->{method} . '(' . $invocant . ', ' . Main::join(([ map { $_->emit() } @{ $self->{arguments} } ]), ', ') . ')') } } ; ((my  $meth = undef) = $self->{method}); if (Main::bool(($meth eq 'postcircumfix:<( )>'))) { ($meth = '') } ; ((my  $call = undef) = '.' . $meth . '(' . Main::join(([ map { $_->emit() } @{ $self->{arguments} } ]), ', ') . ')'); if (Main::bool(($self->{hyper}))) { '[ map { $_' . $call . ' } @( ' . $invocant . ' ) ]' } else { $invocant . $call } }
}
...
sub body { $_[0]->{body} };
sub topic { $_[0]->{topic} };
sub emit { my $self = $_[0]; ((my  $cond = undef) = $self->{cond}); if (Main::bool((Main::isa($cond, 'Var') && ($cond->sigil() eq '@')))) { ($cond = Apply->new(('code' => 'prefix:<@>'), ('arguments' => [$cond]))) } ; 'do { for my ' . $self->{topic}->emit(("" . ' ( ') . $cond->emit(("" . ' ) { ') . Main::join(([ map { $_->emit() } @{ $self->{body} } ]), ';') . ' } }')) }
}
...
sub sig { $_[0]->{sig} };
sub block { $_[0]->{block} };
sub emit { my $self = $_[0]; ((my  $sig = undef) = $self->{sig}); ((my  $pos = undef) = $sig->positional()); (my  $str = undef); ((my  $pos = undef) = $sig->positional()); for my $field ( @{($pos || []) || []} ) { ($str = $str . '' . $field->emit(("" . '?, '))) }; if (Main::bool(($self->{name} eq ''))) { return('(sub (' . $str . ') ' . ' { ' . Main::join(([ map { $_->emit() } @{ $self->{block} } ]), '; ') . ' })') } ; 'sub ' . $self->{name} . '(' . $str . ') ' . ' { ' . Main::join(([ map { $_->emit() } @{ $self->{block} } ]), '; ') . ' }' }
}
