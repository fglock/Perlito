...
' . ($self->{arguments}->[1])->emit_parrot(("" . '  $P0 = $P1 - $P0') . '
' . '  restore $P1' . '
'))) } ; if (Main::bool(($code eq 'infix:<&&>'))) { return((If->new(('cond' => $self->{arguments}->[0]), ('body' => [$self->{arguments}->[1]]), ('otherwise' => [])))->emit_parrot()) } ; if (Main::bool(($code eq 'infix:<||>'))) { return((If->new(('cond' => $self->{arguments}->[0]), ('body' => []), ('otherwise' => [$self->{arguments}->[1]])))->emit_parrot()) } ; if (Main::bool(($code eq 'infix:<eq>'))) { ($label = ($label + 1)); ((my  $id = undef) = $label); return(($self->{arguments}->[0])->emit_parrot(("" . '  $S0 = $P0') . '
' . '  save $S0' . '
' . ($self->{arguments}->[1])->emit_parrot(("" . '  $S1 = $P0') . '
' . '  restore $S0' . '
' . '  if $S0 == $S1 goto eq' . $id . '
...
sub topic { $_[0]->{topic} };
((my  $label = undef) = 100);
sub emit_parrot { my $self = $_[0]; ((my  $cond = undef) = $self->{cond}); ($label = ($label + 1)); ((my  $id = undef) = $label); if (Main::bool((Main::isa($cond, 'Var') && ($cond->sigil() ne '@')))) { ($cond = Lit::Array->new(('array1' => [$cond]))) } ; return('' . $cond->emit_parrot(("" . '  save $P1') . '
' . '  save $P2' . '
' . '  $P1 = new .Iterator, $P0' . '
' . ' test_iter' . $id . ':' . '
