...
sub emit_go { my $self = $_[0]; if (Main::bool((($self->{block}) ? 0 : 1))) { return('') } ; ((my  $str = undef) = ''); (my  $Hash_decl_seen = {}); for my $decl1 ( @{$self->{block} || []} ) { ((my  $decl = undef) = $decl1); if (Main::bool(((Main::isa($decl, 'Bind') && Main::isa(($decl->parameters()), 'Decl')) && ((($decl->parameters())->decl() eq 'my'))))) { ($decl = $decl->parameters()) } ; if (Main::bool((Main::isa($decl, 'Decl') && (($decl->decl() eq 'my'))))) { ((my  $var_name = undef) = (($decl)->var())->emit_go()); if (Main::bool((($Hash_decl_seen->{$var_name}) ? 0 : 1))) { ($str = $str . $decl->emit_go_init()); ($Hash_decl_seen->{$var_name} = 1) }  }  }; (my  $last_statement = undef); if (Main::bool($self->{needs_return})) { ($last_statement = pop( @{$self->{block}} )) } ; for my $decl ( @{$self->{block} || []} ) { if (Main::bool(((((Main::isa($decl, 'Decl') && (($decl->decl() eq 'my')))) ? 0 : 1)))) { ($str = $str . ($decl)->emit_go(("" . ';'))) }  }; if (Main::bool(($self->{needs_return} && $last_statement))) { if (Main::bool(Main::isa($last_statement, 'If'))) { ((my  $cond = undef) = $last_statement->cond()); ((my  $body = undef) = $last_statement->body()); ((my  $otherwise = undef) = $last_statement->otherwise()); if (Main::bool((Main::isa($cond, 'Apply') && ($cond->code() eq 'prefix:<!>')))) { ($cond = ($cond->arguments())->[0]); ($body = $last_statement->otherwise()); ($otherwise = $last_statement->body()) } ; if (Main::bool((Main::isa($cond, 'Var') && ($cond->sigil() eq '@')))) { ($cond = Apply->new(('code' => 'prefix:<@>'), ('arguments' => [$cond]))) } ; ($body = Perlito::Go::LexicalBlock->new(('block' => $body), ('needs_return' => 1), ('top_level' => $self->{top_level}))); ($otherwise = Perlito::Go::LexicalBlock->new(('block' => $otherwise), ('needs_return' => 1), ('top_level' => $self->{top_level}))); ($str = $str . 'if tobool( ' . Call::emit_go_call($cond, 'Bool') . ' ) { ' . $body->emit_go(("" . ' } else { ') . $otherwise->emit_go(("" . ' }')))) } else { if (Main::bool((Main::isa($last_statement, 'Return') || Main::isa($last_statement, 'For')))) { ($str = $str . $last_statement->emit_go()) } else { ($last_statement = Return->new(('result' => $last_statement))); if (Main::bool($self->{top_level})) { ($str = $str . $last_statement->emit_go()) } else { ($str = $str . $last_statement->emit_go_simple()) } } } } ; return($str) }
}
...
'); (my  $Hash_decl_seen = {}); for my $decl1 ( @{$self->{body} || []} ) { ((my  $decl = undef) = $decl1); if (Main::bool(((Main::isa($decl, 'Bind') && Main::isa(($decl->parameters()), 'Decl')) && ((($decl->parameters())->decl() eq 'my'))))) { ($decl = $decl->parameters()) } ; if (Main::bool((Main::isa($decl, 'Decl') && (($decl->decl() eq 'my'))))) { ((my  $var_name = undef) = (($decl)->var())->emit_go()); if (Main::bool((($Hash_decl_seen->{$var_name}) ? 0 : 1))) { ($str = $str . $decl->emit_go_init()); ($Hash_decl_seen->{$var_name} = 1) }  }  }; for my $decl ( @{$self->{body} || []} ) { if (Main::bool((Main::isa($decl, 'Decl') && (($decl->decl() eq 'has'))))) { ($str = $str . '  // accessor ' . ($decl->var())->name(("" . '
...
'); ((my  $Hash_meth_seen = {}) = do { (my  $Hash_a = {}); ($Hash_a->{'join'} = 1); ($Hash_a->{'perl'} = 1); ($Hash_a->{'scalar'} = 1); ($Hash_a->{'isa'} = 1); ($Hash_a->{'values'} = 1); ($Hash_a->{'keys'} = 1); ($Hash_a->{'exists'} = 1); ($Hash_a->{'bind'} = 1); ($Hash_a->{'int'} = 1); ($Hash_a->{'num'} = 1); ($Hash_a->{'str'} = 1); ($Hash_a->{'Str'} = 1); ($Hash_a->{'bool'} = 1); ($Hash_a->{'Bool'} = 1); ($Hash_a->{'array'} = 1); ($Hash_a->{'hash'} = 1); ($Hash_a->{'push'} = 1); ($Hash_a->{'pop'} = 1); ($Hash_a->{'shift'} = 1); ($Hash_a->{'lookup'} = 1); ($Hash_a->{'index'} = 1); ($Hash_a->{'function'} = 1); $Hash_a }); for my $comp_unit ( @{(($comp_units) || []) || []} ) { for my $stmt ( @{(($comp_unit->body()) || []) || []} ) { if (Main::bool((Main::isa($stmt, 'Method') && (($Hash_meth_seen->{$stmt->name()}) ? 0 : 1)))) { ((my  $meth = undef) = $stmt->name()); ($str = $str . 'type ' . $meth . '_er interface ' . '{' . ' f_' . $meth . ' (Capture) *Any }' . '
'); ($Hash_meth_seen->{$meth} = 1) } ; if (Main::bool(((Main::isa($stmt, 'Decl') && (($stmt->decl() eq 'has'))) && (($Hash_meth_seen->{($stmt->var())->name()}) ? 0 : 1)))) { ((my  $meth = undef) = ($stmt->var())->name()); ($str = $str . 'type ' . $meth . '_er interface ' . '{' . ' f_' . $meth . ' (Capture) *Any }' . '
...
sub emit_go { my $self = $_[0]; ((my  $str = undef) = ''); for my $item ( @{$self->{array1} || []} ) { if (Main::bool((((Main::isa($item, 'Var') && ($item->sigil() eq '@'))) || ((Main::isa($item, 'Apply') && ($item->code() eq 'prefix:<@>')))))) { ($str = $str . 'func(a_ *Array) { ' . 'for i_ := 0; i_ <= a_.n; i_++ { (*a).(push_er).f_push( Capture{ p: []*Any{ a_.v[i_] } } ) } ' . '}( (*' . Call::emit_go_call($item, 'array') . ').(*Array) ); ') } else { ($str = $str . '(*a).(push_er).f_push( Capture{ p: []*Any{ ' . $item->emit_go(("" . ' } } ); '))) } }; 'func () *Any { ' . 'a := a_array(); ' . $str . 'return a; ' . '}()' }
}
...
sub hash1 { $_[0]->{hash1} };
sub emit_go { my $self = $_[0]; ((my  $fields = undef) = $self->{hash1}); ((my  $str = undef) = ''); for my $field ( @{($fields || []) || []} ) { ($str = $str . '*(*m).(lookup_er).f_lookup( Capture{ p : []*Any{ ' . ($field->[0])->emit_go(("" . ' }} ) = *(') . ($field->[1])->emit_go(("" . '); ')))) }; 'func() *Any { ' . 'm := h_hash(); ' . $str . 'return m; ' . '}()' }
}
...
sub emit_go { my $self = $_[0]; ((my  $cond = undef) = $self->{cond}); if (Main::bool((Main::isa($cond, 'Apply') && ($cond->code() eq 'prefix:<!>')))) { ((my  $if = undef) = If->new(('cond' => ($cond->arguments())->[0]), ('body' => $self->{otherwise}), ('otherwise' => $self->{body}))); return($if->emit_go()) } ; if (Main::bool((Main::isa($cond, 'Var') && ($cond->sigil() eq '@')))) { ($cond = Apply->new(('code' => 'prefix:<@>'), ('arguments' => [$cond]))) } ; ((my  $s = undef) = 'if tobool( ' . Call::emit_go_call($cond, 'Bool') . ' ) { ' . (Perlito::Go::LexicalBlock->new(('block' => $self->{body}), ('needs_return' => 0)))->emit_go(("" . ' }'))); if (Main::bool((($self->{otherwise}) ? 0 : 1))) { return($s) } ; return($s . ' else { ' . (Perlito::Go::LexicalBlock->new(('block' => $self->{otherwise}), ('needs_return' => 0)))->emit_go(("" . ' }'))) }
}
...
sub body { $_[0]->{body} };
sub emit_go { my $self = $_[0]; ((my  $cond = undef) = $self->{cond}); if (Main::bool((Main::isa($cond, 'Var') && ($cond->sigil() eq '@')))) { ($cond = Apply->new(('code' => 'prefix:<@>'), ('arguments' => [$cond]))) } ; return('for ' . ((Main::bool($self->{init}) ? $self->{init}->emit_go(("" . '; ')) : ';')) . 'tobool( ' . Call::emit_go_call($cond, 'Bool') . ' ); ' . ((Main::bool($self->{continue}) ? $self->{continue}->emit_go(("" . ' ')) : '')) . '{ ' . (Perlito::Go::LexicalBlock->new(('block' => $self->{body}), ('needs_return' => 0)))->emit_go(("" . ' }'))) }
}
