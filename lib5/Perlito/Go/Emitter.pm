...
sub emit_go { my $self = $_[0]; ((my  $ast = undef) = $self->expand_interpolation()); return($ast->emit_go()) }
}
...
sub arguments { $_[0]->{arguments} };
sub emit_go { my $self = $_[0]; ((my  $invocant = undef) = $self->{invocant}->emit_go()); if (Main::bool(Main::isa(($self->{invocant}), 'Proto'))) { if (Main::bool(($self->{invocant}->name() eq 'self'))) { ($invocant = 'v_self') } else { ($invocant = 'Proto_' . $invocant) } } ; ((my  $meth = undef) = $self->{method}); if (Main::bool(($meth eq 'postcircumfix:<( )>'))) { if (Main::bool(($self->{hyper}))) { ($meth = '') } else { return('((*' . $invocant . ').(function_er).f_function( Capture{ p : []*Any{ ' . Main::join(([ map { $_->emit_go() } @{ $self->{arguments} } ]), ', ') . ' } } ))') } } ; if (Main::bool(($self->{hyper}))) { return('func (a_ *Any) *Any { ' . '
' . '  var out = a_array(); ' . '
...
sub emit_go_call { my $invocant = $_[0]; my $meth_name = $_[1]; ((my  $invocant1 = undef) = $invocant->emit_go()); if (Main::bool(Main::isa(($invocant), 'Proto'))) { ($invocant1 = 'Proto_' . $invocant1) } ; ((my  $meth = undef) = $meth_name); return('(*' . $invocant1 . ').(' . $meth . '_er).f_' . $meth . '(Capture{})') }
}
...
sub emit_go { my $self = $_[0]; if (Main::bool(Main::isa(($self->{result}), 'Bind'))) { ((my  $tmp = undef) = ($self->{result})->parameters()); return('(func () { ' . ($self->{result})->emit_go(("" . '; ') . 'p <- ' . $tmp->emit_go(("" . '; ') . 'runtime.Goexit(); ' . '}())'))) } ; return('(func () { ' . 'var tmp *Any = ' . ($self->{result})->emit_go(("" . '; ') . 'p <- tmp; ' . 'runtime.Goexit(); ' . '}())')) };
sub emit_go_simple { my $self = $_[0]; if (Main::bool(Main::isa(($self->{result}), 'Bind'))) { ((my  $tmp = undef) = ($self->{result})->parameters()); return('return (func () *Any { ' . ($self->{result})->emit_go(("" . '; ') . 'return ' . $tmp->emit_go(("" . '; ') . '}())'))) } ; return('return( ' . $self->{result}->emit_go(("" . ')'))) }
}
...
sub emit_go { my $self = $_[0]; ((my  $cond = undef) = $self->{cond}); if (Main::bool((Main::isa($cond, 'Apply') && ($cond->code() eq 'prefix:<!>')))) { ((my  $if = undef) = If->new(('cond' => ($cond->arguments())->[0]), ('body' => $self->{otherwise}), ('otherwise' => $self->{body}))); return($if->emit_go()) } ; if (Main::bool((Main::isa($cond, 'Var') && ($cond->sigil() eq '@')))) { ($cond = Apply->new(('code' => 'prefix:<@>'), ('arguments' => do { (my  $List_a = []); (my  $List_v = []); push( @{$List_a}, $cond ); $List_a }))) } ; ((my  $s = undef) = 'if tobool( ' . Call::emit_go_call($cond, 'Bool') . ' ) { ' . (Perlito::Go::LexicalBlock->new(('block' => $self->{body}), ('needs_return' => 0)))->emit_go(("" . ' }'))); if (Main::bool(!Main::bool(($self->{otherwise})))) { return($s) } ; return($s . ' else { ' . (Perlito::Go::LexicalBlock->new(('block' => $self->{otherwise}), ('needs_return' => 0)))->emit_go(("" . ' }'))) }
}
...
sub body { $_[0]->{body} };
sub emit_go { my $self = $_[0]; ((my  $cond = undef) = $self->{cond}); if (Main::bool((Main::isa($cond, 'Var') && ($cond->sigil() eq '@')))) { ($cond = Apply->new(('code' => 'prefix:<@>'), ('arguments' => do { (my  $List_a = []); (my  $List_v = []); push( @{$List_a}, $cond ); $List_a }))) } ; return('for ' . ((Main::bool($self->{init}) ? $self->{init}->emit_go(("" . '; ')) : ';')) . 'tobool( ' . Call::emit_go_call($cond, 'Bool') . ' ); ' . ((Main::bool($self->{continue}) ? $self->{continue}->emit_go(("" . ' ')) : '')) . '{ ' . (Perlito::Go::LexicalBlock->new(('block' => $self->{body}), ('needs_return' => 0)))->emit_go(("" . ' }'))) }
}
