...
'); ((my  $Hash_meth_seen = {}) = { ('join' => 1), ('perl' => 1), ('scalar' => 1), ('isa' => 1), ('values' => 1), ('keys' => 1), ('exists' => 1), ('bind' => 1), ('int' => 1), ('num' => 1), ('str' => 1), ('Str' => 1), ('bool' => 1), ('Bool' => 1), ('array' => 1), ('hash' => 1), ('push' => 1), ('pop' => 1), ('shift' => 1), ('lookup' => 1), ('index' => 1), ('function' => 1) }); for my $comp_unit ( @{(($comp_units) || []) || []} ) { for my $stmt ( @{(($comp_unit->body()) || []) || []} ) { if (Main::bool((Main::isa($stmt, 'Method') && (($Hash_meth_seen->{$stmt->name()}) ? 0 : 1)))) { ((my  $meth = undef) = $stmt->name()); ($str = $str . 'type ' . $meth . '_er interface ' . '{' . ' f_' . $meth . ' (Capture) *Any }' . '
...
sub name { $_[0]->{name} };
sub emit_go { my $self = $_[0]; ((my  $table = undef) = { ('$' => 'v_'), ('@' => 'List_'), ('%' => 'Hash_'), ('&' => 'Code_') }); ((my  $ns = undef) = ''); if (Main::bool($self->{namespace})) { ($ns = Main::to_go_namespace($self->{namespace}) . '.') } ; (Main::bool((($self->{twigil} eq '.'))) ? ('(*v_self).(' . $self->{name} . '_er).f_' . $self->{name} . '(Capture{})') : ((Main::bool((($self->{name} eq '/'))) ? ('v_MATCH') : ($table->{$self->{sigil}} . $ns . $self->{name})))) };
