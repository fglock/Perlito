...
sub ws2 { $_[0]->{ws2} };
sub ws3 { $_[0]->{ws3} };
sub emit { my $self = $_[0]; if (Main::bool(((($self->{quant} eq '')) && (($self->{greedy} eq ''))))) { return($self->{term}->emit()) } ; if (Main::bool(((($self->{quant} eq '+')) && (($self->{greedy} eq ''))))) { $self->{term}->set_captures_to_array(); return('(do { ' . 'my $last_match_null = 0; ' . 'my $last_pos = $MATCH.to; ' . 'my $count = 0; ' . 'while ' . $self->{term}->emit() . ' && ($last_match_null < 2) ' . '{ ' . 'if $last_pos == $MATCH.to() { ' . '$last_match_null = $last_match_null + 1; ' . '} ' . 'else { ' . '$last_match_null = 0; ' . '}; ' . '$last_pos = $MATCH.to; ' . '$count = $count + 1; ' . '}; ' . '$MATCH.to = $last_pos; ' . '$count > 0; ' . '})') } ; if (Main::bool(((($self->{quant} eq '*')) && (($self->{greedy} eq ''))))) { $self->{term}->set_captures_to_array(); return('(do { ' . 'my $last_match_null = 0; ' . 'my $last_pos = $MATCH.to; ' . 'while ' . $self->{term}->emit() . ' && ($last_match_null < 2) ' . '{ ' . 'if $last_pos == $MATCH.to() { ' . '$last_match_null = $last_match_null + 1; ' . '} ' . 'else { ' . '$last_match_null = 0; ' . '}; ' . '$last_pos = $MATCH.to; ' . '}; ' . '$MATCH.to = $last_pos; ' . '1 ' . '})') } ; if (Main::bool(((($self->{quant} eq '?')) && (($self->{greedy} eq ''))))) { $self->{term}->set_captures_to_array(); return('(do { ' . 'my $last_pos = $MATCH.to; ' . 'if !(do {' . $self->{term}->emit() . '}) ' . '{ ' . '$MATCH.to = $last_pos; ' . '}; ' . '1 ' . '})') } ; warn('Rul::Quantifier: ' . Main::perl($self, ("" . ' not implemented'))); $self->{term}->emit() };
...
sub char { $_[0]->{char} };
sub emit { my $self = $_[0]; (my  $char = $self->{char}); if (Main::bool(($char eq 'n'))) { (my  $rul = Rul::Subrule->new(('metasyntax' => 'is_newline'), ('captures' => 0))); ($rul = $rul->emit()); return($rul) } ; if (Main::bool(($char eq 'N'))) { (my  $rul = Rul::Subrule->new(('metasyntax' => 'not_newline'), ('captures' => 0))); ($rul = $rul->emit()); return($rul) } ; if (Main::bool(($char eq 'd'))) { (my  $rul = Rul::Subrule->new(('metasyntax' => 'digit'), ('captures' => 0))); ($rul = $rul->emit()); return($rul) } ; if (Main::bool(($char eq 's'))) { (my  $rul = Rul::Subrule->new(('metasyntax' => 'space'), ('captures' => 0))); ($rul = $rul->emit()); return($rul) } ; return(Rul::constant($char)) };
sub set_captures_to_array { my $self = $_[0];  }
}
...
sub rule_exp { $_[0]->{rule_exp} };
sub emit { my $self = $_[0]; '(do { ' . 'my $tmp = $MATCH; ' . '$MATCH = Perlito::Match.new( \'str\' => $str, \'from\' => $tmp.to, \'to\' => $tmp.to, \'bool\' => 1  ); ' . '$MATCH.bool = ' . $self->{rule_exp}->emit() . '; ' . '$tmp.bool = ?$MATCH; ' . '$MATCH = $tmp; ' . '?$MATCH; ' . '})' };
sub set_captures_to_array { my $self = $_[0];  }
}
