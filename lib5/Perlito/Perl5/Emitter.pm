...
sub cond { $_[0]->{cond} };
sub body { $_[0]->{body} };
sub emit_perl5 { my $self = $_[0]; ((my  $cond = undef) = $self->{cond}); if (Main::bool((((Main::isa($cond, 'Var') && ($cond->sigil() eq '@'))) ? 0 : 1))) { ($cond = Lit::Array->new(('array1' => do { (my  $List_a = []); (my  $List_v = []); push( @{$List_a}, $cond ); $List_a }))) } ; (my  $sig = undef); if (Main::bool($self->{body}->sig())) { ($sig = 'my ' . $self->{body}->sig()->emit_perl5() . ' ') } ; return('for ' . $sig . '( @{' . $cond->emit_perl5() . ' || []} ) { ' . $self->{body}->emit_perl5() . ' }') }
}
