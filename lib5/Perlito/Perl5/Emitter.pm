...
$self->emit_perl5_indented(0)
        ...
        sub emit_perl5_indented {
            ...
            my $level = $_[1];
(my  $List_body = []);
for ( @{$self->{body} || []} ) {     if (Main::bool(defined($_))) {
            push( @{$List_body}, $_ )
    }
 };
            Perl5::tab($level) . '{' . '
...
' . Perl5::tab(($level + 1)) . Main::join(([ map { $_->emit_perl5_indented(($level + 1)) } @{( $List_body )} ]), ';' . '
') . '
' . Perl5::tab($level) . '}' . '
' . '
'
...
' . 'our ' . '$' . 'MATCH = Perlito::Match->new();' . '
');
for my $comp_unit ( @{(($comp_units) || []) || []} ) {     ($str = $str . $comp_unit->emit_perl5_indented(0)) };
            ($str = $str . '1;' . '
');
            return scalar ($str)
        }
...
$self->emit_perl5_indented(0)
        ...
        sub emit_perl5_indented {
            ...
            my $level = $_[1];
(my  $List_body = []);
for ( @{$self->{stmts} || []} ) {     if (Main::bool(defined($_))) {
            push( @{$List_body}, $_ )
    }
 };
...
;
            if (Main::bool(Main::isa($parameters, 'Lit::Array'))) {
                    ...
                    ((my  $str = undef) = 'do { ');
                    ((my  $i = undef) = 0);
for my $var ( @{($a || []) || []} ) {     ($str = $str . ' ' . emit_perl5_bind($var, Index->new(('obj' => $arguments), ('index_exp' => Val::Int->new(('int' => $i))))) . '; ');
    ($i = ($i + 1)) };
...
;
            if (Main::bool(Main::isa($parameters, 'Lit::Hash'))) {
                    ...
                    ((my  $i = undef) = 0);
(my  $arg = undef);
for my $var ( @{($a || []) || []} ) {     ($arg = Apply->new(('code' => 'Mu'), ('arguments' => do {
(my  $List_a = []);
(my  $List_v = []);
    $List_a
})));
...
' . Perl5::tab($level) . '}' . '
' . ((Main::bool($self->{otherwise}) ? (Perl5::tab($level) . 'else ' . '{' . '
...
;
            'for ( ' . ((Main::bool($self->{init}) ? $self->{init}->emit_perl5() . '; ' : '; ')) . ((Main::bool($cond) ? Perl5::to_bool($cond)->emit_perl5() . '; ' : '; ')) . ((Main::bool($self->{continue}) ? $self->{continue}->emit_perl5() . ' ' : ' ')) . ') { ' . $self->{body}->emit_perl5() . ' }'
        }
    }
...
$self->emit_perl5_indented(0)
        ...
        sub emit_perl5_indented {
            ...
            if (Main::bool(($decl eq 'has'))) {
                    return scalar ('sub ' . $name . ' { $_[0]->{' . $name . '} }')
            }
;
...
$self->emit_perl5_indented(0)
        ...
        sub emit_perl5_indented {
            ...
            ((my  $str = undef) = '');
            ((my  $i = undef) = 1);
for my $field ( @{($pos || []) || []} ) {     ($str = $str . Perl5::tab(($level + 1)) . 'my ' . $field->emit_perl5() . ' = $_[' . $i . '];' . '
');
    ($i = ($i + 1)) };
