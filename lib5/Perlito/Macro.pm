...
(my  $List_items = []);
for my $item ( @{$self->{array1} || []} ) {     if (Main::bool((Main::isa($item, 'Apply') && ((($item->code() eq 'circumfix:<( )>') || ($item->code() eq 'list:<,>')))))) {
for my $arg ( @{(($item->arguments()) || []) || []} ) {     push( @{$List_items}, $arg ) }
    }
    else {
            push( @{$List_items}, $item )
    }
 };
for my $item ( @{$List_items || []} ) {     if (Main::bool(((Main::isa($item, 'Var') && ($item->sigil() eq '@')) || (Main::isa($item, 'Apply') && ((($item->code() eq 'prefix:<@>') || ($item->code() eq 'infix:<..>'))))))) {
...
;
    ...
    package Lit::Hash;
        ...
        sub expand_interpolation {
            my $self = $_[0];
(my  $List_items = []);
for my $item ( @{$self->{hash1} || []} ) {     if (Main::bool((Main::isa($item, 'Apply') && ((($item->code() eq 'circumfix:<( )>') || ($item->code() eq 'list:<,>')))))) {
for my $arg ( @{(($item->arguments()) || []) || []} ) {     push( @{$List_items}, $arg ) }
    }
    else {
            push( @{$List_items}, $item )
...
(my  $List_s = []);
for my $item ( @{$List_items || []} ) {     if (Main::bool((Main::isa($item, 'Apply') && ($item->code() eq 'infix:<=>>')))) {
            push( @{$List_s}, Apply->new(('arguments' => do {
(my  $List_a = []);
(my  $List_v = []);
