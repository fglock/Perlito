...
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; ((my  $ast = undef) = $self->expand_interpolation()); return($ast->emit_python_indented($level)) }
}
...
sub body { $_[0]->{body} };
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; ((my  $body_block = undef) = Perlito::Python::LexicalBlock->new(('block' => $self->{body}->stmts()))); ((my  $sig = undef) = 'v__'); if (Main::bool($self->{body}->sig())) { ($sig = $self->{body}->sig()->emit_python_name()) } ; if (Main::bool($body_block->has_my_decl())) { ((my  $label = undef) = '_anon_' . Perlito::Python::LexicalBlock::get_ident_python()); ((my  $anon_var = undef) = ($self->{body}->sig() || Var->new(('name' => '_'), ('namespace' => ''), ('sigil' => '$'), ('twigil' => '')))); ((my  $anon_sig = undef) = Sig->new(('invocant' => undef), ('positional' => do { (my  $List_a = []); (my  $List_v = []); push( @{$List_a}, $anon_var ); $List_a }), ('named' => do { (my  $Hash_a = {}); $Hash_a }))); Perlito::Python::LexicalBlock::push_stmt_python(Perlito::Python::AnonSub->new(('name' => $label), ('block' => $self->{body}->stmts()), ('sig' => $anon_sig), ('handles_return_exception' => 0))); return(Python::tab($level) . 'for ' . $sig . ' in ' . $self->{cond}->emit_python() . ':' . '
...
sub block { $_[0]->{block} };
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; ((my  $label = undef) = '_anon_' . Perlito::Python::LexicalBlock::get_ident_python()); (my  $block = undef); if (Main::bool(Main::isa($self->{block}, 'Lit::Block'))) { ($block = $self->{block}->stmts()) } else { ($block = do { (my  $List_a = []); (my  $List_v = []); push( @{$List_a}, $self->{block} ); $List_a }) }; Perlito::Python::LexicalBlock::push_stmt_python(Perlito::Python::AnonSub->new(('name' => $label), ('block' => $block), ('sig' => Sig->new(('invocant' => undef), ('positional' => do { (my  $List_a = []); (my  $List_v = []); $List_a }), ('named' => do { (my  $Hash_a = {}); $Hash_a }))), ('handles_return_exception' => 0))); return(Python::tab($level) . 'f_' . $label . '()') }
}
