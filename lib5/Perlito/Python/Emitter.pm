...
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; ((my  $invocant = undef) = $self->{invocant}->emit_python()); if (Main::bool(($self->{method} eq 'new'))) { (my  $List_str = []); for my $field ( @{$self->{arguments} || []} ) { if (Main::bool((Main::isa($field, 'Apply') && ($field->code() eq 'infix:<=>>')))) { push( @{$List_str}, 'v_' . $field->arguments()->[0]->buf() . '=' . $field->arguments()->[1]->emit_python() ) } else { die('Error in constructor, field: ', Main::perl($field, )) } }; return scalar (Python::tab($level) . '__builtin__.' . Main::to_go_namespace($self->{invocant}->name()) . '(' . Main::join($List_str, ', ') . ')') } ; if (Main::bool(((((($self->{method} eq 'id')) || (($self->{method} eq 'yaml'))) || (($self->{method} eq 'join'))) || (($self->{method} eq 'isa'))))) { if (Main::bool(($self->{hyper}))) { return scalar ('mp6_Array(map(lambda: Main.' . $self->{method} . '( v_self[0], ' . Main::join(([ map { $_->emit_python() } @{ $self->{arguments} } ]), ', ') . ') , ' . $invocant . '))') } else { return scalar ('mp6_' . $self->{method} . '(' . $invocant . ', ' . Main::join(([ map { $_->emit_python() } @{ $self->{arguments} } ]), ', ') . ')') } } ; ((my  $meth = undef) = $self->{method}); if (Main::bool(($meth eq 'postcircumfix:<( )>'))) { return scalar (Python::tab($level) . $invocant . '(' . Main::join(([ map { $_->emit_python() } @{ $self->{arguments} } ]), ', ') . ')') } ; if (Main::bool(((($meth eq 'values')) || (($meth eq 'keys'))))) { return scalar (Python::tab($level) . $invocant . '.' . $meth . '(' . Main::join(([ map { $_->emit_python() } @{ $self->{arguments} } ]), ', ') . ')') } ; if (Main::bool(($meth eq 'chars'))) { return scalar (Python::tab($level) . 'len(' . $invocant . ')') } ; ((my  $call = undef) = 'f_' . $meth . '(' . Main::join(([ map { $_->emit_python() } @{ $self->{arguments} } ]), ', ') . ')'); if (Main::bool(($self->{hyper}))) { Python::tab($level) . 'mp6_Array(map(lambda x: x.' . $call . ', ' . $invocant . '))' } else { Python::tab($level) . $invocant . '.' . $call } }
}
