...
for my $field ( @{($pos || []) || []} ) {     push( @{$args}, $field->emit_python_name() ) };
...
for my $decl ( @{$List_block || []} ) {     if (Main::bool((Main::isa($decl, 'Decl') && (($decl->decl() eq 'has'))))) {
            push( @{$has_decl}, $decl )
    }
    else {
...
for my $stmt ( @{$List_anon_block || []} ) {     push( @{$List_s}, $stmt->emit_python_indented($level) ) };
    push( @{$List_s}, $s2 ) };
...
for my $field ( @{($pos || []) || []} ) {     ((my  $arg = undef) = $field->emit_python_name());
    push( @{$args}, $arg );
