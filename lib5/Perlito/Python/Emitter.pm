...
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; return scalar (Python::tab($level) . ((Main::bool((($self->{twigil} eq '.'))) ? ('v_self[0].v_' . $self->{name} . '') : ((Main::bool((($self->{name} eq '/'))) ? ($table->{$self->{sigil}} . 'MATCH[0]') : ($table->{$self->{sigil}} . $self->{name} . '[0]')))))) };
sub emit_python_name { my $self = $_[0]; return scalar (((Main::bool((($self->{twigil} eq '.'))) ? ('v_self[0].v_' . $self->{name}) : ((Main::bool((($self->{name} eq '/'))) ? ($table->{$self->{sigil}} . 'MATCH') : ($table->{$self->{sigil}} . $self->{name})))))) }
}
...
sub emit_python_init { my $self = $_[0]; if (Main::bool((($self->{var})->sigil() eq '%'))) { return scalar ('mp6_Hash({})') } else { if (Main::bool((($self->{var})->sigil() eq '@'))) { return scalar ('mp6_Array([])') } else { return scalar ('mp6_Mu()') } }; return scalar ('') }
}
