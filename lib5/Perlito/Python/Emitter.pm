...
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; (my  $needs_interpolation = 0); my  $List_items; for my $item ( @{$self->{array1} || []} ) { if (Main::bool((Main::isa($item, 'Apply') && ((($item->code() eq 'circumfix:<( )>') || ($item->code() eq 'list:<,>')))))) { for my $arg ( @{[@{(($item->arguments()) || []) || []}] || []} ) { push( @{$List_items}, $arg ) } } else { push( @{$List_items}, $item ) } }; for my $item ( @{$List_items || []} ) { if (Main::bool(((Main::isa($item, 'Var') && ($item->sigil() eq '@')) || (Main::isa($item, 'Apply') && ((($item->code() eq 'prefix:<@>') || ($item->code() eq 'infix:<..>'))))))) { ($needs_interpolation = 1) }  }; if (Main::bool($needs_interpolation)) { my  $List_block; (my  $temp_array = Var->new(('name' => 'a'), ('namespace' => ''), ('sigil' => '@'), ('twigil' => ''))); (my  $input_array = Var->new(('name' => 'b'), ('namespace' => ''), ('sigil' => '@'), ('twigil' => ''))); push( @{$List_block}, Decl->new(('decl' => 'my'), ('type' => ''), ('var' => $temp_array)) ); (my  $index = 0); for my $item ( @{$List_items || []} ) { if (Main::bool(((Main::isa($item, 'Var') && ($item->sigil() eq '@')) || (Main::isa($item, 'Apply') && ((($item->code() eq 'prefix:<@>') || ($item->code() eq 'infix:<..>'))))))) { push( @{$List_block}, Call->new(('method' => 'extend'), ('arguments' => [Index->new(('obj' => $input_array), ('index_exp' => Val::Int->new(('int' => $index))))]), ('hyper' => ''), ('invocant' => $temp_array)) ) } else { push( @{$List_block}, Call->new(('method' => 'push'), ('arguments' => [Index->new(('obj' => $input_array), ('index_exp' => Val::Int->new(('int' => $index))))]), ('hyper' => ''), ('invocant' => $temp_array)) ) }; ($index = ($index + 1)) }; push( @{$List_block}, $temp_array ); (my  $label = '_anon_' . Perlito::Python::LexicalBlock::get_ident_python()); Perlito::Python::LexicalBlock::push_stmt_python(Perlito::Python::AnonSub->new(('name' => $label), ('block' => $List_block), ('sig' => Sig->new(('invocant' => undef), ('positional' => [$input_array]), ('named' => {  }))), ('handles_return_exception' => 1))); return(Python::tab($level) . 'f_' . $label . '(mp6_Array([' . Main::join(([ map { $_->emit_python() } @{ $List_items } ]), ', ') . ']))') } else { Python::tab($level) . 'mp6_Array([' . Main::join(([ map { $_->emit_python() } @{ $List_items } ]), ', ') . '])' } }
}
...
sub hash1 { $_[0]->{hash1} };
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; my  $List_s; my  $List_items; for my $item ( @{$self->{hash1} || []} ) { if (Main::bool((Main::isa($item, 'Apply') && ((($item->code() eq 'circumfix:<( )>') || ($item->code() eq 'list:<,>')))))) { for my $arg ( @{[@{(($item->arguments()) || []) || []}] || []} ) { push( @{$List_items}, $arg ) } } else { push( @{$List_items}, $item ) } }; push( @{$List_s}, Decl->new(('decl' => 'my'), ('type' => undef), ('var' => Var->new(('sigil' => '%'), ('twigil' => ''), ('name' => 'a')))) ); for my $item1 ( @{$List_items || []} ) { (my  $item = $item1); if (Main::bool((Main::isa($item, 'Do') && Main::isa($item->block(), 'Apply')))) { ($item = $item->block()) } ; if (Main::bool((Main::isa($item, 'Apply') && ($item->code() eq 'infix:<=>>')))) { push( @{$List_s}, Apply->new(('arguments' => [Lookup->new(('obj' => Var->new(('name' => 'a'), ('namespace' => ''), ('sigil' => '%'), ('twigil' => ''))), ('index_exp' => $item->arguments()->[0])), $item->arguments()->[1]]), ('code' => 'infix:<=>'), ('namespace' => '')) ) } else { if (Main::bool(((Main::isa($item, 'Var') && ($item->sigil() eq '%')) || (Main::isa($item, 'Apply') && ($item->code() eq 'prefix:<%>'))))) { push( @{$List_s}, Call->new(('arguments' => [$item]), ('hyper' => ''), ('invocant' => Var->new(('name' => 'a'), ('namespace' => ''), ('sigil' => '%'), ('twigil' => ''))), ('method' => 'update')) ) } else { die('Error in hash composer: ', Main::perl($item, )) } } }; push( @{$List_s}, Var->new(('sigil' => '%'), ('twigil' => ''), ('name' => 'a')) ); (my  $code = Do->new(('block' => Lit::Block->new(('stmts' => $List_s))))); $code->emit_python_indented($level) }
}
...
sub body { $_[0]->{body} };
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; (my  $body_block = Perlito::Python::LexicalBlock->new(('block' => $self->{body}->stmts()))); (my  $sig = 'v__'); if (Main::bool($self->{body}->sig())) { ($sig = $self->{body}->sig()->emit_python_name()) } ; if (Main::bool($body_block->has_my_decl())) { (my  $label = '_anon_' . Perlito::Python::LexicalBlock::get_ident_python()); (my  $anon_var = ($self->{body}->sig() || Var->new(('name' => '_'), ('namespace' => ''), ('sigil' => '$'), ('twigil' => '')))); (my  $anon_sig = Sig->new(('invocant' => undef), ('positional' => [$anon_var]), ('named' => {  }))); Perlito::Python::LexicalBlock::push_stmt_python(Perlito::Python::AnonSub->new(('name' => $label), ('block' => $self->{body}->stmts()), ('sig' => $anon_sig), ('handles_return_exception' => 0))); return(Python::tab($level) . 'for ' . $sig . ' in ' . $self->{cond}->emit_python() . ':' . '
