...
sub emit_ruby { my $self = $_[0]; $self->emit_ruby_indented(0) };
sub emit_ruby_indented { my $self = $_[0]; my $level = $_[1]; ((my  $needs_interpolation = undef) = 0); for my $item ( @{$self->{array1} || []} ) { if (Main::bool((((Main::isa($item, 'Var') && ($item->sigil() eq '@'))) || ((Main::isa($item, 'Apply') && ($item->code() eq 'prefix:<@>')))))) { ($needs_interpolation = 1) }  }; if (Main::bool($needs_interpolation)) { (my  $List_block = []); ((my  $temp_array = undef) = Var->new(('name' => 'a'), ('namespace' => ''), ('sigil' => '@'), ('twigil' => ''))); for my $item ( @{$self->{array1} || []} ) { if (Main::bool((((Main::isa($item, 'Var') && ($item->sigil() eq '@'))) || ((Main::isa($item, 'Apply') && ($item->code() eq 'prefix:<@>')))))) { push( @{$List_block}, Call->new(('method' => 'concat'), ('arguments' => [$item]), ('hyper' => ''), ('invocant' => $temp_array)) ) } else { push( @{$List_block}, Call->new(('method' => 'push'), ('arguments' => [$item]), ('hyper' => ''), ('invocant' => $temp_array)) ) } }; push( @{$List_block}, $temp_array ); ((my  $body_block = undef) = Perlito::Ruby::LexicalBlock->new(('block' => $List_block))); return(Ruby::tab($level) . 'Proc.new ' . '{' . ' |list_a| ' . '
...
sub hash1 { $_[0]->{hash1} };
sub emit_ruby { my $self = $_[0]; $self->emit_ruby_indented(0) };
sub emit_ruby_indented { my $self = $_[0]; my $level = $_[1]; ((my  $fields = undef) = $self->{hash1}); (my  $List_dict = []); for my $field ( @{($fields || []) || []} ) { push( @{$List_dict}, (($field->[0])->emit_ruby(("" . ' => ') . ($field->[1])->emit_ruby())) ) }; Ruby::tab($level) . '{' . Main::join($List_dict, ', ') . '}' }
}
...
sub body { $_[0]->{body} };
sub emit_ruby { my $self = $_[0]; $self->emit_ruby_indented(0) };
sub emit_ruby_indented { my $self = $_[0]; my $level = $_[1]; ((my  $body_block = undef) = Perlito::Ruby::LexicalBlock->new(('block' => $self->{body}))); if (Main::bool($body_block->has_my_decl())) { ($body_block = Do->new(('block' => $self->{body}))) } ; if (Main::bool(($self->{init} && $self->{continue}))) { die('not implemented (While)') } ; Ruby::tab($level) . 'while ' . Ruby::to_bool(' && ', [$self->{cond}]) . '
' . $body_block->emit_ruby_indented(($level + 1)) . '
' . Ruby::tab($level) . 'end' }
}
