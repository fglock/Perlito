...
sub end_token { $_[0]->{end_token} };
        ((my  $Operator = undef) = do {
(my  $Hash_a = {});
    $Hash_a
});
        ((my  $Precedence = undef) = do {
(my  $Hash_a = {});
    $Hash_a
});
        ((my  $Assoc = undef) = do {
(my  $Hash_a = {});
    $Hash_a
        ...
        ((my  $Allow_space_before = undef) = do {
(my  $Hash_a = {});
    $Hash_a
});
        sub is_assoc_type {
...
(my  $End_token = undef);
        sub op_parse {
            ...
            my $pos = $_[2];
            ((my  $from = undef) = $pos);
for my $tok ( @{(($End_token) || []) || []} ) {     ((my  $l = undef) = Main::chars($tok, ));
    ((my  $s = undef) = substr($str, $pos, $l));
    if (Main::bool(($s eq $tok))) {
            ((my  $c1 = undef) = substr($str, (($pos + $l) - 1), 1));
            ((my  $c2 = undef) = substr($str, ($pos + $l), 1));
            if (Main::bool((is_ident_middle($c1) && ((is_ident_middle($c2) || ($c2 eq '(')))))) {
...
(my  $List_v = []);
    ...
    $List_a
            ...
            if (Main::bool(((($c01 eq 'Â«')) || (($c01 eq 'Â»'))))) {
                    ($hyper_left = $c01);
                    ($pos = ($pos + 1));
                    ($c02 = substr($str, $pos, 2))
            }
            else {
                    if (Main::bool(((($c02 eq '<<')) || (($c02 eq '>>'))))) {
                            ($hyper_left = $c02);
                            ($pos = ($pos + 2));
                            ($c02 = substr($str, $pos, 2))
                    }
...
;
            ...
            if (Main::bool(exists($Op1->{$op1}))) {
                    ...
                    if (Main::bool((is_ident_middle($op1) && ((is_ident_middle($c2) || ($c2 eq '(')))))) {
...
;
        ...
        sub add_op {
            ...
            my $precedence = $_[2];
            my $param = $_[3];
            if (Main::bool(!Main::bool((defined($param))))) {
                    ($param = do {
(my  $Hash_a = {});
    $Hash_a
})
...
;
            ...
            ($Assoc->{$assoc}->{$name} = 1);
            ($Allow_space_before->{$fixity}->{$name} = (Main::bool($param->{'no_space_before'}) ? 0 : 1));
            if (Main::bool(((Main::chars($name, )) == 1))) {
                    ($Op1->{$name} = 1)
            }
            else {
                    if (Main::bool(((Main::chars($name, )) == 2))) {
                            ($Op2->{$name} = 1)
                    }
                    else {
                            if (Main::bool(((Main::chars($name, )) == 3))) {
                                    ($Op3->{$name} = 1)
                            }
...
(my  $Hash_a = {});
    ...
    $Hash_a
        ...
        sub precedence_parse {
            my $self = $_[0];
            ((my  $get_token = undef) = $self->get_token());
            ((my  $reduce = undef) = $self->reduce());
            ((my  $last_end_token = undef) = $End_token);
            ($End_token = $self->end_token());
            ((my  $op_stack = undef) = do {
(my  $List_a = []);
(my  $List_v = []);
    $List_a
});
            ((my  $num_stack = undef) = do {
(my  $List_a = []);
(my  $List_v = []);
    $List_a
});
...
(my  $List_v = []);
    ...
    $List_a
            ...
            ((my  $last_has_space = undef) = 0);
            ((my  $token = undef) = $get_token->());
            if (Main::bool((($token->[0]) eq 'space'))) {
                    ($token = $get_token->())
            }
;
for ( ; Main::bool(((defined($token)) && (($token->[0] ne 'end'))));  ) {     if (Main::bool(((($token->[1] eq ',')) && (((($last->[1] eq '*start*')) || (($last->[1] eq ','))))))) {
            push( @{$num_stack}, do {
(my  $List_a = []);
(my  $List_v = []);
    push( @{$List_a}, 'term' );
...
for ( ; Main::bool((scalar( @{$op_stack} ) && (($pr <= $Precedence->{($op_stack->[0])->[1]}))));  ) { $reduce->($op_stack, $num_stack) };
...
for ( ; Main::bool(scalar( @{$op_stack} ));  ) { $reduce->($op_stack, $num_stack) };
                            push( @{$num_stack}, $token );
                            ($End_token = $last_end_token);
                            return scalar ($num_stack)
                    }
                    else {
                            if (Main::bool(is_term($token))) {
                                    if (Main::bool(is_term($last))) {
                                            Main::say('#      last:  ', Main::perl($last, ));
                                            Main::say('#      token: ', Main::perl($token, ));
                                            Main::say('#      space: ', $last_has_space);
...
;
                                            if (Main::bool($Operator->{'ternary'}->{$token->[1]})) {
                                                    ($token->[0] = 'ternary')
                                            }
                                            else {
                                                    ($token->[0] = 'infix')
