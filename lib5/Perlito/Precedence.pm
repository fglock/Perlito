...
(my  $Op2 = undef);
(my  $End_token = undef);
sub op_parse { my $self = $_[0]; my $str = $_[1]; my $pos = $_[2]; ((my  $from = undef) = $pos); for my $tok ( @{(($End_token) || []) || []} ) { ((my  $l = undef) = Main::chars($tok, )); ((my  $s = undef) = substr($str, $pos, $l)); if (Main::bool(($s eq $tok))) { ((my  $c1 = undef) = substr($str, (($pos + $l) - 1), 1)); ((my  $c2 = undef) = substr($str, ($pos + $l), 1)); if (Main::bool((is_ident_middle($c1) && ((is_ident_middle($c2) || ($c2 eq '(')))))) {  } else { return(Perlito::Match->new(('str' => $str), ('from' => $from), ('to' => ($pos + 2)), ('bool' => 1), ('capture' => do { (my  $List_a = []); (my  $List_v = []); push( @{$List_a}, 'end' ); push( @{$List_a}, $s ); $List_a }))) } }  }; ((my  $c01 = undef) = substr($str, $pos, 1)); ((my  $c02 = undef) = substr($str, $pos, 2)); ((my  $hyper_left = undef) = 0); ((my  $hyper_right = undef) = 0); if (Main::bool(((($c01 eq '«')) || (($c01 eq '»'))))) { ($hyper_left = $c01); ($pos = ($pos + 1)); ($c02 = substr($str, $pos, 2)) } else { if (Main::bool(((($c02 eq '<<')) || (($c02 eq '>>'))))) { ($hyper_left = $c02); ($pos = ($pos + 2)); ($c02 = substr($str, $pos, 2)) }  }; ((my  $op2 = undef) = $c02); if (Main::bool(exists($Op2->{$op2}))) { ((my  $c1 = undef) = substr($str, ($pos + 1), 1)); ((my  $c2 = undef) = substr($str, ($pos + 2), 1)); if (Main::bool((is_ident_middle($c1) && ((is_ident_middle($c2) || ($c2 eq '(')))))) {  } else { ($pos = ($pos + 2)); ((my  $c01 = undef) = substr($str, $pos, 1)); ((my  $c02 = undef) = substr($str, $pos, 2)); if (Main::bool(((($c01 eq '«')) || (($c01 eq '»'))))) { ($hyper_right = $c01); ($pos = ($pos + 1)) } else { if (Main::bool(((($c02 eq '<<')) || (($c02 eq '>>'))))) { ($hyper_right = $c02); ($pos = ($pos + 2)) }  }; return(Perlito::Match->new(('str' => $str), ('from' => $from), ('to' => $pos), ('bool' => 1), ('capture' => do { (my  $List_a = []); (my  $List_v = []); push( @{$List_a}, 'op' ); push( @{$List_a}, $op2 ); push( @{$List_a}, do { (my  $Hash_a = {}); ($Hash_a->{'hyper_left'} = $hyper_left); ($Hash_a->{'hyper_right'} = $hyper_right); $Hash_a } ); $List_a }))) } } ; ((my  $op1 = undef) = substr($str, $pos, 1)); if (Main::bool(exists($Op1->{$op1}))) { ((my  $c2 = undef) = substr($str, ($pos + 1), 1)); if (Main::bool((is_ident_middle($op1) && ((is_ident_middle($c2) || ($c2 eq '(')))))) {  } else { ($pos = ($pos + 1)); ((my  $c01 = undef) = substr($str, $pos, 1)); ((my  $c02 = undef) = substr($str, $pos, 2)); if (Main::bool(((($c01 eq '«')) || (($c01 eq '»'))))) { ($hyper_right = $c01); ($pos = ($pos + 1)) } else { if (Main::bool(((($c02 eq '<<')) || (($c02 eq '>>'))))) { ($hyper_right = $c02); ($pos = ($pos + 2)) }  }; return(Perlito::Match->new(('str' => $str), ('from' => $from), ('to' => $pos), ('bool' => 1), ('capture' => do { (my  $List_a = []); (my  $List_v = []); push( @{$List_a}, 'op' ); push( @{$List_a}, $op1 ); push( @{$List_a}, do { (my  $Hash_a = {}); ($Hash_a->{'hyper_left'} = $hyper_left); ($Hash_a->{'hyper_right'} = $hyper_right); $Hash_a } ); $List_a }))) } } ; return(Perlito::Match->new(('bool' => 0))) };
sub add_op { my $fixity = $_[0]; my $name = $_[1]; my $precedence = $_[2]; my $param = $_[3]; if (Main::bool(((defined($param)) ? 0 : 1))) { ($param = do { (my  $Hash_a = {}); $Hash_a }) } ; ((my  $assoc = undef) = ($param->{'assoc'} || 'left')); ($Operator->{$fixity}->{$name} = 1); ($Precedence->{$name} = $precedence); ($Assoc->{$assoc}->{$name} = 1); ($Allow_space_before->{$fixity}->{$name} = (Main::bool($param->{'no_space_before'}) ? 0 : 1)); if (Main::bool(((Main::chars($name, )) == 1))) { ($Op1->{$name} = 1) } else { if (Main::bool(((Main::chars($name, )) == 2))) { ($Op2->{$name} = 1) }  } };
((my  $prec = undef) = 100);
