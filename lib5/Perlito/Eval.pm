...
our $MATCH = Perlito::Match->new();
{
    ...
    ;
    {
        ...
        sub eval {
            ...
            warn( 'Interpreter runtime error: variable \'',
                $name, '\' not found' );
        }
    ...
    ;
    {
        ...
        sub eval {
            ...
            warn( 'Interpreter runtime error: method \'',
        ...
        sub eval {
            ...
            for my $e ( @{ [ @{ ( ($env) || [] ) || [] } ] || [] } ) {
                if ( Main::bool( exists( $e->{$code} ) ) ) {
                    return (
                        ( ( $e->{$code} )->( $env, $self->{arguments} ) ) );
                }
            }
            warn( 'Interpreter runtime error: subroutine \'',
                $code, '()\' not found' );
        }
    }
    ...
    ;
    {
        ...
        sub eval {
            ...
            if (
                Main::bool( ( ( exists( ( $env->[0] )->{$name} ) ) ? 0 : 1 ) ) )
            {
                ( ( $env->[0] )->{$name} = undef );
            }
            return (undef);
