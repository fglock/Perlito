...
sub test_ends { if (Main::bool($testing_started)) {  } else { return((undef)) }; if (Main::bool($num_of_tests_planned)) {  } else { Main::say('1..' . $num_of_tests_run) }; if (Main::bool(($num_of_tests_planned != $num_of_tests_run))) { Main::say('# Looks like you planned ' . $num_of_tests_planned . ' tests, but ran ' . $num_of_tests_run) } else {  }; if (Main::bool($num_of_tests_failed)) { Main::say('# Looks like you failed ' . $num_of_tests_failed . ' tests of ' . $num_of_tests_run) } else {  }; ($num_of_tests_run = 0); ($num_of_tests_failed = 0); ($num_of_tests_planned = 0); ($testing_started = 0) }
}
