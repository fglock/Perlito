...
sub proclaim { my $cond = $_[0]; my $desc = $_[1]; my $todo = $_[2]; my $got = $_[3]; my $expected = $_[4]; my $depends = $_[5]; my $negate = $_[6]; ($testing_started = 1); ($num_of_tests_run = ($num_of_tests_run + 1)); if ($cond) { Main::say('ok ', $num_of_tests_run) } else { Main::say('not ok ', $num_of_tests_run);Test::report_failure($todo, $got, $expected, $negate) }; return($cond) };
sub report_failure { my $todo = $_[0]; my $got = $_[1]; my $expected = $_[2]; my $negate = $_[3]; Main::say('### Test::report_failure not implemented') };
sub test_ends { if ($testing_started) {  } else { return((undef)) }; if ($num_of_tests_planned) {  } else { Main::say('1..' . $num_of_tests_run) }; if (($num_of_tests_planned != $num_of_tests_run)) { Main::say('# Looks like you planned ' . $num_of_tests_planned . ' tests, but ran ' . $num_of_tests_run) } else {  }; if ($num_of_tests_failed) { Main::say('# Looks like you failed ' . $num_of_tests_failed . ' tests of ' . $num_of_tests_run) } else {  }; ($num_of_tests_run = 0); ($num_of_tests_failed = 0); ($num_of_tests_planned = 0); ($testing_started = 0) }
}
