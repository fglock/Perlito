...
sub double_quoted_buf {
    ...
    ($MATCH->{('capture')} = Perlito5::AST::Var->new(('sigil' => chr(36)), ('name' => $MATCH->{('ident')}->flat())));
    1
}))))
}))) || ((do {
    ($MATCH->{('to')} = $pos1);
...
sub val_buf {
    ...
    if ((!($args))) {
        ...
        ($MATCH->{('capture')} = Perlito5::AST::Apply->new(('namespace' => ''), ('code' => 'list:<.>'), ('arguments' => [map($_->capture(), @{$MATCH->{('double_quoted_buf')}})])))
    };
;
    1
})))
