...
;
    {
    package Perlito5::Clojure::LexicalBlock;
        sub new { shift; bless { @_ }, "Perlito5::Clojure::LexicalBlock" }
        sub block { $_[0]->{block} };
        sub emit_clojure {
            my $self = $_[0];
...
;
    ...
    package If;
        ...
        sub emit_clojure {
            my $self = $_[0];
            ((my  $block1) = Perlito5::Clojure::LexicalBlock->new(('block' => (defined $self->{body} ? $self->{body} : ($self->{body} ||= bless([], 'ARRAY'))))));
            ((my  $block2) = Perlito5::Clojure::LexicalBlock->new(('block' => (defined $self->{otherwise} ? $self->{otherwise} : ($self->{otherwise} ||= bless([], 'ARRAY'))))));
...
;
    ...
    package For;
        ...
        sub emit_clojure {
            my $self = $_[0];
            ((my  $cond) = $self->{cond});
            ((my  $block) = Perlito5::Clojure::LexicalBlock->new(('block' => (defined $self->{body} ? $self->{body} : ($self->{body} ||= bless([], 'ARRAY'))))));
...
;
    ...
    package Sub;
        ...
        sub emit_clojure {
            ...
            ((my  $sig) = $self->{sig});
            ((my  $pos) = $sig->positional());
            ((my  $block) = Perlito5::Clojure::LexicalBlock->new(('block' => (defined $self->{block} ? $self->{block} : ($self->{block} ||= bless([], 'ARRAY'))))));
            (my  $str);
            if (($pos)) {
                for my $field ( @{($pos)} ) {
