...
;
    {
    package Perlito6::Lisp::LexicalBlock;
        sub new { shift; bless { @_ }, "Perlito6::Lisp::LexicalBlock" }
        sub block { $_[0]->{block} };
        sub emit_lisp {
            my $self = $_[0];
...
;
    ...
    package If;
        ...
        sub emit_lisp {
            my $self = $_[0];
            ((my  $block1) = Perlito6::Lisp::LexicalBlock->new(('block' => $self->{body}->stmts())));
            if ($self->{otherwise}) {
                ((my  $block2) = Perlito6::Lisp::LexicalBlock->new(('block' => $self->{otherwise}->stmts())));
...
;
    ...
    package For;
        ...
        sub emit_lisp {
            my $self = $_[0];
            ((my  $cond) = $self->{cond});
            ((my  $block) = Perlito6::Lisp::LexicalBlock->new(('block' => (defined $self->{body} ? $self->{body} : ($self->{body} ||= bless([], 'ARRAY'))))));
...
;
    ...
    package While;
        ...
        sub emit_lisp {
            ...
            ('(progn ' . (($self->{init} ? ($self->{init}->emit_lisp() . ' ') : '')) . '(loop while (sv-bool ' . $self->{cond}->emit_lisp() . ') do ' . (Perlito6::Lisp::LexicalBlock->new(('block' => $List_body)))->emit_lisp() . '))')
        }
    }
...
;
    ...
    package Sub;
        ...
        sub emit_lisp {
            ...
            ((my  $sig) = $self->{sig});
            ((my  $pos) = $sig->positional());
            ((my  $block) = Perlito6::Lisp::LexicalBlock->new(('block' => (defined $self->{block} ? $self->{block} : ($self->{block} ||= bless([], 'ARRAY'))))));
            (my  $str);
            if (($pos)) {
                for my $field ( @{($pos)} ) {
