...
;
    {
    package Perlito6::Grammar::Regex;
        sub new { shift; bless { @_ }, "Perlito6::Grammar::Regex" }
    ...
    ($MATCH->{capture} = (($MATCH->{'Perlito6::Grammar.var_sigil'} . chr(47) . '[' . $MATCH->{'Perlito6::Grammar.val_int'} . ']')))
}) || 1))))
})) || (do {
    (($MATCH)->{to} = $pos1);
    (((((do {
    ...
    ($MATCH->{capture} = (Rul::Var->new(('sigil' => "".($MATCH->{'Perlito6::Grammar.var_sigil'})), ('twigil' => "".($MATCH->{'Perlito6::Grammar.var_twigil'})), ('name' => "".($MATCH->{'Perlito6::Grammar.full_ident'})))))
}) || 1))))
}))
})));
            $MATCH
