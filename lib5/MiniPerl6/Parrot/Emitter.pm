...
sub emit_parrot { my $self = shift; my $List__ = \@_; do { [] }; (my  $a = $self->{body}); my  $item; (my  $s = '.namespace [ ' . '"' . $self->{name} . '"' . ' ] ' . '
' . '.sub _ :main :anon' . '
' . '.end' . '
' . '
...
sub emit_parrot { my $self = shift; my $List__ = \@_; do { [] }; (my  $a = $self->{array1}); my  $item; (my  $s = '  save $P1' . '
...
sub emit_parrot { my $self = shift; my $List__ = \@_; do { [] }; (my  $a = $self->{hash1}); my  $item; (my  $s = '  save $P1' . '
' . '  save $P2' . '
' . '  $P1 = new .Hash' . '
...
sub class { @_ == 1 ? ( $_[0]->{class} ) : ( $_[0]->{class} = $_[1] ) };
sub fields { @_ == 1 ? ( $_[0]->{fields} ) : ( $_[0]->{fields} = $_[1] ) };
sub emit_parrot { my $self = shift; my $List__ = \@_; do { [] }; (my  $fields = $self->{fields}); (my  $str = ''); ($str = '  save $P1' . '
' . '  save $S2' . '
' . '  $P1 = new ' . '"' . $self->{class} . '"' . '
...
sub name { @_ == 1 ? ( $_[0]->{name} ) : ( $_[0]->{name} = $_[1] ) };
sub emit_parrot { my $self = shift; my $List__ = \@_; do { [] }; (($self->{twigil} eq '.') ? '  $P0 = getattribute self, \'' . $self->{name} . '\'' . '
' : '  $P0 = ' . $self->full_name() . ' ' . '
') };
...
sub arguments { @_ == 1 ? ( $_[0]->{arguments} ) : ( $_[0]->{arguments} = $_[1] ) };
(my  $label = 100);
sub emit_parrot { my $self = shift; my $List__ = \@_; do { [] }; (my  $code = $self->{code}); do { if (($code eq 'die')) { return('  $P0 = new .Exception' . '
...
(my  $label = 100);
sub emit_parrot { my $self = shift; my $List__ = \@_; do { [] }; ($label = ($label + 1)); (my  $id = $label); return($self->{cond}->emit_parrot() . '  unless $P0 goto ifelse' . $id . '
...
sub emit_parrot { my $self = shift; my $List__ = \@_; do { [] }; (my  $decl = $self->{decl}); (my  $name = $self->{var}->name()); (($decl eq 'has') ? '  addattribute self, ' . '"' . $name . '"' . '
...
sub emit_parrot { my $self = shift; my $List__ = \@_; do { [] }; (my  $sig = $self->{sig}); (my  $invocant = $sig->invocant()); (my  $pos = $sig->positional()); (my  $str = ''); (my  $i = 0); my  $field; do { for my $field ( @{$pos} ) { ($str = $str . '  $P0 = params[' . $i . ']' . '
