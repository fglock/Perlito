...
sub emit_parrot { my $self = shift; my $List__ = \@_; do { [] }; (my  $a = $self->{array1}); my  $item; (my  $s = '  save $P1' . Main::newline() . '  $P1 = new .ResizablePMCArray' . Main::newline()); do { for my $item ( @{$a} ) { ($s = $s . $item->emit_parrot());($s = $s . '  push $P1, $P0' . Main->newline()) } }; (my  $s = $s . '  $P0 = $P1' . Main::newline() . '  restore $P1' . Main::newline()); return($s) }
