...
sub block { $_[0]->{block} };
sub emit_ruby { my $self = $_[0]; $self->emit_ruby_indented(0) };
sub emit_ruby_indented { my $self = $_[0]; my $level = $_[1]; my  $List_s; push( @{$List_s}, Ruby::tab($level) . 'lambda{ || ' ); push( @{$List_s}, MiniPerl6::Ruby::LexicalBlock->new( 'block' => $self->{block},'needs_return' => 1, )->emit_ruby(($level + 1)) ); push( @{$List_s}, Ruby::tab($level) . '}.call()' ); return(Main::join($List_s, '
')) }
}
