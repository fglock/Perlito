...
sub fields { $_[0]->{fields} };
sub emit_ruby { my $self = $_[0]; $self->emit_ruby_indented(0) };
sub emit_ruby_indented { my $self = $_[0]; my $level = $_[1]; (my  $fields = $self->{fields}); my  $List_str; for my $field ( @{$fields || []} ) { push( @{$List_str}, 'o.v_' . $field->[0]->buf() . '=' . $field->[1]->emit_ruby() . '; ' ) }; Ruby::tab($level) . 'Proc.new { |o| ' . Main::join($List_str, ' ') . 'o }.call(' . Main::to_go_namespace($self->{class}) . '.new)' }
}
