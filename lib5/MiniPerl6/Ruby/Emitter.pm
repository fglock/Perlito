...
sub emit_ruby { my $self = $_[0]; $self->emit_ruby_indented(0) };
sub emit_ruby_indented { my $self = $_[0]; my $level = $_[1]; (my  $needs_interpolation = 0); for my $item ( @{$self->{array1} || []} ) { if (Main::bool(((Main::isa($item, 'Var') && ($item->sigil() eq '@')) || (Main::isa($item, 'Apply') && ($item->code() eq 'prefix:<@>'))))) { ($needs_interpolation = 1) } else {  } }; if (Main::bool($needs_interpolation)) { my  $List_block;(my  $temp_array = Var->new( 'name' => 'a','namespace' => '','sigil' => '@','twigil' => '', ));(my  $input_array = Var->new( 'name' => 'b','namespace' => '','sigil' => '@','twigil' => '', ));push( @{$List_block}, Decl->new( 'decl' => 'my','type' => '','var' => $temp_array, ) );(my  $index = 0);for my $item ( @{$self->{array1} || []} ) { if (Main::bool(((Main::isa($item, 'Var') && ($item->sigil() eq '@')) || (Main::isa($item, 'Apply') && ($item->code() eq 'prefix:<@>'))))) { push( @{$List_block}, Call->new( 'method' => 'concat','arguments' => [$item],'hyper' => '','invocant' => $temp_array, ) ) } else { push( @{$List_block}, Call->new( 'method' => 'push','arguments' => [$item],'hyper' => '','invocant' => $temp_array, ) ) };($index = ($index + 1)) };push( @{$List_block}, $temp_array );(my  $label = '_anon_' . MiniPerl6::Ruby::LexicalBlock::get_ident_ruby());(my  $code = MiniPerl6::Ruby::AnonSub->new( 'name' => '','block' => $List_block,'sig' => Sig->new( 'invocant' => (undef),'positional' => [],'named' => {  }, ),'handles_return_exception' => 0, ));return(Ruby::tab($level) . $code->emit_ruby() . '.call()') } else { Ruby::tab($level) . 'Mp6_Array.new([' . Main::join([ map { $_->emit_ruby() } @{ $self->{array1} } ], ', ') . '])' } }
}
