...
sub to_str { my $cond = $_[0]; if (Main::bool(Main::isa($cond, 'Val::Buf'))) { return($cond->emit_ruby()) } else {  }; return('(' . $cond->emit_ruby() . ').to_s') };
...
sub emit_ruby { my $self = $_[0]; $self->emit_ruby_indented(0) };
sub emit_ruby_indented { my $self = $_[0]; my $level = $_[1]; Ruby::tab($level) . 'mp6_Undef()' }
}
...
sub emit_ruby { my $self = $_[0]; $self->emit_ruby_indented(0) };
sub emit_ruby_indented { my $self = $_[0]; my $level = $_[1]; return(Ruby::tab($level) . (Main::bool(($self->{twigil} eq '.')) ? 'v_self[0].v_' . $self->{name} . '' : (Main::bool(($self->{name} eq '/')) ? $table->{$self->{sigil}} . 'MATCH[0]' : $table->{$self->{sigil}} . $self->{name} . '[0]'))) };
sub emit_ruby_name { my $self = $_[0]; return((Main::bool(($self->{twigil} eq '.')) ? 'v_self[0].v_' . $self->{name} : (Main::bool(($self->{name} eq '/')) ? $table->{$self->{sigil}} . 'MATCH' : $table->{$self->{sigil}} . $self->{name}))) };
...
sub emit_ruby { my $self = $_[0]; if (Main::bool(Main::isa($self->{arguments}->[1], 'Apply'))) { (my  $args2 = $self->{arguments}->[1]->arguments());if (Main::bool(Main::isa($args2->[1], 'Apply'))) { ($args2->[1] = Do->new( 'block' => [$args2->[1]], )) } else {  } } else {  }; (my  $code = $self->{code}); if (Main::bool(Main::isa($code, 'Str'))) {  } else { return('(' . $self->{code}->emit_ruby() . ').(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ', ') . ')') }; if (Main::bool(($code eq 'self'))) { return('v_self[0]') } else {  }; if (Main::bool(($code eq 'make'))) { return('v_MATCH[0].__setattr__(\'v_capture\', ' . $self->{arguments}->[0]->emit_ruby() . ')') } else {  }; if (Main::bool(($code eq 'false'))) { return('False') } else {  }; if (Main::bool(($code eq 'true'))) { return('True') } else {  }; if (Main::bool(($code eq 'say'))) { my  $List_s;for my $arg ( @{$self->{arguments} || []} ) { push( @{$List_s}, Ruby::to_str($arg) ) };return('puts(' . Main::join($List_s, ' + ') . ')') } else {  }; if (Main::bool(($code eq 'print'))) { my  $List_s;for my $arg ( @{$self->{arguments} || []} ) { push( @{$List_s}, Ruby::to_str($arg) ) };return('print(' . Main::join($List_s, ' + ') . ')') } else {  }; if (Main::bool(($code eq 'warn'))) { return('mp6_warn(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; if (Main::bool(($code eq 'array'))) { return('[' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ' ') . ']') } else {  }; if (Main::bool(($code eq 'Int'))) { return('mp6_to_num(' . $self->{arguments}->[0]->emit_ruby() . ')') } else {  }; if (Main::bool(($code eq 'Num'))) { return('mp6_to_num(' . $self->{arguments}->[0]->emit_ruby() . ')') } else {  }; if (Main::bool(($code eq 'prefix:<~>'))) { return('str(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ' ') . ')') } else {  }; if (Main::bool(($code eq 'prefix:<!>'))) { return('not (' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ' ') . ')') } else {  }; if (Main::bool(($code eq 'prefix:<?>'))) { return('not (not (' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ' ') . '))') } else {  }; if (Main::bool(($code eq 'prefix:<$>'))) { return('mp6_to_scalar(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ' ') . ')') } else {  }; if (Main::bool(($code eq 'prefix:<@>'))) { return('(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ' ') . ')') } else {  }; if (Main::bool(($code eq 'prefix:<%>'))) { return('%{' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ' ') . '}') } else {  }; if (Main::bool(($code eq 'infix:<~>'))) { return('(str(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ') + str(') . '))') } else {  }; if (Main::bool(($code eq 'infix:<+>'))) { return('(mp6_to_num(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ') + mp6_to_num(') . '))') } else {  }; if (Main::bool(($code eq 'infix:<->'))) { return('(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ' - ') . ')') } else {  }; if (Main::bool(($code eq 'infix:<*>'))) { return('(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ' * ') . ')') } else {  }; if (Main::bool(($code eq 'infix:</>'))) { return('(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ' / ') . ')') } else {  }; if (Main::bool(($code eq 'infix:<&&>'))) { return('(mp6_to_bool(' . $self->{arguments}->[0]->emit_ruby() . ') ' . 'and mp6_to_bool(' . $self->{arguments}->[1]->emit_ruby() . '))') } else {  }; if (Main::bool(($code eq 'infix:<||>'))) { return('(mp6_to_bool(' . $self->{arguments}->[0]->emit_ruby() . ') ' . 'or mp6_to_bool(' . $self->{arguments}->[1]->emit_ruby() . '))') } else {  }; if (Main::bool(($code eq 'infix:<eq>'))) { return('(str(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ') == str(') . '))') } else {  }; if (Main::bool(($code eq 'infix:<ne>'))) { return('(str(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ') != str(') . '))') } else {  }; if (Main::bool(($code eq 'infix:<==>'))) { return('(mp6_to_num(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ') == mp6_to_num(') . '))') } else {  }; if (Main::bool(($code eq 'infix:<!=>'))) { return('(mp6_to_num(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ') != mp6_to_num(') . '))') } else {  }; if (Main::bool(($code eq 'infix:<<>'))) { return('(mp6_to_num(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ') < mp6_to_num(') . '))') } else {  }; if (Main::bool(($code eq 'infix:<>>'))) { return('(mp6_to_num(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ') > mp6_to_num(') . '))') } else {  }; if (Main::bool(($code eq 'exists'))) { (my  $arg = $self->{arguments}->[0]);if (Main::bool(Main::isa($arg, 'Lookup'))) { return('(' . $arg->obj()->emit_ruby() . ').has_key(' . $arg->index_exp()->emit_ruby() . ')') } else {  } } else {  }; if (Main::bool(($code eq 'ternary:<?? !!>'))) { (my  $ast = Do->new( 'block' => [If->new( 'cond' => $self->{arguments}->[0],'body' => [$self->{arguments}->[1]],'otherwise' => [$self->{arguments}->[2]], )], ));return($ast->emit_ruby()) } else {  }; if (Main::bool(($code eq 'substr'))) { return($self->{arguments}->[0]->emit_ruby() . '[' . $self->{arguments}->[1]->emit_ruby() . ':' . $self->{arguments}->[1]->emit_ruby() . ' + ' . $self->{arguments}->[2]->emit_ruby() . ']') } else {  }; if (Main::bool(($code eq 'index'))) { return('mp6_index(' . $self->{arguments}->[0]->emit_ruby() . ', ' . $self->{arguments}->[1]->emit_ruby() . ')') } else {  }; if (Main::bool(($code eq 'shift'))) { return($self->{arguments}->[0]->emit_ruby() . '.f_shift()') } else {  }; if (Main::bool(($code eq 'pop'))) { return($self->{arguments}->[0]->emit_ruby() . '.f_pop()') } else {  }; if (Main::bool(($code eq 'push'))) { return($self->{arguments}->[0]->emit_ruby() . '.f_push(' . $self->{arguments}->[1]->emit_ruby() . ')') } else {  }; if (Main::bool(($code eq 'unshift'))) { return($self->{arguments}->[0]->emit_ruby() . '.f_unshift(' . $self->{arguments}->[1]->emit_ruby() . ')') } else {  }; if (Main::bool($self->{namespace})) { return(Main::to_go_namespace($self->{namespace}) . '_proto.f_' . $self->{code} . '(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; 'f_' . $self->{code} . '(' . Main::join([ map { $_->emit_ruby() } @{ $self->{arguments} } ], ', ') . ')' };
sub emit_ruby_indented { my $self = $_[0]; my $level = $_[1]; Ruby::tab($level) . $self->emit_ruby() }
}
...
sub emit_ruby_init { my $self = $_[0]; if (Main::bool(($self->{var}->sigil() eq '%'))) { return('mp6_Hash({})') } else { if (Main::bool(($self->{var}->sigil() eq '@'))) { return('mp6_Array([])') } else { return('mp6_Undef()') } }; return('') }
}
