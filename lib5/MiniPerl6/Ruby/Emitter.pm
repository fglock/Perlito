...
sub emit_ruby { my $self = $_[0]; $self->emit_ruby_indented(0) };
sub emit_ruby_indented { my $self = $_[0]; my $level = $_[1]; (my  $has_body = (Main::bool(($self->{body} || [])) ? 1 : 0)); (my  $has_otherwise = (Main::bool(($self->{otherwise} || [])) ? 1 : 0)); (my  $body_block = MiniPerl6::Ruby::LexicalBlock->new(('block' => $self->{body}))); (my  $otherwise_block = MiniPerl6::Ruby::LexicalBlock->new(('block' => $self->{otherwise}))); if (Main::bool($body_block->has_my_decl())) { ($body_block = Do->new(('block' => $self->{body}))) } ; if (Main::bool(($has_otherwise && $otherwise_block->has_my_decl()))) { ($otherwise_block = Do->new(('block' => $self->{otherwise}))) } ; (my  $s = Ruby::tab($level) . 'if ' . Ruby::to_bool(' && ', [$self->{cond}]) . '
...
sub body { $_[0]->{body} };
sub emit_ruby { my $self = $_[0]; $self->emit_ruby_indented(0) };
sub emit_ruby_indented { my $self = $_[0]; my $level = $_[1]; (my  $body_block = MiniPerl6::Ruby::LexicalBlock->new(('block' => $self->{body}))); if (Main::bool($body_block->has_my_decl())) { ($body_block = Do->new(('block' => $self->{body}))) } ; if (Main::bool(($self->{init} && $self->{continue}))) { die('not implemented (While)') } ; Ruby::tab($level) . 'while ' . Ruby::to_bool(' && ', [$self->{cond}]) . '
' . $body_block->emit_ruby_indented(($level + 1)) . '
' . Ruby::tab($level) . 'end' }
}
