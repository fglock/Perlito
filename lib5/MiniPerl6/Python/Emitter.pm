...
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; return(Python::tab($level) . (($self->{twigil} eq '.') ? 'v_self[0].v_' . $self->{name} . '' : (($self->{name} eq '/') ? $table->{$self->{sigil}} . 'MATCH[0]' : $table->{$self->{sigil}} . $self->{name} . '[0]'))) };
sub emit_python_name { my $self = $_[0]; return((($self->{twigil} eq '.') ? 'v_self[0].v_' . $self->{name} : (($self->{name} eq '/') ? $table->{$self->{sigil}} . 'MATCH' : $table->{$self->{sigil}} . $self->{name}))) };
...
sub arguments { $_[0]->{arguments} };
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; (my  $invocant = $self->{invocant}->emit_python()); if ((($self->{method} eq 'perl') || (($self->{method} eq 'yaml') || (($self->{method} eq 'say') || (($self->{method} eq 'join') || ($self->{method} eq 'isa')))))) { if ($self->{hyper}) { return('map(lambda: Main.' . $self->{method} . '( v_self[0], ' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ', ') . ') , ' . $invocant . ')
') } else { return('mp6_' . $self->{method} . '(' . $invocant . ', ' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ', ') . ')') } } else {  }; (my  $meth = $self->{method}); if (($meth eq 'postcircumfix:<( )>')) { return(Python::tab($level) . $invocant . '(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; if ((($meth eq 'values') || ($meth eq 'keys'))) { return(Python::tab($level) . $invocant . '.' . $meth . '(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; if (($meth eq 'chars')) { return(Python::tab($level) . 'len(' . $invocant . ')') } else {  }; (my  $call = 'f_' . $meth . '(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ', ') . ')'); if ($self->{hyper}) { Python::tab($level) . 'map(lambda x: x.' . $call . ', ' . $invocant . ')' } else { Python::tab($level) . $invocant . '.' . $call } }
}
...
sub emit_python { my $self = $_[0]; if (Main::isa($self->{arguments}->[1], 'Apply')) { (my  $args2 = $self->{arguments}->[1]->arguments());if (Main::isa($args2->[1], 'Apply')) { ($args2->[1] = Do->new( 'block' => [$args2->[1]], )) } else {  } } else {  }; (my  $code = $self->{code}); if (Main::isa($code, 'Str')) {  } else { return('(' . $self->{code}->emit_python() . ').(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ', ') . ')') }; if (($code eq 'self')) { return('v_self[0]') } else {  }; if (($code eq 'make')) { return('v_MATCH[0].__setattr__(\'v_capture\', ' . $self->{arguments}->[0]->emit_python() . ')') } else {  }; if (($code eq 'false')) { return('False') } else {  }; if (($code eq 'say')) { return('mp6_say(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; if (($code eq 'print')) { return('mp6_print(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; if (($code eq 'warn')) { return('mp6_warn(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; if (($code eq 'array')) { return('[' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ' ') . ']') } else {  }; if (($code eq 'Int')) { return('mp6_to_num(' . $self->{arguments}->[0]->emit_python() . ')') } else {  }; if (($code eq 'Num')) { return('mp6_to_num(' . $self->{arguments}->[0]->emit_python() . ')') } else {  }; if (($code eq 'prefix:<~>')) { return('str(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ' ') . ')') } else {  }; if (($code eq 'prefix:<!>')) { return('not (' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ' ') . ')') } else {  }; if (($code eq 'prefix:<?>')) { return('not (not (' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ' ') . '))') } else {  }; if (($code eq 'prefix:<$>')) { return('mp6_to_scalar(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ' ') . ')') } else {  }; if (($code eq 'prefix:<@>')) { return('(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ' ') . ')') } else {  }; if (($code eq 'prefix:<%>')) { return('%{' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ' ') . '}') } else {  }; if (($code eq 'infix:<~>')) { return('(str(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ') + str(') . '))') } else {  }; if (($code eq 'infix:<+>')) { return('(mp6_to_num(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ') + mp6_to_num(') . '))') } else {  }; if (($code eq 'infix:<->')) { return('(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ' - ') . ')') } else {  }; if (($code eq 'infix:<*>')) { return('(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ' * ') . ')') } else {  }; if (($code eq 'infix:</>')) { return('(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ' / ') . ')') } else {  }; if (($code eq 'infix:<&&>')) { return('(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ' and ') . ')') } else {  }; if (($code eq 'infix:<||>')) { return('(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ' or ') . ')') } else {  }; if (($code eq 'infix:<eq>')) { return('(str(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ') == str(') . '))') } else {  }; if (($code eq 'infix:<ne>')) { return('(str(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ') != str(') . '))') } else {  }; if (($code eq 'infix:<==>')) { return('(mp6_to_num(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ') == mp6_to_num(') . '))') } else {  }; if (($code eq 'infix:<!=>')) { return('(mp6_to_num(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ') != mp6_to_num(') . '))') } else {  }; if (($code eq 'infix:<<>')) { return('(mp6_to_num(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ') < mp6_to_num(') . '))') } else {  }; if (($code eq 'infix:<>>')) { return('(mp6_to_num(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ') > mp6_to_num(') . '))') } else {  }; if (($code eq 'exists')) { (my  $arg = $self->{arguments}->[0]);if (Main::isa($arg, 'Lookup')) { return('(' . $arg->obj()->emit_python() . ').has_key(' . $arg->index_exp()->emit_python() . ')') } else {  } } else {  }; if (($code eq 'ternary:<?? !!>')) { (my  $ast = Do->new( 'block' => [If->new( 'cond' => $self->{arguments}->[0],'body' => [$self->{arguments}->[1]],'otherwise' => [$self->{arguments}->[2]], )], ));return($ast->emit_python()) } else {  }; if (($code eq 'substr')) { return($self->{arguments}->[0]->emit_python() . '[' . $self->{arguments}->[1]->emit_python() . ':' . $self->{arguments}->[1]->emit_python() . ' + ' . $self->{arguments}->[2]->emit_python() . ']') } else {  }; if (($code eq 'index')) { return('mp6_index(' . $self->{arguments}->[0]->emit_python() . ', ' . $self->{arguments}->[1]->emit_python() . ')') } else {  }; if (($code eq 'shift')) { return($self->{arguments}->[0]->emit_python() . '.f_shift()') } else {  }; if (($code eq 'pop')) { return($self->{arguments}->[0]->emit_python() . '.f_pop()') } else {  }; if (($code eq 'push')) { return($self->{arguments}->[0]->emit_python() . '.f_push(' . $self->{arguments}->[1]->emit_python() . ')') } else {  }; if (($code eq 'unshift')) { return($self->{arguments}->[0]->emit_python() . '.f_unshift(' . $self->{arguments}->[1]->emit_python() . ')') } else {  }; if ($self->{namespace}) { return(Main::to_go_namespace($self->{namespace}) . '_proto.f_' . $self->{code} . '(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; 'f_' . $self->{code} . '(' . Main::join([ map { $_->emit_python() } @{ $self->{arguments} } ], ', ') . ')' };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; Python::tab($level) . $self->emit_python() }
}
