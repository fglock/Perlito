...
sub body { $_[0]->{body} };
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; my  $List_s; (my  $block = MiniPerl6::Python::LexicalBlock->new( 'block' => $self->{body}, )); (my  $label = '_anon_' . MiniPerl6::Python::LexicalBlock::get_ident_python()); push( @{$List_s}, Python::tab($level) . 'try:' ); push( @{$List_s}, Python::tab(($level + 1)) . 'type(' . $self->{name} . ')' ); push( @{$List_s}, Python::tab($level) . 'except NameError:' ); push( @{$List_s}, Python::tab(($level + 1)) . 'class ' . $self->{name} . ':' ); push( @{$List_s}, Python::tab(($level + 2)) . 'def __init__(v_self, **arg):' ); push( @{$List_s}, Python::tab(($level + 3)) . 'for kw in arg.keys():' ); push( @{$List_s}, Python::tab(($level + 4)) . 'v_self.__dict__.update({\'v_\' + kw:arg[kw]})' ); push( @{$List_s}, Python::tab($level) . $self->{name} . '_proto = ' . $self->{name} . '()' ); push( @{$List_s}, Python::tab($level) . 'class ' . $label . ':' ); push( @{$List_s}, Python::tab(($level + 1)) . 'self = ' . $self->{name} ); push( @{$List_s}, $block->emit_python_indented(($level + 1)) ); return(Main::join($List_s, '
')) }
}
...
sub fields { $_[0]->{fields} };
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; (my  $fields = $self->{fields}); my  $List_str; for my $field ( @{$fields} ) { push( @{$List_str}, $field->[0]->buf() . '=' . $field->[1]->emit_python() ) }; Python::tab($level) . Main::to_python_namespace($self->{class}) . '(' . Main::join($List_str, ', ') . ')' }
}
