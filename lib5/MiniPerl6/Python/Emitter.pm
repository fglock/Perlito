...
sub emit_python { my $self = $_[0]; $self->emit_python_indented(0) };
sub emit_python_indented { my $self = $_[0]; my $level = $_[1]; if (Main::isa($self->{parameters}, 'Index')) { return(Python::tab($level) . $self->{parameters}->obj()->emit_python() . '.set(' . $self->{parameters}->index_exp()->emit_python() . ', ' . $self->{arguments}->emit_python() . ')') } else {  }; if (Main::isa($self->{parameters}, 'Call')) { return(Python::tab($level) . $self->{parameters}->invocant()->emit_python() . '.' . $self->{parameters}->method() . ' = ' . $self->{arguments}->emit_python()) } else {  }; Python::tab($level) . $self->{parameters}->emit_python() . ' = ' . $self->{arguments}->emit_python() }
}
