...
sub arguments { $_[0]->{arguments} };
sub namespace { $_[0]->{namespace} };
sub emit { my $self = $_[0]; (my  $ns = ''); if ($self->{namespace}) { ($ns = $self->{namespace} . '::') } else {  }; (my  $code = $ns . $self->{code}); if (Main::isa($code, 'Str')) {  } else { return('(' . $self->{code}->emit() . ')->(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')') }; if (($code eq 'self')) { return('$self') } else {  }; if (($code eq 'false')) { return('0') } else {  }; if (($code eq 'make')) { return('($MATCH->{capture} = (' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . '))') } else {  }; if (($code eq 'say')) { return('Main::say(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; if (($code eq 'print')) { return('Main::print(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; if (($code eq 'warn')) { return('warn(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; if (($code eq 'array')) { return('@{' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . '}') } else {  }; if (($code eq 'pop')) { return('pop( @{' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . '} )') } else {  }; if (($code eq 'push')) { return('push( @{' . $self->{arguments}->[0]->emit() . '}, ' . $self->{arguments}->[1]->emit() . ' )') } else {  }; if (($code eq 'shift')) { return('shift( @{' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . '} )') } else {  }; if (($code eq 'Int')) { return('(0+' . $self->{arguments}->[0]->emit() . ')') } else {  }; if (($code eq 'Num')) { return('(0+' . $self->{arguments}->[0]->emit() . ')') } else {  }; if (($code eq 'prefix:<~>')) { return('("" . ' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')') } else {  }; if (($code eq 'prefix:<!>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ' ? 0 : 1)') } else {  }; if (($code eq 'prefix:<?>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ' ? 1 : 0)') } else {  }; if (($code eq 'prefix:<$>')) { return('${' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . '}') } else {  }; if (($code eq 'prefix:<@>')) { return('@{' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . '}') } else {  }; if (($code eq 'prefix:<%>')) { return('%{' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . '}') } else {  }; if (($code eq 'infix:<~>')) { return('' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' . ') . '') } else {  }; if (($code eq 'infix:<+>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' + ') . ')') } else {  }; if (($code eq 'infix:<->')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' - ') . ')') } else {  }; if (($code eq 'infix:<*>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' * ') . ')') } else {  }; if (($code eq 'infix:</>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' / ') . ')') } else {  }; if (($code eq 'infix:<>>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' > ') . ')') } else {  }; if (($code eq 'infix:<<>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' < ') . ')') } else {  }; if (($code eq 'infix:<>=>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' >= ') . ')') } else {  }; if (($code eq 'infix:<<=>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' <= ') . ')') } else {  }; if (($code eq 'infix:<x>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' x ') . ')') } else {  }; if (($code eq 'infix:<&&>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' && ') . ')') } else {  }; if (($code eq 'infix:<||>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' || ') . ')') } else {  }; if (($code eq 'infix:<eq>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' eq ') . ')') } else {  }; if (($code eq 'infix:<ne>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ne ') . ')') } else {  }; if (($code eq 'infix:<==>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' == ') . ')') } else {  }; if (($code eq 'infix:<!=>')) { return('(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' != ') . ')') } else {  }; if (($code eq 'ternary:<?? !!>')) { (my  $cond = $self->{arguments}->[0]);if ((Main::isa($cond, 'Var') && ($cond->sigil() eq '@'))) { ($cond = Apply->new( 'code' => 'prefix:<@>','arguments' => [$cond], )) } else {  };return('(' . $cond->emit() . ' ? ' . $self->{arguments}->[1]->emit() . ' : ' . $self->{arguments}->[2]->emit() . ')') } else {  }; $code . '(' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')' }
}
