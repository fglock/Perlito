...
sub to_bool { my $op = $_[0]; my $args = $_[1]; my  $List_s; for my $cond ( @{$args || []} ) { if (Main::bool((Main::isa($cond, 'Val::Int') || Main::isa($cond, 'Val::Num')))) { push( @{$List_s}, '(' . $cond->emit() . ' != 0 )' ) } else { if (Main::bool(((Main::isa($cond, 'Apply') && ($cond->code() eq 'infix:<||>')) || ((Main::isa($cond, 'Apply') && ($cond->code() eq 'infix:<&&>')) || ((Main::isa($cond, 'Apply') && ($cond->code() eq 'prefix:<!>')) || ((Main::isa($cond, 'Apply') && ($cond->code() eq 'prefix:<?>')) || Main::isa($cond, 'Val::Bit'))))))) { push( @{$List_s}, $cond->emit() ) } else { push( @{$List_s}, 'Main::bool(' . $cond->emit() . ')' ) } } }; return('(' . Main::join($List_s, $op) . ')') }
}
...
sub emit { my $self = $_[0]; (my  $cond = $self->{cond}); if (Main::bool((Main::isa($cond, 'Apply') && ($cond->code() eq 'prefix:<!>')))) { (my  $if = If->new( 'cond' => $cond->arguments()->[0],'body' => $self->{otherwise},'otherwise' => $self->{body}, ));return($if->emit()) } else {  }; return('if (' . Perl5::to_bool(' && ', [$cond]) . ') { ' . Main::join([ map { $_->emit() } @{ $self->{body} } ], ';') . ' } ' . 'else { ' . Main::join([ map { $_->emit() } @{ $self->{otherwise} } ], ';') . ' }') }
}
...
sub body { $_[0]->{body} };
sub emit { my $self = $_[0]; (my  $cond = $self->{cond}); if (Main::bool((Main::isa($cond, 'Var') && ($cond->sigil() eq '@')))) { ($cond = Apply->new( 'code' => 'prefix:<@>','arguments' => [$cond], )) } else {  }; 'for ( ' . (Main::bool($self->{init}) ? $self->{init}->emit() . '; ' : '; ') . (Main::bool($cond) ? Perl5::to_bool(' && ', [$cond]) . '; ' : '; ') . (Main::bool($self->{continue}) ? $self->{continue}->emit() . ' ' : ' ') . ') { ' . Main::join([ map { $_->emit() } @{ $self->{body} } ], '; ') . ' }' }
}
