...
sub array { $_[0]->{array} };
sub emit { my $self = $_[0]; (my  $a = $self->{array}); my  $item; (my  $s = '  save $P1' . Main::newline() . '  $P1 = new .ResizablePMCArray' . Main::newline()); do { for my $item ( @{$a} ) { ($s = $s . $item->emit());($s = $s . '  push $P1, $P0' . Main->newline()) } }; (my  $s = $s . '  $P0 = $P1' . Main::newline() . '  restore $P1' . Main::newline()); return($s) }
}
...
sub hash { $_[0]->{hash} };
sub emit { my $self = $_[0]; (my  $a = $self->{hash}); my  $item; (my  $s = '  save $P1' . Main::newline() . '  save $P2' . Main::newline() . '  $P1 = new .Hash' . Main::newline()); do { for my $item ( @{$a} ) { ($s = $s . $item->[0]->emit());($s = $s . '  $P2 = $P0' . Main->newline());($s = $s . $item->[1]->emit());($s = $s . '  set $P1[$P2], $P0' . Main->newline()) } }; (my  $s = $s . '  $P0 = $P1' . Main::newline() . '  restore $P2' . Main::newline() . '  restore $P1' . Main::newline()); return($s) }
}
...
sub class { $_[0]->{class} };
sub fields { $_[0]->{fields} };
sub emit { my $self = $_[0]; (my  $fields = $self->{fields}); (my  $str = ''); ($str = '  save $P1' . Main::newline() . '  save $S2' . Main::newline() . '  $P1 = new ' . Main::quote() . $self->{class} . Main::quote() . Main::newline()); do { for my $field ( @{$fields} ) { ($str = $str . $field->[0]->emit() . '  $S2 = $P0' . Main::newline() . $field->[1]->emit() . '  setattribute $P1, $S2, $P0' . Main::newline()) } }; ($str = $str . '  $P0 = $P1' . Main::newline() . '  restore $S2' . Main::newline() . '  restore $P1' . Main::newline()); $str }
}
...
sub topic { $_[0]->{topic} };
(my  $label = 100);
sub emit { my $self = $_[0]; (my  $cond = $self->{cond}); ($label = ($label + 1)); (my  $id = $label); do { if ((Main::isa($cond, 'Var') && ($cond->sigil() ne '@'))) { ($cond = Lit::Array->new( 'array' => [$cond], )) } else {  } }; return('' . $cond->emit() . '  save $P1' . Main::newline() . '  save $P2' . Main::newline() . '  $P1 = new .Iterator, $P0' . Main::newline() . ' test_iter' . $id . ':' . Main::newline() . '  unless $P1 goto iter_done' . $id . Main::newline() . '  $P2 = shift $P1' . Main::newline() . '  store_lex \'' . $self->{topic}->full_name() . '\', $P2' . Main::newline() . Main::join([ map { $_->emit() } @{ $self->{body} } ], '') . '  goto test_iter' . $id . Main::newline() . ' iter_done' . $id . ':' . Main::newline() . '  restore $P2' . Main::newline() . '  restore $P1' . Main::newline() . '') }
}
