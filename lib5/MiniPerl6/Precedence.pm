...
my  $Op2;
my  $End_token;
sub op_parse { my $self = $_[0]; my $str = $_[1]; my $pos = $_[2]; (my  $from = $pos); for my $tok ( @{($End_token) || []} ) { (my  $l = Main::chars($tok, )); (my  $s = substr($str, $pos, $l)); if (Main::bool(($s eq $tok))) { (my  $c1 = substr($str, (($pos + $l) - 1), 1)); (my  $c2 = substr($str, ($pos + $l), 1)); if (Main::bool((is_ident_middle($c1) && is_ident_middle($c2)))) {  } else { return(MiniPerl6::Match->new(('str' => $str), ('from' => $from), ('to' => ($pos + 2)), ('bool' => 1), ('capture' => ['end', $s]))) } }  }; (my  $c01 = substr($str, $pos, 1)); (my  $c02 = substr($str, $pos, 2)); (my  $hyper_left = 0); (my  $hyper_right = 0); if (Main::bool(((($c01 eq '«')) || (($c01 eq '»'))))) { ($hyper_left = $c01); ($pos = ($pos + 1)); ($c02 = substr($str, $pos, 2)) } else { if (Main::bool(((($c02 eq '<<')) || (($c02 eq '>>'))))) { ($hyper_left = $c02); ($pos = ($pos + 2)); ($c02 = substr($str, $pos, 2)) }  }; (my  $op2 = $c02); if (Main::bool(exists($Op2->{$op2}))) { (my  $c1 = substr($str, ($pos + 1), 1)); (my  $c2 = substr($str, ($pos + 2), 1)); if (Main::bool((is_ident_middle($c1) && is_ident_middle($c2)))) {  } else { ($pos = ($pos + 2)); (my  $c01 = substr($str, $pos, 1)); (my  $c02 = substr($str, $pos, 2)); if (Main::bool(((($c01 eq '«')) || (($c01 eq '»'))))) { ($hyper_right = $c01); ($pos = ($pos + 1)) } else { if (Main::bool(((($c02 eq '<<')) || (($c02 eq '>>'))))) { ($hyper_right = $c02); ($pos = ($pos + 2)) }  }; return(MiniPerl6::Match->new(('str' => $str), ('from' => $from), ('to' => $pos), ('bool' => 1), ('capture' => ['op', $op2, { ('hyper_left' => $hyper_left),('hyper_right' => $hyper_right), }]))) } } ; (my  $op1 = substr($str, $pos, 1)); if (Main::bool(exists($Op1->{$op1}))) { (my  $c2 = substr($str, ($pos + 1), 1)); if (Main::bool((is_ident_middle($op1) && is_ident_middle($c2)))) {  } else { ($pos = ($pos + 1)); (my  $c01 = substr($str, $pos, 1)); (my  $c02 = substr($str, $pos, 2)); if (Main::bool(((($c01 eq '«')) || (($c01 eq '»'))))) { ($hyper_right = $c01); ($pos = ($pos + 1)) } else { if (Main::bool(((($c02 eq '<<')) || (($c02 eq '>>'))))) { ($hyper_right = $c02); ($pos = ($pos + 2)) }  }; return(MiniPerl6::Match->new(('str' => $str), ('from' => $from), ('to' => $pos), ('bool' => 1), ('capture' => ['op', $op1, { ('hyper_left' => $hyper_left),('hyper_right' => $hyper_right), }]))) } } ; return(MiniPerl6::Match->new(('bool' => 0))) };
