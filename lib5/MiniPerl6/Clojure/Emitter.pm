...
') } ; if (Main::bool(Main::isa($decl, 'Method'))) { (my  $sig = $decl->sig()); (my  $invocant = $sig->invocant()); (my  $pos = $sig->positional()); (my  $str_specific = '(' . $invocant->emit_clojure() . ' ' . $class_name . ')'); (my  $str_optionals = ''); for my $field ( @{$pos || []} ) { ($str_optionals = $str_optionals . ' ' . $field->emit_clojure()) }; if (Main::bool(($str_optionals))) { ($str_specific = $str_specific . ' &optional' . $str_optionals) } ; (my  $block = MiniPerl6::Clojure::LexicalBlock->new(('block' => $decl->block()))); ($str = $str . ';; method ' . $decl->name() . '
...
sub emit_clojure { my $self = $_[0]; (my  $block1 = MiniPerl6::Clojure::LexicalBlock->new(('block' => $self->{body}))); (my  $block2 = MiniPerl6::Clojure::LexicalBlock->new(('block' => $self->{otherwise}))); '(if (sv-bool ' . $self->{cond}->emit_clojure() . ') ' . $block1->emit_clojure() . ' ' . $block2->emit_clojure() . ')' }
}
...
sub body { $_[0]->{body} };
sub topic { $_[0]->{topic} };
sub emit_clojure { my $self = $_[0]; (my  $cond = $self->{cond}); (my  $block = MiniPerl6::Clojure::LexicalBlock->new(('block' => $self->{body}))); if (Main::bool((Main::isa($cond, 'Var') && ($cond->sigil() eq '@')))) { ($cond = Apply->new(('code' => 'prefix:<@>'), ('arguments' => [$cond]))) } ; '(dolist (' . $self->{topic}->emit_clojure() . ' ' . $cond->emit_clojure() . ') ' . $block->emit_clojure() . ')' }
}
