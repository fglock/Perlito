...
') } }; do { if ($self->{methods}->{'perl'}) {  } else { ($str = $str . 'func (v_self *' . $class_name . ') f_perl (v Capture) *Any { ' . 'return toStr( "::' . $self->{name} . '(" ');(my  $sep = '');do { for my $decl ( values( %{$self->{attributes}} ) ) { do { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'has'))) { ($str = $str . $sep . '+ "' . $decl->var()->name() . ' => "' . '+ tostr((*(*v_self).f_' . $decl->var()->name() . '(Capture{})).(perl_er).f_perl(Capture{})) ');($sep = '+ ", " ') } else {  } } } };($str = $str . '+ ")" ) }' . '
...
'); my  $Hash_decl_seen; do { for my $decl1 ( @{$self->{body}} ) { (my  $decl = $decl1);do { if ((Main::isa($decl, 'Bind') && (Main::isa($decl->parameters(), 'Decl') && ($decl->parameters()->decl() eq 'my')))) { ($decl = $decl->parameters()) } else {  } };do { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'my'))) { (my  $var_name = $decl->var()->emit_go());do { if ($Hash_decl_seen->{$var_name}) {  } else { ($str = $str . $decl->emit_go_init());($Hash_decl_seen->{$var_name} = 1) } } } else {  } } } }; do { for my $decl ( @{$self->{body}} ) { do { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'has'))) { ($str = $str . '  // accessor ' . $decl->var()->name() . '
...
'); ($str = $str . '}' . '
'); return($str) };
sub emit_go_program { my $comp_units = $_[0]; (my  $str = ''); my  $Hash_unit_seen; my  $List_tmp_comp_unit; do { for my $comp_unit ( @{$comp_units} ) { (my  $name = $comp_unit->name());do { if ($Hash_unit_seen->{$name}) { do { for my $stmt ( @{$comp_unit->body()} ) { push( @{$Hash_unit_seen->{$name}->body()}, $stmt ) } } } else { ($Hash_unit_seen->{$name} = $comp_unit);push( @{$List_tmp_comp_unit}, $comp_unit ) } } } }; ($comp_units = $List_tmp_comp_unit); do { for my $comp_unit ( @{$comp_units} ) { do { for my $stmt ( @{$comp_unit->body()} ) { do { if (Main::isa($stmt, 'Method')) { ($comp_unit->methods()->{$stmt->name()} = $stmt) } else {  } };do { if ((Main::isa($stmt, 'Decl') && ($stmt->decl() eq 'has'))) { ($comp_unit->attributes()->{$stmt->var()->name()} = $stmt) } else {  } } } } } }; do { for my $comp_unit ( @{$comp_units} ) { ($str = $str . $comp_unit->emit_go()) } }; do { if ($Hash_unit_seen->{'MiniPerl6::Grammar'}) {  } else { ($str = $str . 'type MiniPerl6__Grammar struct{}
') } }; ($str = $str . '// interfaces for all methods
'); (my  $Hash_meth_seen = { 'join' => 1,'perl' => 1,'scalar' => 1,'isa' => 1,'values' => 1,'keys' => 1,'bind' => 1,'int' => 1,'str' => 1,'Str' => 1,'bool' => 1,'Bool' => 1,'array' => 1,'hash' => 1,'push' => 1,'pop' => 1,'shift' => 1,'lookup' => 1,'index' => 1,'function' => 1, }); do { for my $comp_unit ( @{$comp_units} ) { do { for my $stmt ( @{$comp_unit->body()} ) { do { if ((Main::isa($stmt, 'Method') && ($Hash_meth_seen->{$stmt->name()} ? 0 : 1))) { (my  $meth = $stmt->name());($str = $str . 'type ' . $meth . '_er interface { f_' . $meth . ' (Capture) *Any }
...
sub seq { $_[0]->{seq} };
sub emit_go { my $self = $_[0]; '[]*Any{ ' . Main::join([ map { $_->emit_go() } @{ $self->{seq} } ], ', ') . ' }' }
}
...
sub hash1 { $_[0]->{hash1} };
sub emit_go { my $self = $_[0]; (my  $fields = $self->{hash1}); (my  $str = ''); do { for my $field ( @{$fields} ) { ($str = $str . '*(*m).(lookup_er).f_lookup( Capture{ p : []*Any{ ' . $field->[0]->emit_go() . ' }} ) = *(' . $field->[1]->emit_go() . '); ') } }; 'func() *Any { ' . 'm := h_hash(); ' . $str . 'return m; ' . '}()' }
}
...
sub class { $_[0]->{class} };
sub fields { $_[0]->{fields} };
sub emit_go { my $self = $_[0]; (my  $fields = $self->{fields}); (my  $str = ''); do { for my $field ( @{$fields} ) { ($str = $str . 'if m.v_' . $field->[0]->buf() . ' == nil {' . '
' . 'var p Any; ' . '
' . 'm.v_' . $field->[0]->buf() . ' = &p; ' . '
' . '}' . '
...
sub name { $_[0]->{name} };
sub emit_go { my $self = $_[0]; (my  $table = { '$' => 'v_','@' => 'List_','%' => 'Hash_','&' => 'Code_', }); (my  $ns = ''); do { if ($self->{namespace}) { ($ns = Main::to_go_namespace($self->{namespace}) . '.') } else {  } }; (($self->{twigil} eq '.') ? '(*v_self).(' . $self->{name} . '_er).f_' . $self->{name} . '(Capture{})' : (($self->{name} eq '/') ? 'v_MATCH' : $table->{$self->{sigil}} . $ns . $self->{name})) };
...
sub arguments { $_[0]->{arguments} };
sub emit_go { my $self = $_[0]; (my  $invocant = $self->{invocant}->emit_go()); do { if (Main::isa($self->{invocant}, 'Proto')) { do { if (($self->{invocant}->name() eq 'self')) { ($invocant = 'v_self') } else { ($invocant = 'Proto_' . $invocant) } } } else {  } }; (my  $meth = $self->{method}); do { if (($meth eq 'postcircumfix:<( )>')) { do { if ($self->{hyper}) { ($meth = '') } else { return('((*' . $invocant . ').(function_er).f_function( Capture{ p : []*Any{ ' . Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ' } } ))') } } } else {  } }; do { if ($self->{hyper}) { return('func (a_ *Any) *Any { ' . '
' . '  var out = a_array(); ' . '
...
sub emit_go_call { my $invocant = $_[0]; my $meth_name = $_[1]; (my  $invocant1 = $invocant->emit_go()); do { if (Main::isa($invocant, 'Proto')) { ($invocant1 = 'Proto_' . $invocant1) } else {  } }; (my  $meth = $meth_name); return('(*' . $invocant1 . ').(' . $meth . '_er).f_' . $meth . '(Capture{})') }
}
...
sub emit_go { my $self = $_[0]; (my  $cond = $self->{cond}); do { if ((Main::isa($cond, 'Apply') && ($cond->code() eq 'prefix:<!>'))) { (my  $if = If->new( 'cond' => $cond->arguments()->[0],'body' => $self->{otherwise},'otherwise' => $self->{body}, ));return($if->emit_go()) } else {  } }; do { if ((Main::isa($cond, 'Var') && ($cond->sigil() eq '@'))) { ($cond = Apply->new( 'code' => 'prefix:<@>','arguments' => [$cond], )) } else {  } }; (my  $s = 'if tobool( ' . Call::emit_go_call($cond, 'Bool') . ' ) { ' . MiniPerl6::Go::LexicalBlock->new( 'block' => $self->{body},'needs_return' => 0, )->emit_go() . ' }'); do { if (@{$self->{otherwise}}) {  } else { return($s) } }; return($s . ' else { ' . MiniPerl6::Go::LexicalBlock->new( 'block' => $self->{otherwise},'needs_return' => 0, )->emit_go() . ' }') }
}
...
sub emit_go_init { my $self = $_[0]; do { if (($self->{decl} eq 'my')) { (my  $str = '');($str = $str . 'var ' . $self->{var}->emit_go() . ' *Any;' . '
...
sub emit_go_bind { my $self = $_[0]; (my  $str = ''); (my  $i = 0); do { for my $decl ( @{$self->{positional}} ) { ($str = $str . '  var ' . $decl->emit_go() . ' *Any;
' . '  if len(v.p) > ' . $i . ' {
' . '    ' . $decl->emit_go() . ' = v.p[' . $i . '];
' . '  }
