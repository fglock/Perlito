...
sub emit_go { my $self = shift; my $List__ = \@_; do { [] }; (my  $class_name = Main::to_go_namespace($self->{name})); (my  $str = ('// instances of class ' . ($self->{name} . (Main->newline() . ('type ' . ($class_name . (' struct {' . Main->newline()))))))); do { for my $decl ( values( %{$self->{attributes}} ) ) { do { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'has'))) { ($str = ($str . ('  ' . ('v_' . ($decl->var()->name() . (' Any;' . Main->newline())))))) } else {  } } } }; ($str = ($str . ('}' . Main->newline()))); ($str = ($str . ('// methods in class ' . ($self->{name} . (Main->newline() . ('var Method_' . ($class_name . (' struct {' . Main->newline())))))))); do { for my $decl ( values( %{$self->{methods}} ) ) { do { if (Main::isa($decl, 'Method')) { ($str = ($str . ('  ' . ('f_' . ($decl->name() . (' func (*' . ($class_name . (', Capture) Any;' . Main->newline())))))))) } else {  } } } }; do { for my $decl ( values( %{$self->{attributes}} ) ) { do { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'has'))) { ($str = ($str . ('  ' . ('f_' . ($decl->var()->name() . (' func (*' . ($class_name . (', Capture) Any;' . Main->newline())))))))) } else {  } } } }; ($str = ($str . ('}' . Main->newline()))); ($str = ($str . ('// namespace ' . ($self->{name} . (Main->newline() . ('var Namespace_' . ($class_name . (' struct {' . Main->newline())))))))); do { for my $decl ( @{$self->{body}} ) { do { if (Main::isa($decl, 'Sub')) { ($str = ($str . ('  ' . ('f_' . ($decl->name() . (' Function;' . Main->newline())))))) } else {  } } } }; ($str = ($str . ('}' . Main->newline()))); ($str = ($str . ('var Run_' . ($class_name . (' func ();' . Main->newline()))))); ($str = ($str . ('// method wrappers for ' . ($self->{name} . Main->newline())))); do { for my $decl ( values( %{$self->{methods}} ) ) { do { if (Main::isa($decl, 'Method')) { ($str = ($str . ('func (v_self *' . ($class_name . (') f_' . ($decl->name() . (' (v Capture) Any {' . (Main->newline() . ('  return Method_' . ($class_name . ('.f_' . ($decl->name() . ('(v_self, v);' . (Main->newline() . ('}' . Main->newline()))))))))))))))) } else {  } } } }; do { for my $decl ( values( %{$self->{attributes}} ) ) { do { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'has'))) { ($str = ($str . ('func (v_self *' . ($class_name . (') f_' . ($decl->var()->name() . (' (v Capture) Any {' . (Main->newline() . ('  return Method_' . ($class_name . ('.f_' . ($decl->var()->name() . ('(v_self, v);' . (Main->newline() . ('}' . Main->newline()))))))))))))))) } else {  } } } }; do { if (($class_name ne 'MiniPerl6__Match')) { ($str = ($str . ('func (v_self ' . ($class_name . (') Bool () Bool { return b_true }' . Main->newline())))));($str = ($str . ('func (v_self ' . ($class_name . (') Int () Int { panic("converting class to int") }' . Main->newline())))));($str = ($str . ('func (v_self ' . ($class_name . (') Str () Str { panic("converting class to string") }' . Main->newline())))));($str = ($str . ('func (v_self ' . ($class_name . (') Array () Array { panic("converting class to array") }' . Main->newline())))));($str = ($str . ('func (v_self ' . ($class_name . (') Hash () Hash { panic("converting class to hash") }' . Main->newline()))))) } else {  } }; ($str = ($str . ('func (v_self ' . ($class_name . (') Equal (j Any) Bool { panic("comparing class") }' . Main->newline()))))); ($str = ($str . ('func (v_self ' . ($class_name . (') Fetch () Any { return v_self }' . Main->newline()))))); ($str = ($str . ('func (v_self ' . ($class_name . (') f_isa (v Capture) Any { ' . ('return Str{ s : "' . ($class_name . ('" }.Str_equal( v.p[0] ) ' . ('}' . Main->newline()))))))))); ($str = ($str . ('// prototype of ' . ($self->{name} . (Main->newline() . ('var Proto_' . ($class_name . (' Any;' . Main->newline())))))))); ($str = ($str . ('func Init_' . ($class_name . ('() {' . Main->newline()))))); ($str = ($str . ('  this_namespace := &Namespace_' . ($class_name . (';' . (Main->newline() . ('  this_namespace = this_namespace;' . (Main->newline() . ('  Proto_' . ($class_name . (' = new(Scalar);' . (Main->newline() . ('  Proto_' . ($class_name . ('.(bind_er).Bind( new(' . ($class_name . (') );' . Main->newline()))))))))))))))))); do { for my $decl1 ( @{$self->{body}} ) { (my  $decl = $decl1);do { if ((Main::isa($decl, 'Bind') && (Main::isa($decl->parameters(), 'Decl') && ($decl->parameters()->decl() eq 'my')))) { ($decl = $decl->parameters()) } else {  } };do { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'my'))) { ($str = ($str . ('  var ' . ($decl->var()->emit_go() . (' Any = new(Scalar);' . Main->newline())))));do { if (($decl->var()->sigil() eq '%')) { ($str = ($str . ($decl->var()->emit_go() . ('.(bind_er).Bind( h_hash() );' . Main->newline())))) } else { do { if (($decl->var()->sigil() eq '@')) { ($str = ($str . ($decl->var()->emit_go() . ('.(bind_er).Bind( a_array() );' . Main->newline())))) } else { ($str = ($str . ($decl->var()->emit_go() . ('.(bind_er).Bind( u_undef );' . Main->newline())))) } } } };($str = ($str . Main->newline())) } else {  } } } }; do { for my $decl ( @{$self->{body}} ) { do { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'has'))) { ($str = ($str . ('  // accessor ' . ($decl->var()->name() . (Main->newline() . ('  Method_' . ($class_name . ('.f_' . ($decl->var()->name() . (' = func (v_self *' . ($class_name . (', v Capture) Any {' . (Main->newline() . ('    ' . ('if v_self.v_' . ($decl->var()->name() . (' == nil {' . (Main->newline() . ('      ' . ('var p = new(Scalar);' . (Main->newline() . ('      ' . ('v_self.v_' . ($decl->var()->name() . (' = p;' . (Main->newline() . ('    ' . ('}' . (Main->newline() . ('    ' . ('return v_self.v_' . ($decl->var()->name() . (Main->newline() . ('  };' . Main->newline())))))))))))))))))))))))))))))))))) } else {  } };do { if (Main::isa($decl, 'Method')) { (my  $sig = $decl->sig());(my  $block = MiniPerl6::Go::LexicalBlock->new( 'block' => $decl->block(),'needs_return' => 1,'top_level' => 1, ));($str = ($str . ('  // method ' . ($decl->name() . (Main->newline() . ('  Method_' . ($class_name . ('.f_' . ($decl->name() . (' = func (' . ($sig->invocant()->emit_go() . (' *' . ($class_name . (', v Capture) Any {' . (Main->newline() . ('    ' . ($sig->emit_go_bind() . (Main->newline() . ('    p := make(chan Any); go func () { ' . (Main->newline() . ('        ' . ($block->emit_go() . ('; return }(); ' . (Main->newline() . ('    return <-p; ' . (Main->newline() . ('  };' . Main->newline()))))))))))))))))))))))))))) } else {  } };do { if (Main::isa($decl, 'Sub')) { (my  $sig = $decl->sig());(my  $block = MiniPerl6::Go::LexicalBlock->new( 'block' => $decl->block(),'needs_return' => 1,'top_level' => 1, ));($str = ($str . ('  // sub ' . ($decl->name() . (Main->newline() . ('  Namespace_' . ($class_name . ('.f_' . ($decl->name() . (' = Function{ f : func (v Capture) Any {' . (Main->newline() . ('    ' . ($sig->emit_go_bind() . (Main->newline() . ('    p := make(chan Any); go func () { ' . (Main->newline() . ('        ' . ($block->emit_go() . ('; return }(); ' . (Main->newline() . ('    return <-p; ' . (Main->newline() . ('  } };' . Main->newline()))))))))))))))))))))))) } else {  } } } }; ($str = ($str . ('  // main runtime block of ' . ($self->{name} . (Main->newline() . ('  Run_' . ($class_name . (' = func () {' . Main->newline())))))))); do { for my $decl ( @{$self->{body}} ) { do { if ((((Main::isa($decl, 'Decl') && (($decl->decl() eq 'has') || ($decl->decl() eq 'my'))) ? 0 : 1) && ((Main::isa($decl, 'Method') ? 0 : 1) && (Main::isa($decl, 'Sub') ? 0 : 1)))) { ($str = ($str . ('    ' . ($decl->emit_go() . (';' . Main->newline()))))) } else {  } } } }; ($str = ($str . ('  }' . Main->newline()))); ($str = ($str . ('}' . Main->newline()))); return($str) }
...
sub emit_go { my $self = shift; my $List__ = \@_; do { [] }; (my  $fields = $self->{hash}); (my  $str = ''); do { for my $field ( @{$fields} ) { ($str = ($str . ('m.Lookup( ' . ($field->[0]->emit_go() . (' ).(bind_er).Bind( ' . ($field->[1]->emit_go() . ' ); ')))))) } }; ('func() Hash { ' . ('m := h_hash(); ' . ($str . ('return m; ' . '}()')))) }
...
sub class { @_ == 1 ? ( $_[0]->{class} ) : ( $_[0]->{class} = $_[1] ) };
sub fields { @_ == 1 ? ( $_[0]->{fields} ) : ( $_[0]->{fields} = $_[1] ) };
sub emit_go { my $self = shift; my $List__ = \@_; do { [] }; (my  $fields = $self->{fields}); (my  $str = ''); do { for my $field ( @{$fields} ) { ($str = ($str . ('m.v_' . ($field->[0]->buf() . (' = new(Scalar); ' . ('m.v_' . ($field->[0]->buf() . ('.(bind_er).Bind( ' . ($field->[1]->emit_go() . ' ); '))))))))) } }; ('func() *' . (Main::to_go_namespace($self->{class}) . (' { ' . ('var m = new(' . (Main::to_go_namespace($self->{class}) . ('); ' . ($str . ('return m; ' . '}()')))))))) }
...
sub arguments { @_ == 1 ? ( $_[0]->{arguments} ) : ( $_[0]->{arguments} = $_[1] ) };
sub emit_go { my $self = shift; my $List__ = \@_; do { [] }; (my  $invocant = $self->{invocant}->emit_go()); do { if (Main::isa($self->{invocant}, 'Proto')) { ($invocant = ('Proto_' . $invocant)) } else {  } }; (my  $meth = $self->{method}); do { if (($meth eq 'postcircumfix:<( )>')) { do { if ($self->{hyper}) { ($meth = '') } else { return(($invocant . ('.Apply( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ' } } )')))) } } } else {  } }; do { if ($self->{hyper}) { ('func (a_ Any) Array { ' . ('var out = a_array(); ' . ('var i Array = a_.Array(); ' . ('for pos := 0; pos < i.n; pos++ { ' . ('out.Push( i.v[pos].Fetch().(' . ($meth . ('_er).f_' . ($meth . ('(Capture{ p : []Any{}  })) } ' . ('return out; ' . ('}(' . ($invocant . ')')))))))))))) } else { ($invocant . ('.Fetch().(' . ($meth . ('_er).f_' . ($meth . ('( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ' } } )'))))))) } } }
...
sub arguments { @_ == 1 ? ( $_[0]->{arguments} ) : ( $_[0]->{arguments} = $_[1] ) };
sub namespace { @_ == 1 ? ( $_[0]->{namespace} ) : ( $_[0]->{namespace} = $_[1] ) };
sub emit_go { my $self = shift; my $List__ = \@_; do { [] }; (my  $code = $self->{code}); do { if (Main::isa($code, 'Str')) {  } else { return(('(' . ($self->{code}->emit_go() . (')->(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')'))))) } }; do { if (($code eq 'self')) { return('v_self') } else {  } }; do { if (($code eq 'false')) { return('b_false') } else {  } }; do { if (($code eq 'make')) { return(('func () Any { ' . ('tmp := ' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ('; ' . ('Proto_MiniPerl6__Match.Fetch().(capture_er).f_capture(Capture{}).(bind_er).Bind(tmp); ' . ('return tmp; ' . '}()'))))))) } else {  } }; do { if (($code eq 'say')) { return(('Print( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ', Str{s:"\\n"} } } )'))) } else {  } }; do { if (($code eq 'print')) { return(('Print( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ' } } )'))) } else {  } }; do { if (($code eq 'warn')) { return(('Print_stderr( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ', Str{s:"\\n"} } } )'))) } else {  } }; do { if (($code eq 'die')) { return(('Die( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ' } } )'))) } else {  } }; do { if (($code eq 'defined')) { return(('f_defined(' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ')'))) } else {  } }; do { if (($code eq 'pop')) { return(('Pop(' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ')'))) } else {  } }; do { if (($code eq 'index')) { return(('f_index(' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ')'))) } else {  } }; do { if (($code eq 'substr')) { return(('Substr( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ' } } )'))) } else {  } }; do { if (($code eq 'scalar')) { return(('f_scalar( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ' } } )'))) } else {  } }; do { if (($code eq 'prefix:<~>')) { return(('(' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ').Str()'))) } else {  } }; do { if (($code eq 'prefix:<!>')) { return(('(' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ').Bool().Not()'))) } else {  } }; do { if (($code eq 'prefix:<?>')) { return(('(' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ').Bool()'))) } else {  } }; do { if (($code eq 'prefix:<$>')) { return(('f_scalar( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ' } } )'))) } else {  } }; do { if (($code eq 'prefix:<@>')) { return(('(' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ')'))) } else {  } }; do { if (($code eq 'prefix:<%>')) { return(('(' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ').f_hash()'))) } else {  } }; do { if (($code eq 'infix:<~>')) { return(('Str{ s: strings.Join( []string{' . ('(' . ($self->{arguments}->[0]->emit_go() . (').Str().s, ' . ('(' . ($self->{arguments}->[1]->emit_go() . (').Str().s' . '}, "" ) }')))))))) } else {  } }; do { if (($code eq 'infix:<+>')) { return(('Int{ i:' . ('(' . ($self->{arguments}->[0]->emit_go() . (').Int().i + ' . ('(' . ($self->{arguments}->[1]->emit_go() . (').Int().i ' . '}')))))))) } else {  } }; do { if (($code eq 'infix:<->')) { return(('Int{ i:' . ('(' . ($self->{arguments}->[0]->emit_go() . (').Int().i - ' . ('(' . ($self->{arguments}->[1]->emit_go() . (').Int().i ' . '}')))))))) } else {  } }; do { if (($code eq 'infix:<>>')) { return(('Bool{ b:' . ('(' . ($self->{arguments}->[0]->emit_go() . (').Int().i > ' . ('(' . ($self->{arguments}->[1]->emit_go() . (').Int().i ' . '}')))))))) } else {  } }; do { if (($code eq 'infix:<&&>')) { return(('f_and( ' . ('func () Any { return ' . ($self->{arguments}->[0]->emit_go() . (' }, ' . ('func () Any { return ' . ($self->{arguments}->[1]->emit_go() . (' } ' . ')')))))))) } else {  } }; do { if (($code eq 'infix:<||>')) { return(('f_or( ' . ('func () Any { return ' . ($self->{arguments}->[0]->emit_go() . (' }, ' . ('func () Any { return ' . ($self->{arguments}->[1]->emit_go() . (' } ' . ')')))))))) } else {  } }; do { if (($code eq 'infix:<eq>')) { return(('(' . ($self->{arguments}->[0]->emit_go() . (').Str().Str_equal(' . ($self->{arguments}->[1]->emit_go() . ')'))))) } else {  } }; do { if (($code eq 'infix:<ne>')) { return(('(' . ($self->{arguments}->[0]->emit_go() . (').Str().Str_equal(' . ($self->{arguments}->[1]->emit_go() . ').Not()'))))) } else {  } }; do { if (($code eq 'infix:<==>')) { return(('(' . ($self->{arguments}->[0]->emit_go() . (').Equal(' . ($self->{arguments}->[1]->emit_go() . ')'))))) } else {  } }; do { if (($code eq 'infix:<!=>')) { return(('(' . ($self->{arguments}->[0]->emit_go() . (').Equal(' . ($self->{arguments}->[1]->emit_go() . ').Not()'))))) } else {  } }; do { if (($code eq 'ternary:<?? !!>')) { return(('func () Any { ' . ('if (' . ($self->{arguments}->[0]->emit_go() . (').Bool().b ' . ('{ return ' . ($self->{arguments}->[1]->emit_go() . (' }; ' . ('return ' . ($self->{arguments}->[2]->emit_go() . (' ' . '}()'))))))))))) } else {  } }; ($code = ('f_' . $self->{code})); do { if ($self->{namespace}) { ($code = ('Namespace_' . (Main::to_go_namespace($self->{namespace}) . ('.' . $code)))) } else { ($code = ('this_namespace.' . $code)) } }; ($code . ('.Apply( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit_go() } @{ $self->{arguments} } ], ', ') . ' } } )'))) }
...
sub emit_go { my $self = shift; my $List__ = \@_; do { [] }; ('func (a_ Any) { ' . ('var i Array = a_.Array(); ' . ('for pos := 0; pos < i.n; pos++ { ' . ('func (' . ($self->{topic}->emit_go() . (' Any) { ' . (MiniPerl6::Go::LexicalBlock->new( 'block' => $self->{body},'needs_return' => 0, )->emit_go() . (' }(i.v[pos]) ' . ('} ' . ('}(' . ($self->{cond}->emit_go() . ')'))))))))))) }
