...
'); (my  $Hash_meth_seen = { 'join' => 1,'perl' => 1,'scalar' => 1,'isa' => 1,'values' => 1,'keys' => 1,'bind' => 1,'int' => 1,'str' => 1,'Str' => 1,'bool' => 1,'array' => 1,'hash' => 1,'push' => 1,'lookup' => 1,'index' => 1, }); do { for my $comp_unit ( @{$comp_units} ) { do { for my $stmt ( @{$comp_unit->body()} ) { do { if ((Main::isa($stmt, 'Method') && ($Hash_meth_seen->{$stmt->name()} ? 0 : 1))) { (my  $meth = $stmt->name());($str = $str . 'type ' . $meth . '_er interface { f_' . $meth . ' (Capture) *Any }
