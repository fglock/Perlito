...
sub emit { my $self = shift; my $List__ = \@_; do { [] }; (my  $class_name = Main::to_javascript_namespace($self->{name})); (my  $str = ('// class ' . ($self->{name} . (Main->newline() . ('type Class_' . ($class_name . (' struct {' . Main->newline()))))))); do { for my $decl ( @{$self->{body}} ) { do { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'has'))) { ($str = ($str . ('  ' . ('v_' . ($decl->var()->name() . (' Any;' . Main->newline())))))) } else {  } } } }; ($str = ($str . ('}' . Main->newline()))); (my  $str = ($str . ('// namespace ' . ($self->{name} . (Main->newline() . ('var ' . ($class_name . (' struct {' . Main->newline())))))))); do { for my $decl ( @{$self->{body}} ) { do { if (Main::isa($decl, 'Sub')) { (my  $sig = $decl->sig());(my  $pos = $sig->positional());($str = ($str . ('  ' . ('f_' . ($decl->name() . (' Function;' . Main->newline())))))) } else {  } } } }; ($str = ($str . ('}' . Main->newline()))); do { if (($class_name eq 'Main')) { ($str = ($str . ('func main() {' . Main->newline()))) } else {  } }; ($str = ($str . ('    this_namespace := &' . ($class_name . ';')))); do { for my $decl ( @{$self->{body}} ) { do { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'my'))) { ($str = ($str . ($decl->var()->emit() . ' := ')));do { if (($decl->var()->sigil() eq '%')) { ($str = ($str . 'new(Scalar).Bind( h_hash() );')) } else { do { if (($decl->var()->sigil() eq '@')) { ($str = ($str . 'new(Scalar).Bind( a_array() );')) } else { ($str = ($str . 'new(Scalar).Bind( u_undef );')) } } } };($str = ($str . Main->newline())) } else {  } };do { if ((Main::isa($decl, 'Bind') && (Main::isa($decl->parameters(), 'Decl') && ($decl->parameters()->decl() eq 'my')))) { ($str = ($str . ($decl->parameters()->var()->emit() . (' := new(Scalar);' . Main->newline())))) } else {  } } } }; do { for my $decl ( @{$self->{body}} ) { do { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'has'))) { ($str = ($str . ('  // accessor ' . ($decl->var()->name() . (Main->newline() . ('  ' . ($class_name . ('.f_' . ($decl->var()->name() . (' = Function{ f : func (v Capture) Any {' . (Main->newline() . ('    ' . ('return this.v_' . ($decl->var()->name() . (Main->newline() . ('  } };' . Main->newline())))))))))))))))) } else {  } };do { if (Main::isa($decl, 'Method')) { (my  $sig = $decl->sig());(my  $pos = $sig->positional());(my  $invocant = $sig->invocant());(my  $block = MiniPerl6::Go::LexicalBlock->new( 'block' => $decl->block(),'needs_return' => 1,'top_level' => 1, ));($str = ($str . ('  // method ' . ($decl->name() . (Main->newline() . ('  (v_self ' . ($class_name . (') f_' . ($decl->name() . (' = Function{ f : func (v Capture) Any {' . (Main->newline() . ('    ' . ($sig->emit_bind() . (Main->newline() . ('    ' . ($block->emit() . (Main->newline() . ('  } };' . Main->newline())))))))))))))))))) } else {  } };do { if (Main::isa($decl, 'Sub')) { (my  $sig = $decl->sig());(my  $pos = $sig->positional());(my  $block = MiniPerl6::Go::LexicalBlock->new( 'block' => $decl->block(),'needs_return' => 1,'top_level' => 1, ));($str = ($str . ('  // sub ' . ($decl->name() . (Main->newline() . ('  ' . ($class_name . ('.f_' . ($decl->name() . (' = Function{ f : func (v Capture) Any {' . (Main->newline() . ('    ' . ($sig->emit_bind() . (Main->newline() . ('    ' . ($block->emit() . (Main->newline() . ('  } };' . Main->newline())))))))))))))))))) } else {  } } } }; do { for my $decl ( @{$self->{body}} ) { do { if ((((Main::isa($decl, 'Decl') && (($decl->decl() eq 'has') || ($decl->decl() eq 'my'))) ? 0 : 1) && ((Main::isa($decl, 'Method') ? 0 : 1) && (Main::isa($decl, 'Sub') ? 0 : 1)))) { ($str = ($str . ($decl->emit() . ';'))) } else {  } } } }; do { if (($class_name eq 'Main')) { ($str = ($str . ('}' . Main->newline()))) } else {  } } }
...
sub class { @_ == 1 ? ( $_[0]->{class} ) : ( $_[0]->{class} = $_[1] ) };
sub fields { @_ == 1 ? ( $_[0]->{fields} ) : ( $_[0]->{fields} = $_[1] ) };
sub emit { my $self = shift; my $List__ = \@_; do { [] }; (my  $fields = $self->{fields}); (my  $str = ''); do { for my $field ( @{$fields} ) { ($str = ($str . ('m.v_' . ($field->[0]->buf() . (' = ' . ($field->[1]->emit() . '; ')))))) } }; ('func() *' . (Main::to_javascript_namespace($self->{class}) . (' { ' . ('var m = new(' . (Main::to_javascript_namespace($self->{class}) . ('); ' . ($str . ('return m; ' . '}()')))))))) }
...
sub arguments { @_ == 1 ? ( $_[0]->{arguments} ) : ( $_[0]->{arguments} = $_[1] ) };
sub emit { my $self = shift; my $List__ = \@_; do { [] }; (my  $invocant = $self->{invocant}->emit()); do { if (($invocant eq 'self')) { ($invocant = 'this') } else {  } }; do { if (($self->{method} eq 'values')) { do { if ($self->{hyper}) { die('not implemented') } else { return(('@{' . ($invocant . '}'))) } } } else {  } }; do { if ((($self->{method} eq 'perl') || (($self->{method} eq 'isa') || ($self->{method} eq 'scalar')))) { do { if ($self->{hyper}) { return(('(func (a_) { ' . ('var out = []; ' . ('if ( typeof a_ == \'undefined\' ) { return out }; ' . ('for(var i = 0; i < a_.length; i++) { ' . ('out.Push( f_' . ($self->{method} . ('(a_[i]) ) } return out;' . (' })(' . ($invocant . ')')))))))))) } else {  } };return(('f_' . ($self->{method} . ('(' . ($invocant . (($self->{arguments} ? (', ' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ')) : '') . ')')))))) } else {  } }; do { if (($self->{method} eq 'join')) { return(($invocant . ('.' . ($self->{method} . ('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')')))))) } else {  } }; do { if ((($self->{method} eq 'yaml') || (($self->{method} eq 'say') || ($self->{method} eq 'chars')))) { do { if ($self->{hyper}) { return(('(func (a_) { ' . ('var out = []; ' . ('if ( typeof a_ == \'undefined\' ) { return out }; ' . ('for(var i = 0; i < a_.length; i++) { ' . ('out.Push( Main.' . ($self->{method} . ('(a_[i]) ) } return out;' . (' })(' . ($invocant . ')')))))))))) } else { do { if (defined($self->{arguments})) { return(('Main.' . ($self->{method} . ('(' . ($invocant . (', ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')'))))))) } else { return(('Main.' . ($self->{method} . ('(' . ($invocant . ')'))))) } } } } } else {  } }; (my  $meth = $self->{method}); do { if (($meth eq 'postcircumfix:<( )>')) { do { if ($self->{hyper}) { ($meth = '') } else { return(($invocant . ('.Apply( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ' } } )')))) } } } else {  } }; do { if ($self->{hyper}) { ('(func (a_) { ' . ('var out = []; ' . ('if ( typeof a_ == \'undefined\' ) { return out }; ' . ('for(var i = 0; i < a_.length; i++) { ' . ('out.Push( a_[i].f_' . ($meth . ('() ) } return out;' . (' })(' . ($invocant . ')'))))))))) } else { ($invocant . ('.f_' . ($meth . ('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')'))))) } } }
