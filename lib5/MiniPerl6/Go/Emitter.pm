...
sub arguments { @_ == 1 ? ( $_[0]->{arguments} ) : ( $_[0]->{arguments} = $_[1] ) };
sub emit { my $self = shift; my $List__ = \@_; do { [] }; (my  $invocant = $self->{invocant}->emit()); do { if (Main::isa($self->{invocant}, 'Proto')) { ($invocant = ('Proto_' . $invocant)) } else {  } }; do { if (($self->{method} eq 'values')) { do { if ($self->{hyper}) { die('not implemented') } else { return(('@{' . ($invocant . '}'))) } } } else {  } }; do { if ((($self->{method} eq 'perl') || (($self->{method} eq 'isa') || ($self->{method} eq 'scalar')))) { do { if ($self->{hyper}) { return(('(func (a_) { ' . ('var out = []; ' . ('if ( typeof a_ == \'undefined\' ) { return out }; ' . ('for(var i = 0; i < a_.length; i++) { ' . ('out.Push( f_' . ($self->{method} . ('(a_[i]) ) } return out;' . (' })(' . ($invocant . ')')))))))))) } else {  } };return(('f_' . ($self->{method} . ('(' . ($invocant . (($self->{arguments} ? (', ' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ')) : '') . ')')))))) } else {  } }; do { if (($self->{method} eq 'join')) { return(($invocant . ('.' . ($self->{method} . ('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')')))))) } else {  } }; do { if ((($self->{method} eq 'yaml') || (($self->{method} eq 'say') || ($self->{method} eq 'chars')))) { do { if ($self->{hyper}) { return(('(func (a_) { ' . ('var out = []; ' . ('if ( typeof a_ == \'undefined\' ) { return out }; ' . ('for(var i = 0; i < a_.length; i++) { ' . ('out.Push( Main.' . ($self->{method} . ('(a_[i]) ) } return out;' . (' })(' . ($invocant . ')')))))))))) } else { do { if (defined($self->{arguments})) { return(('Main.' . ($self->{method} . ('(' . ($invocant . (', ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')'))))))) } else { return(('Main.' . ($self->{method} . ('(' . ($invocant . ')'))))) } } } } } else {  } }; (my  $meth = $self->{method}); do { if (($meth eq 'postcircumfix:<( )>')) { do { if ($self->{hyper}) { ($meth = '') } else { return(($invocant . ('.Apply( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ' } } )')))) } } } else {  } }; do { if ($self->{hyper}) { ('(func (a_) { ' . ('var out = []; ' . ('if ( typeof a_ == \'undefined\' ) { return out }; ' . ('for(var i = 0; i < a_.length; i++) { ' . ('out.Push( a_[i].f_' . ($meth . ('() ) } return out;' . (' })(' . ($invocant . ')'))))))))) } else { ($invocant . ('.Fetch().(' . ($meth . ('_er).f_' . ($meth . ('( Capture{ p : []Any{ ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ' } } )'))))))) } } }
