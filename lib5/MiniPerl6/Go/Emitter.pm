...
sub emit_go { my $self = shift; my $List__ = \@_; do { [] }; ('Str{s:"' . (Main::lisp_escape_string($self->{buf}) . '"}')) }
...
sub name { @_ == 1 ? ( $_[0]->{name} ) : ( $_[0]->{name} = $_[1] ) };
sub emit_go { my $self = shift; my $List__ = \@_; do { [] }; (my  $table = { '$' => 'v_','@' => 'List_','%' => 'Hash_','&' => 'Code_', }); (my  $ns = ''); do { if ($self->{namespace}) { ($ns = (Main::to_go_namespace($self->{namespace}) . '.')) } else {  } }; (($self->{twigil} eq '.') ? ('this.v_' . ($self->{name} . '')) : (($self->{name} eq '/') ? ($table->{$self->{sigil}} . 'MATCH') : ($table->{$self->{sigil}} . ($ns . $self->{name})))) };
