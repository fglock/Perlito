...
sub emit_go_bind { my $self = shift; my $List__ = \@_; do { [] }; (my  $str = ''); (my  $i = 0); do { for my $decl ( @{$self->{positional}} ) { ($str = $str . '  var ' . $decl->emit_go() . ' *Any;
' . '  if len(v.p) > ' . $i . ' {
' . '    ' . $decl->emit_go() . ' = v.p[' . $i . '];
' . '  }
...
sub sig { @_ == 1 ? ( $_[0]->{sig} ) : ( $_[0]->{sig} = $_[1] ) };
sub block { @_ == 1 ? ( $_[0]->{block} ) : ( $_[0]->{block} = $_[1] ) };
sub emit_go { my $self = shift; my $List__ = \@_; do { [] }; do { if (($self->{name} eq '')) { return('toFunction( func(v Capture) *Any { ' . '    ' . $self->{sig}->emit_go_bind() . '
' . '    p := make(chan *Any); go func () { ' . '
' . '        ' . MiniPerl6::Go::LexicalBlock->new( 'block' => $self->{block},'needs_return' => 1,'top_level' => 1, )->emit_go() . '; return }(); ' . '
' . '    return <-p; ' . '
