...
sub class { @_ == 1 ? ( $_[0]->{class} ) : ( $_[0]->{class} = $_[1] ) };
sub fields { @_ == 1 ? ( $_[0]->{fields} ) : ( $_[0]->{fields} = $_[1] ) };
sub emit_go { my $self = shift; my $List__ = \@_; do { [] }; (my  $fields = $self->{fields}); (my  $str = ''); do { for my $field ( @{$fields} ) { ($str = ($str . ('m.v_' . ($field->[0]->buf() . ('.(bind_er).Bind( ' . ($field->[1]->emit_go() . ' ); ')))))) } }; ('func() *' . (Main::to_go_namespace($self->{class}) . (' { ' . ('var m = new(' . (Main::to_go_namespace($self->{class}) . ('); ' . ($str . ('return m; ' . '}()')))))))) }
