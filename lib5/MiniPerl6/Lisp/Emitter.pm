...
sub block { $_[0]->{block} };
sub emit_lisp { my $self = $_[0]; if (@{$self->{block}}) {  } else { return('nil') }; (my  $str = ''); (my  $has_my_decl = 0); (my  $my_decl = ''); (my  $my_ignore = ''); my  $Hash_decl_seen; for my $decl ( @{$self->{block}} ) { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'my'))) { (my  $var_name = $decl->var()->emit_lisp());if ($Hash_decl_seen->{$var_name}) {  } else { ($has_my_decl = 1);($my_decl = $my_decl . Decl::emit_lisp_initializer($decl->var()));($my_ignore = $my_ignore . '(declare (ignorable ' . $var_name . '))
');($Hash_decl_seen->{$var_name} = 1) } } else {  };if ((Main::isa($decl, 'Bind') && (Main::isa($decl->parameters(), 'Decl') && ($decl->parameters()->decl() eq 'my')))) { (my  $var_name = $decl->parameters()->var()->emit_lisp());if ($Hash_decl_seen->{$var_name}) {  } else { ($has_my_decl = 1);($my_decl = $my_decl . Decl::emit_lisp_initializer($decl->parameters()->var()));($my_ignore = $my_ignore . '(declare (ignorable ' . $var_name . '))
');($Hash_decl_seen->{$var_name} = 1) } } else {  } }; if ($has_my_decl) { ($str = $str . '(let (' . $my_decl . ')
' . $my_ignore) } else { ($str = $str . '(progn ') }; for my $decl ( @{$self->{block}} ) { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'my'))) {  } else { ($str = $str . $decl->emit_lisp()) } }; return($str . ')') }
}
...
' . '(in-package mp-Main)' . '
') } else {  } }; if (($self->{name} ne 'Pair')) { ($str = $str . '(defmethod sv-perl ((self ' . $class_name . '))' . Main->newline() . '  (mp-Main-sv-lisp_dump_object "' . Main::lisp_escape_string($self->{name}) . '"' . ' (list ' . $dumper . ')))' . Main->newline() . Main->newline()) } else {  }; ($str = $str . '(defun run-' . $class_name . ' ()' . '
'); for my $decl ( @{$self->{body}} ) { if ((((Main::isa($decl, 'Decl') && (($decl->decl() eq 'has') || ($decl->decl() eq 'my'))) ? 0 : 1) && ((Main::isa($decl, 'Method') ? 0 : 1) && (Main::isa($decl, 'Sub') ? 0 : 1)))) { ($str = $str . $decl->emit_lisp() . Main->newline()) } else {  } }; ($str = $str . ')' . '
'); if ($has_my_decl) { ($str = $str . ')') } else {  }; ($str = $str . Main->newline() . Main->newline()) };
sub emit_lisp_program { my $comp_units = $_[0]; (my  $str = ''); my  $Hash_unit_seen; my  $List_tmp_comp_unit; for my $comp_unit ( @{$comp_units} ) { (my  $name = $comp_unit->name());if ($Hash_unit_seen->{$name}) { for my $stmt ( @{$comp_unit->body()} ) { push( @{$Hash_unit_seen->{$name}->body()}, $stmt ) } } else { ($Hash_unit_seen->{$name} = $comp_unit);push( @{$List_tmp_comp_unit}, $comp_unit ) } }; ($comp_units = $List_tmp_comp_unit); for my $comp_unit ( @{$comp_units} ) { for my $stmt ( @{$comp_unit->body()} ) { if (Main::isa($stmt, 'Method')) { ($comp_unit->methods()->{$stmt->name()} = $stmt) } else {  };if ((Main::isa($stmt, 'Decl') && ($stmt->decl() eq 'has'))) { ($comp_unit->attributes()->{$stmt->var()->name()} = $stmt) } else {  } } }; for my $comp_unit ( @{$comp_units} ) { (my  $class_name = Main::to_lisp_namespace($comp_unit->name()));if (($class_name ne 'mp-Main')) { ($str = $str . '(defpackage ' . $class_name . '
...
');for my $decl ( @{$comp_unit->body()} ) { if ((Main::isa($decl, 'Decl') && ($decl->decl() eq 'has'))) { (my  $accessor_name = $decl->var()->name());($str = $str . ';; has $.' . $accessor_name . '
