...
'))); for my $decl ( @{$self->{body} || []} ) { if (Main::bool((Main::isa($decl, 'Decl') && (($decl->decl() eq 'my'))))) { ($str = $str . $decl->emit_javascript_init()) } ; if (Main::bool(((Main::isa($decl, 'Bind') && Main::isa(($decl->parameters()), 'Decl')) && ((($decl->parameters())->decl() eq 'my'))))) { ($str = $str . '  var ' . (($decl->parameters())->var())->emit_javascript(("" . ';') . Main->newline())) }  }; for my $decl ( @{$self->{body} || []} ) { if (Main::bool((Main::isa($decl, 'Decl') && (($decl->decl() eq 'has'))))) { ($str = $str . '  // accessor ' . ($decl->var())->name(("" . Main->newline()))); ($str = $str . '  ' . $class_name . '.v_' . ($decl->var())->name(("" . ' = null;') . Main->newline())); ($str = $str . '  ' . $class_name . '.f_' . ($decl->var())->name(("" . ' = function () { return this.v_') . ($decl->var())->name(("" . ' }') . Main->newline()))) } ; if (Main::bool(Main::isa($decl, 'Method'))) { (my  $sig = $decl->sig()); (my  $pos = $sig->positional()); (my  $invocant = $sig->invocant()); (my  $block = MiniPerl6::Javascript::LexicalBlock->new(('block' => $decl->block()), ('needs_return' => 1), ('top_level' => 1))); ($str = $str . '  // method ' . $decl->name(("" . Main->newline(("" . '  ') . $class_name . '.f_' . $decl->name())))); ($str = $str . ' = function (' . Main::join(([ map { $_->emit_javascript() } @{ (@{$pos || []}) } ]), ', ') . ') {' . Main->newline(("" . '    var ') . $invocant->emit_javascript(("" . ' = this;') . Main->newline()))); ($str = $str . '    ' . $block->emit_javascript(("" . Main->newline(("" . '  }') . Main->newline())))); ($str = $str . '  ' . $class_name . '.f_' . $decl->name(("" . ';  // v8 bug workaround') . Main->newline())) } ; if (Main::bool(Main::isa($decl, 'Sub'))) { (my  $sig = $decl->sig()); (my  $pos = $sig->positional()); (my  $block = MiniPerl6::Javascript::LexicalBlock->new(('block' => $decl->block()), ('needs_return' => 1), ('top_level' => 1))); ($str = $str . '  // sub ' . $decl->name(("" . Main->newline(("" . '  ') . $class_name . '.f_' . $decl->name())))); ($str = $str . ' = function (' . Main::join(([ map { $_->emit_javascript() } @{ (@{$pos || []}) } ]), ', ') . ') {' . Main->newline(("" . '    ') . $block->emit_javascript(("" . Main->newline())))); ($str = $str . '  }' . Main->newline()) }  }; for my $decl ( @{$self->{body} || []} ) { if (Main::bool(((((((Main::isa($decl, 'Decl') && (((($decl->decl() eq 'has')) || (($decl->decl() eq 'my')))))) ? 0 : 1)) && (((Main::isa($decl, 'Method')) ? 0 : 1))) && (((Main::isa($decl, 'Sub')) ? 0 : 1))))) { ($str = $str . ($decl)->emit_javascript(("" . ';'))) }  }; ($str = $str . '}' . ')();' . Main->newline()) }
}
...
sub hash1 { $_[0]->{hash1} };
sub emit_javascript { my $self = $_[0]; (my  $needs_interpolation = 0); for my $item ( @{$self->{hash1} || []} ) { if (Main::bool(((Main::isa(($item->[0]), 'Val::Buf')) ? 0 : 1))) { ($needs_interpolation = 1) }  }; if (Main::bool($needs_interpolation)) { (my  $s = ''); for my $field ( @{$self->{hash1} || []} ) { ($s = $s . 'a[' . ($field->[0])->emit_javascript(("" . '] = ') . ($field->[1])->emit_javascript(("" . '; ')))) }; return('(function () { var a = []; ' . $s . ' return a })()') } else { (my  $str = ''); for my $field ( @{$self->{hash1} || []} ) { ($str = $str . ($field->[0])->emit_javascript(("" . ':') . ($field->[1])->emit_javascript(("" . ',')))) }; return('{ ' . $str . ' }') } }
}
...
sub emit_javascript { my $self = $_[0]; if (Main::bool(Main::isa($self->{parameters}, 'Lit::Array'))) { (my  $a = $self->{parameters}->array1()); (my  $str = 'do { '); (my  $i = 0); for my $var ( @{$a || []} ) { (my  $bind = Bind->new(('parameters' => $var), ('arguments' => Index->new(('obj' => $self->{arguments}), ('index_exp' => Val::Int->new(('int' => $i))))))); ($str = $str . ' ' . $bind->emit_javascript(("" . '; '))); ($i = ($i + 1)) }; return($str . $self->{parameters}->emit_javascript(("" . ' }'))) } ; if (Main::bool(Main::isa($self->{parameters}, 'Lit::Hash'))) { (my  $a = $self->{parameters}->hash1()); (my  $b = $self->{arguments}->hash1()); (my  $str = 'do { '); (my  $i = 0); my  $arg; for my $var ( @{$a || []} ) { ($arg = Val::Undef->new()); for my $var2 ( @{$b || []} ) { if (Main::bool((($var2->[0])->buf() eq ($var->[0])->buf()))) { ($arg = $var2->[1]) }  }; (my  $bind = Bind->new(('parameters' => $var->[1]), ('arguments' => $arg))); ($str = $str . ' ' . $bind->emit_javascript(("" . '; '))); ($i = ($i + 1)) }; return($str . $self->{parameters}->emit_javascript(("" . ' }'))) } ; if (Main::bool(Main::isa($self->{parameters}, 'Lit::Object'))) { (my  $class = $self->{parameters}->class()); (my  $a = $self->{parameters}->fields()); (my  $b = $self->{arguments}); (my  $str = 'do { '); (my  $i = 0); my  $arg; for my $var ( @{$a || []} ) { (my  $bind = Bind->new(('parameters' => $var->[1]), ('arguments' => Call->new(('invocant' => $b), ('method' => ($var->[0])->buf()), ('arguments' => []), ('hyper' => 0))))); ($str = $str . ' ' . $bind->emit_javascript(("" . '; '))); ($i = ($i + 1)) }; return($str . $self->{parameters}->emit_javascript(("" . ' }'))) } ; if (Main::bool(Main::isa($self->{parameters}, 'Call'))) { return('(' . ($self->{parameters}->invocant())->emit_javascript(("" . '.v_') . $self->{parameters}->method(("" . ' = ') . $self->{arguments}->emit_javascript(("" . ')'))))) } ; '(' . $self->{parameters}->emit_javascript(("" . ' = ') . $self->{arguments}->emit_javascript(("" . ')'))) }
}
...
sub arguments { $_[0]->{arguments} };
sub namespace { $_[0]->{namespace} };
sub emit_javascript { my $self = $_[0]; (my  $code = $self->{code}); if (Main::bool(Main::isa($code, 'Str'))) {  } else { return('(' . $self->{code}->emit_javascript(("" . ')->(') . Main::join(([ map { $_->emit() } @{ $self->{arguments} } ]), ', ') . ')')) }; if (Main::bool(($code eq 'self'))) { return('v_self') } ; if (Main::bool(($code eq 'False'))) { return('false') } ; if (Main::bool(($code eq 'True'))) { return('true') } ; if (Main::bool(($code eq 'make'))) { return('(v_MATCH.v_capture = ' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ', ') . ')') } ; if (Main::bool(($code eq 'say'))) { return('f_say(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ', ') . ')') } ; if (Main::bool(($code eq 'print'))) { return('f_print(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ', ') . ')') } ; if (Main::bool(($code eq 'warn'))) { return('f_warn(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' + ') . ')') } ; if (Main::bool(($code eq 'defined'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' ') . ' != null)') } ; if (Main::bool(($code eq 'substr'))) { return('(' . ($self->{arguments}->[0])->emit_javascript(("" . ' || "").substr(') . ($self->{arguments}->[1])->emit_javascript(("" . ', ') . ($self->{arguments}->[2])->emit_javascript(("" . ')'))))) } ; if (Main::bool(($code eq 'Int'))) { return('parseInt(' . ($self->{arguments}->[0])->emit_javascript(("" . ')'))) } ; if (Main::bool(($code eq 'Num'))) { return('parseFloat(' . ($self->{arguments}->[0])->emit_javascript(("" . ')'))) } ; if (Main::bool(($code eq 'prefix:<~>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' ') . ').f_string()') } ; if (Main::bool(($code eq 'prefix:<!>'))) { return('( f_bool(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' ') . ') ? false : true)') } ; if (Main::bool(($code eq 'prefix:<?>'))) { return('( f_bool(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' ') . ') ? true : false)') } ; if (Main::bool(($code eq 'prefix:<$>'))) { return('f_scalar(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' ') . ')') } ; if (Main::bool(($code eq 'prefix:<@>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' ') . ')') } ; if (Main::bool(($code eq 'prefix:<%>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' ') . ').f_hash()') } ; if (Main::bool(($code eq 'infix:<~>'))) { return('( f_string(' . ($self->{arguments}->[0])->emit_javascript(("" . ')') . ' + f_string(' . ($self->{arguments}->[1])->emit_javascript(("" . ') )')))) } ; if (Main::bool(($code eq 'infix:<+>'))) { return('f_add(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ', ') . ')') } ; if (Main::bool(($code eq 'infix:<->'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' - ') . ')') } ; if (Main::bool(($code eq 'infix:<*>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' * ') . ')') } ; if (Main::bool(($code eq 'infix:</>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' / ') . ')') } ; if (Main::bool(($code eq 'infix:<>>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' > ') . ')') } ; if (Main::bool(($code eq 'infix:<<>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' < ') . ')') } ; if (Main::bool(($code eq 'infix:<>=>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' >= ') . ')') } ; if (Main::bool(($code eq 'infix:<<=>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' <= ') . ')') } ; if (Main::bool(($code eq 'infix:<&&>'))) { return('( f_bool(' . ($self->{arguments}->[0])->emit_javascript(("" . ')') . ' && f_bool(' . ($self->{arguments}->[1])->emit_javascript(("" . ') )')))) } ; if (Main::bool(($code eq 'infix:<||>'))) { return('( f_bool(' . ($self->{arguments}->[0])->emit_javascript(("" . ')') . ' || f_bool(' . ($self->{arguments}->[1])->emit_javascript(("" . ') )')))) } ; if (Main::bool(($code eq 'infix:<eq>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' == ') . ')') } ; if (Main::bool(($code eq 'infix:<ne>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' != ') . ')') } ; if (Main::bool(($code eq 'infix:<==>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' == ') . ')') } ; if (Main::bool(($code eq 'infix:<!=>'))) { return('(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ' != ') . ')') } ; if (Main::bool(($code eq 'exists'))) { (my  $arg = $self->{arguments}->[0]); if (Main::bool(Main::isa($arg, 'Lookup'))) { return('(' . ($arg->obj())->emit_javascript(("" . ').hasOwnProperty(') . ($arg->index_exp())->emit_javascript(("" . ')')))) }  } ; if (Main::bool(($code eq 'ternary:<?? !!>'))) { return('( f_bool(' . ($self->{arguments}->[0])->emit_javascript(("" . ')') . ' ? ' . ($self->{arguments}->[1])->emit_javascript(("" . ' : ') . ($self->{arguments}->[2])->emit_javascript(("" . ')'))))) } ; ($code = 'f_' . $self->{code}); if (Main::bool($self->{namespace})) { ($code = Main::to_javascript_namespace($self->{namespace}) . '.' . $code) } else { if (Main::bool((((((($code ne 'f_index')) && (($code ne 'f_die'))) && (($code ne 'f_pop'))) && (($code ne 'f_shift'))) && (($code ne 'f_push'))))) { ($code = 'v__NAMESPACE.' . $code) }  }; $code . '(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ', ') . ')' }
}
