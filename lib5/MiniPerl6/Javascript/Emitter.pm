...
sub arguments { @_ == 1 ? ( $_[0]->{arguments} ) : ( $_[0]->{arguments} = $_[1] ) };
sub emit { my $self = shift; my $List__ = \@_; do { [] }; (my  $invocant = $self->{invocant}->emit()); do { if (($invocant eq 'self')) { ($invocant = 'this') } else {  } }; do { if (($self->{method} eq 'values')) { do { if ($self->{hyper}) { die('not implemented') } else { return(('@{' . ($invocant . '}'))) } } } else {  } }; do { if ((($self->{method} eq 'perl') || (($self->{method} eq 'isa') || ($self->{method} eq 'scalar')))) { do { if ($self->{hyper}) { return(('(function (a_) { ' . ('var out = []; ' . ('if ( typeof a_ == \'undefined\' ) { return out }; ' . ('for(var i = 0; i < a_.length; i++) { ' . ('out.push( f_' . ($self->{method} . ('(a_[i]) ) } return out;' . (' })(' . ($invocant . ')')))))))))) } else {  } };return(('f_' . ($self->{method} . ('(' . ($invocant . (($self->{arguments} ? (', ' . Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ')) : '') . ')')))))) } else {  } }; do { if (($self->{method} eq 'join')) { return(($invocant . ('.' . ($self->{method} . ('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')')))))) } else {  } }; do { if ((($self->{method} eq 'yaml') || (($self->{method} eq 'say') || ($self->{method} eq 'chars')))) { do { if ($self->{hyper}) { return(('(function (a_) { ' . ('var out = []; ' . ('if ( typeof a_ == \'undefined\' ) { return out }; ' . ('for(var i = 0; i < a_.length; i++) { ' . ('out.push( Main.' . ($self->{method} . ('(a_[i]) ) } return out;' . (' })(' . ($invocant . ')')))))))))) } else { do { if (defined($self->{arguments})) { return(('Main.' . ($self->{method} . ('(' . ($invocant . (', ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')'))))))) } else { return(('Main.' . ($self->{method} . ('(' . ($invocant . ')'))))) } } } } } else {  } }; (my  $meth = $self->{method}); do { if (($meth eq 'postcircumfix:<( )>')) { ($meth = '') } else {  } }; do { if ($self->{hyper}) { ('(function (a_) { ' . ('var out = []; ' . ('if ( typeof a_ == \'undefined\' ) { return out }; ' . ('for(var i = 0; i < a_.length; i++) { ' . ('out.push( a_[i].f_' . ($meth . ('() ) } return out;' . (' })(' . ($invocant . ')'))))))))) } else { ($invocant . ('.f_' . ($meth . ('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')'))))) } } }
...
sub arguments { @_ == 1 ? ( $_[0]->{arguments} ) : ( $_[0]->{arguments} = $_[1] ) };
sub namespace { @_ == 1 ? ( $_[0]->{namespace} ) : ( $_[0]->{namespace} = $_[1] ) };
sub emit { my $self = shift; my $List__ = \@_; do { [] }; (my  $code = $self->{code}); do { if (Main::isa($code, 'Str')) {  } else { return(('(' . ($self->{code}->emit() . (')->(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')'))))) } }; do { if (($code eq 'self')) { return('this') } else {  } }; do { if (($code eq 'false')) { return('0') } else {  } }; do { if (($code eq 'make')) { return(('(v_MATCH.v_capture = ' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')'))) } else {  } }; do { if (($code eq 'say')) { return(('say(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' + ') . ')'))) } else {  } }; do { if (($code eq 'print')) { return(('print(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' + ') . ')'))) } else {  } }; do { if (($code eq 'warn')) { return(('warn(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' + ') . ')'))) } else {  } }; do { if (($code eq 'defined')) { return(('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ' != null)'))) } else {  } }; do { if (($code eq 'substr')) { return(('(' . ($self->{arguments}->[0]->emit() . (').substr(' . ($self->{arguments}->[1]->emit() . (', ' . ($self->{arguments}->[2]->emit() . ')'))))))) } else {  } }; do { if (($code eq 'prefix:<~>')) { return(('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ').f_string()'))) } else {  } }; do { if (($code eq 'prefix:<!>')) { return(('( f_bool(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ') ? false : true)'))) } else {  } }; do { if (($code eq 'prefix:<?>')) { return(('( f_bool(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ') ? true : false)'))) } else {  } }; do { if (($code eq 'prefix:<$>')) { return(('f_scalar(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'prefix:<@>')) { return(('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ')'))) } else {  } }; do { if (($code eq 'prefix:<%>')) { return(('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' ') . ').f_hash()'))) } else {  } }; do { if (($code eq 'infix:<~>')) { return(('( f_string(' . ($self->{arguments}->[0]->emit() . (')' . (' + f_string(' . ($self->{arguments}->[1]->emit() . ') )')))))) } else {  } }; do { if (($code eq 'infix:<+>')) { return(('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' + ') . ')'))) } else {  } }; do { if (($code eq 'infix:<->')) { return(('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' - ') . ')'))) } else {  } }; do { if (($code eq 'infix:<>>')) { return(('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' > ') . ')'))) } else {  } }; do { if (($code eq 'infix:<&&>')) { return(('( f_bool(' . ($self->{arguments}->[0]->emit() . (')' . (' && f_bool(' . ($self->{arguments}->[1]->emit() . ') )')))))) } else {  } }; do { if (($code eq 'infix:<||>')) { return(('( f_bool(' . ($self->{arguments}->[0]->emit() . (')' . (' || f_bool(' . ($self->{arguments}->[1]->emit() . ') )')))))) } else {  } }; do { if (($code eq 'infix:<eq>')) { return(('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' == ') . ')'))) } else {  } }; do { if (($code eq 'infix:<ne>')) { return(('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' != ') . ')'))) } else {  } }; do { if (($code eq 'infix:<==>')) { return(('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' == ') . ')'))) } else {  } }; do { if (($code eq 'infix:<!=>')) { return(('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ' != ') . ')'))) } else {  } }; do { if (($code eq 'ternary:<?? !!>')) { return(('( f_bool(' . ($self->{arguments}->[0]->emit() . (')' . (' ? ' . ($self->{arguments}->[1]->emit() . (' : ' . ($self->{arguments}->[2]->emit() . ')')))))))) } else {  } }; ($code = ('f_' . $self->{code})); do { if ($self->{namespace}) { ($code = (Main::to_javascript_namespace($self->{namespace}) . ('.' . $code))) } else {  } }; ($code . ('(' . (Main::join([ map { $_->emit() } @{ $self->{arguments} } ], ', ') . ')'))) }
...
sub emit { my $self = shift; my $List__ = \@_; do { [] }; return(('throw(' . ($self->{result}->emit() . ')'))) }
...
sub block { @_ == 1 ? ( $_[0]->{block} ) : ( $_[0]->{block} = $_[1] ) };
sub emit { my $self = shift; my $List__ = \@_; do { [] }; (my  $sig = $self->{sig}); (my  $invocant = $sig->invocant()); (my  $pos = $sig->positional()); (my  $str = Main::join([ map { $_->emit() } @{ @{$pos} } ], ', ')); ('function ' . ($self->{name} . ('(' . ($str . (') { ' . (MiniPerl6::Javascript::LexicalBlock->new( 'block' => $self->{block},'needs_return' => 1,'top_level' => 1, )->emit() . ' }')))))) }
...
sub sig { @_ == 1 ? ( $_[0]->{sig} ) : ( $_[0]->{sig} = $_[1] ) };
sub block { @_ == 1 ? ( $_[0]->{block} ) : ( $_[0]->{block} = $_[1] ) };
sub emit { my $self = shift; my $List__ = \@_; do { [] }; (my  $sig = $self->{sig}); (my  $pos = $sig->positional()); (my  $str = Main::join([ map { $_->emit() } @{ @{$pos} } ], ', ')); ('function ' . ($self->{name} . ('(' . ($str . (') { ' . (MiniPerl6::Javascript::LexicalBlock->new( 'block' => $self->{block},'needs_return' => 1,'top_level' => 1, )->emit() . ' }')))))) }
