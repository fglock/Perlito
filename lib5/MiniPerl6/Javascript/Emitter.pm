...
sub arguments { $_[0]->{arguments} };
sub emit_javascript { my $self = $_[0]; (my  $invocant = $self->{invocant}->emit_javascript()); if (($invocant eq 'self')) { ($invocant = 'v_self') } else {  }; if (($self->{method} eq 'values')) { if ($self->{hyper}) { die('not implemented') } else { return('f_values(' . $invocant . ')') } } else {  }; if ((($self->{method} eq 'perl') || (($self->{method} eq 'isa') || ($self->{method} eq 'scalar')))) { if ($self->{hyper}) { return('(function (a_) { ' . 'var out = []; ' . 'if ( typeof a_ == \'undefined\' ) { return out }; ' . 'for(var i = 0; i < a_.length; i++) { ' . 'out.push( f_' . $self->{method} . '(a_[i]) ) } return out;' . ' })(' . $invocant . ')') } else {  };return('f_' . $self->{method} . '(' . $invocant . ($self->{arguments} ? ', ' . Main::join([ map { $_->emit_javascript() } @{ $self->{arguments} } ], ', ') : '') . ')') } else {  }; if ((($self->{method} eq 'join') || (($self->{method} eq 'shift') || (($self->{method} eq 'push') || ($self->{method} eq 'pop'))))) { return($invocant . '.' . $self->{method} . '(' . Main::join([ map { $_->emit_javascript() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; if ((($self->{method} eq 'yaml') || (($self->{method} eq 'say') || ($self->{method} eq 'chars')))) { if ($self->{hyper}) { return('(function (a_) { ' . 'var out = []; ' . 'if ( typeof a_ == \'undefined\' ) { return out }; ' . 'for(var i = 0; i < a_.length; i++) { ' . 'out.push( Main.' . $self->{method} . '(a_[i]) ) } return out;' . ' })(' . $invocant . ')') } else { if (defined($self->{arguments})) { return('Main.' . $self->{method} . '(' . $invocant . ', ' . Main::join([ map { $_->emit_javascript() } @{ $self->{arguments} } ], ', ') . ')') } else { return('Main.' . $self->{method} . '(' . $invocant . ')') } } } else {  }; (my  $meth = $self->{method}); if ($self->{hyper}) { return('(function (a_) { ' . 'var out = []; ' . 'if ( typeof a_ == \'undefined\' ) { return out }; ' . 'for(var i = 0; i < a_.length; i++) { ' . 'out.push( a_[i].f_' . $meth . '() ) } return out;' . ' })(' . $invocant . ')') } else {  }; if (($meth eq 'postcircumfix:<( )>')) { return('(' . $invocant . ')(' . Main::join([ map { $_->emit_javascript() } @{ $self->{arguments} } ], ', ') . ')') } else {  }; return($invocant . '.f_' . $meth . '(' . Main::join([ map { $_->emit_javascript() } @{ $self->{arguments} } ], ', ') . ')') }
}
