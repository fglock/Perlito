...
sub emit_javascript { my $self = $_[0]; (my  $needs_interpolation = 0); my  $List_items; for my $item ( @{$self->{array1}} ) { if (Main::bool((Main::isa($item, 'Apply') && ((($item->code() eq 'circumfix:<( )>') || ($item->code() eq 'list:<,>')))))) { for my $arg ( @{[@{($item->arguments()) || []}]} ) { push( @{$List_items}, $arg ) } } else { push( @{$List_items}, $item ) } }; for my $item ( @{$List_items} ) { if (Main::bool((((Main::isa($item, 'Var') && ($item->sigil() eq '@'))) || ((Main::isa($item, 'Apply') && ($item->code() eq 'prefix:<@>')))))) { ($needs_interpolation = 1) }  }; if (Main::bool($needs_interpolation)) { (my  $s = ''); for my $item ( @{$List_items} ) { if (Main::bool((((Main::isa($item, 'Var') && ($item->sigil() eq '@'))) || ((Main::isa($item, 'Apply') && ($item->code() eq 'prefix:<@>')))))) { ($s = $s . '(function(a_) { ' . 'for (var i_ = 0; i_ < a_.length ; i_++) { a.push(a_[i_]) }' . '})(' . $item->emit_javascript() . '); ') } else { ($s = $s . 'a.push(' . $item->emit_javascript() . '); ') } }; '(function () { var a = []; ' . $s . ' return a })()' } else { '[' . Main::join(([ map { $_->emit_javascript() } @{ $List_items } ]), ', ') . ']' } }
}
...
sub cond { $_[0]->{cond} };
sub body { $_[0]->{body} };
sub emit_javascript { my $self = $_[0]; (my  $cond = $self->{cond}); if (Main::bool((((Main::isa($cond, 'Var') && ($cond->sigil() eq '@'))) ? 0 : 1))) { ($cond = Lit::Array->new(('array1' => [$cond]))) } ; (my  $body = MiniPerl6::Javascript::LexicalBlock->new(('block' => $self->{body}->stmts()), ('needs_return' => 0))); (my  $sig = 'v__'); if (Main::bool($self->{body}->sig())) { ($sig = $self->{body}->sig()->emit_javascript()) } ; '(function (a_) { for (var i_ = 0; i_ < a_.length ; i_++) { ' . '(function (' . $sig . ') { ' . $body->emit_javascript() . ' })(a_[i_]) } })' . '(' . $cond->emit_javascript() . ')' }
}
