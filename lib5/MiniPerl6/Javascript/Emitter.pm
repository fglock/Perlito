...
sub emit_javascript { my $self = $_[0]; (my  $invocant = $self->{invocant}->emit_javascript()); if (Main::bool(($invocant eq 'self'))) { ($invocant = 'v_self') } ; if (Main::bool(($self->{method} eq 'new'))) { (my  $str = ''); for my $field  ( @{$self->{arguments} || []} ) { if (Main::bool((Main::isa($field, 'Apply') && ($field->code() eq 'infix:<=>>')))) { ($str = $str . 'v_' . $field->arguments()->[0]->buf() . ': ' . $field->arguments()->[1]->emit_javascript() . ',') } else { die('Error in constructor, field: ', Main::perl($field, )) } }; return('function () { ' . 'var tmp = {' . $str . '}; ' . 'tmp.__proto__ = ' . Main::to_javascript_namespace($invocant) . '; ' . 'return tmp ' . '}()') } ; if (Main::bool(((((((($self->{method} eq 'perl')) || (($self->{method} eq 'isa'))) || (($self->{method} eq 'scalar'))) || (($self->{method} eq 'keys'))) || (($self->{method} eq 'values'))) || (($self->{method} eq 'elems'))))) { if (Main::bool(($self->{hyper}))) { return('(function (a_) { ' . 'var out = []; ' . 'if ( typeof a_ == \'undefined\' ) { return out }; ' . 'for(var i = 0; i < a_.length; i++) { ' . 'out.push( f_' . $self->{method} . '(a_[i]) ) } return out;' . ' })(' . $invocant . ')') } ; return('f_' . $self->{method} . '(' . $invocant . ((Main::bool(@{$self->{arguments} || []}) ? ', ' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ', ') : '')) . ')') } ; if (Main::bool((((((($self->{method} eq 'join')) || (($self->{method} eq 'shift'))) || (($self->{method} eq 'unshift'))) || (($self->{method} eq 'push'))) || (($self->{method} eq 'pop'))))) { return($invocant . '.' . $self->{method} . '(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ', ') . ')') } ; if (Main::bool((((($self->{method} eq 'yaml')) || (($self->{method} eq 'say'))) || (($self->{method} eq 'chars'))))) { if (Main::bool(($self->{hyper}))) { return('(function (a_) { ' . 'var out = []; ' . 'if ( typeof a_ == \'undefined\' ) { return out }; ' . 'for(var i = 0; i < a_.length; i++) { ' . 'out.push( Main.' . $self->{method} . '(a_[i]) ) } return out;' . ' })(' . $invocant . ')') } else { if (@{$self->{arguments} || []}) { return('Main.' . $self->{method} . '(' . $invocant . ', ' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ', ') . ')') } else { return('Main.' . $self->{method} . '(' . $invocant . ')') } } } ; (my  $meth = $self->{method}); if (Main::bool(($self->{hyper}))) { return('(function (a_) { ' . 'var out = []; ' . 'if ( typeof a_ == \'undefined\' ) { return out }; ' . 'for(var i = 0; i < a_.length; i++) { ' . 'out.push( a_[i].f_' . $meth . '(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ', ') . ') ) ' . '}; ' . 'return out;' . ' })(' . $invocant . ')') } ; if (Main::bool(($meth eq 'postcircumfix:<( )>'))) { return('(' . $invocant . ')(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ', ') . ')') } ; return($invocant . '.f_' . $meth . '(' . Main::join(([ map { $_->emit_javascript() } @{ $self->{arguments} } ]), ', ') . ')') }
}
...
sub cond { $_[0]->{cond} };
sub body { $_[0]->{body} };
sub emit_javascript { my $self = $_[0]; (my  $body = MiniPerl6::Javascript::LexicalBlock->new(('block' => $self->{body}->stmts()), ('needs_return' => 0))); (my  $sig = 'v__'); if (Main::bool($self->{body}->sig())) { ($sig = $self->{body}->sig()->emit_javascript()) } ; '(function (a_) { for (var i_ = 0; i_ < a_.length ; i_++) { ' . '(function (' . $sig . ') { ' . $body->emit_javascript() . ' })(a_[i_]) } })' . '(' . $self->{cond}->emit_javascript() . ')' }
}
