...
package GLOBAL;
    ...
    $List_a
        ...
        if (((((\@ARGV)->[0] eq '-V')) || (((\@ARGV)->[0] eq '--version')))) {
            ...
            if ((((((\@ARGV)->[0] eq '-h')) || (((\@ARGV)->[0] eq '--help'))) || (($backend eq '')))) {
                ($backend = '');
                Main::say($_V6_COMPILER_NAME, (' '), $_V6_COMPILER_VERSION, (chr(10) . 'perlito5 [switches] [programfile]' . chr(10) . '  switches:' . chr(10) . '    -h --help' . chr(10) . '    -v --verbose' . chr(10) . '    -V --version' . chr(10) . '    -Ctarget        target backend: go, js, lisp, parrot, perl5, python, ruby, ast-perl6, java' . chr(10) . '    --expand_use --noexpand_use' . chr(10) . '                    expand ' . chr(39) . 'use' . chr(39) . ' statements at compile time' . chr(10) . '    -e program      one line of program (omit programfile)' . chr(10)));
                shift( @{(\@ARGV)} )
            }
        };
        ...
        if (($backend && (\@ARGV))) {
            (my  $prelude_filename);
            if (($backend eq 'lisp')) {
                ($prelude_filename = ($perl6lib . chr(47) . 'Perlito5' . chr(47) . 'Lisp' . chr(47) . 'Prelude.pm'))
            };
            if (($backend eq 'js')) {
                ($prelude_filename = ($perl6lib . chr(47) . 'Perlito5' . chr(47) . 'Javascript' . chr(47) . 'Prelude.pm'))
            };
            if (($backend eq 'go')) {
                ($prelude_filename = ($perl6lib . chr(47) . 'Perlito5' . chr(47) . 'Go' . chr(47) . 'Prelude.pm'))
            };
            if ($prelude_filename) {
                if ($verbose) {
    ...
    $List_a
            ...
            if (($backend eq 'go')) {
                ...
                ((my  $filename) = ($perl6lib . chr(47) . 'Perlito5' . chr(47) . 'Go' . chr(47) . 'Runtime.go'));
                if (($verbose)) {
                    warn((chr(47) . chr(47) . ' now loading: '), $filename)
                };
                ((my  $source) = IO::slurp($filename));
            ...
            if (($backend eq 'lisp')) {
                ...
                ((my  $filename) = ($perl6lib . chr(47) . 'Perlito5' . chr(47) . 'Lisp' . chr(47) . 'Runtime.lisp'));
                if (($verbose)) {
                    warn((chr(47) . chr(47) . ' now loading: '), $filename)
                };
                ((my  $source) = IO::slurp($filename));
            ...
            if (($backend eq 'js')) {
                Main::say((chr(47) . chr(47) . ' Do not edit this file - Generated by '), $_V6_COMPILER_NAME, (' '), $_V6_COMPILER_VERSION);
                ((my  $filename) = ($perl6lib . chr(47) . 'Perlito5' . chr(47) . 'Javascript' . chr(47) . 'Runtime.js'));
                if (($verbose)) {
                    warn((chr(47) . chr(47) . ' now loading: '), $filename)
                };
                ((my  $source) = IO::slurp($filename));
            ...
            if (($backend eq 'ruby')) {
                Main::say((chr(35) . ' Do not edit this file - Generated by '), $_V6_COMPILER_NAME, (' '), $_V6_COMPILER_VERSION);
                Main::say('');
                Main::say(('require ' . chr(39) . 'Perlito5' . chr(47) . 'Ruby' . chr(47) . 'Runtime.rb' . chr(39)));
