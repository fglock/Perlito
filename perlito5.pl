...
package main;
    ...
    sub add_comp_unit {
        ...
        for my $comp_unit ( @{($parse)} ) {
            if ((($expand_use && Perlito5::Runtime::isa($comp_unit, 'Use')))) {
                expand_use($comp_unit)
            }
            else {
                if ((Perlito5::Runtime::isa($comp_unit, 'CompUnit'))) {
                    if (($verbose)) {
                        warn(('parsed comp_unit: ' . chr(39)), $comp_unit->name(), (chr(39)))
                    };
                    for my $stmt ( @{($comp_unit->body())} ) {
                        if ((($expand_use && Perlito5::Runtime::isa($stmt, 'Use')))) {
                            expand_use($stmt)
                        }
                    }
                }
    ...
    $List_a
        ...
        if ((($backend eq 'perl5'))) {
            ...
            Perlito5::Runtime::print(CompUnit::emit_perl5_program($comp_units))
        };
        if ((($backend eq 'js'))) {
            ...
            ((my  $source) = IO::slurp($filename));
            Perlito5::Runtime::say($source);
            Perlito5::Runtime::print(CompUnit::emit_javascript_program($comp_units))
        };
        if ((($backend eq 'java'))) {
            Perlito5::Runtime::say((chr(47) . chr(47) . ' Do not edit this file - Generated by '), $_V6_COMPILER_NAME, (' '), $_V6_COMPILER_VERSION);
            Perlito5::Runtime::print(CompUnit::emit_java_program($comp_units))
        }
    }
}
