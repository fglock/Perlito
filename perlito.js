...
(function () {
  ...
  For.f_emit_javascript = function () {
    var v_self = this;
    try { var v_cond;var v_body;var v_sig;(v_cond = v_self.v_cond);if ( f_bool(( f_bool((f_and(f_isa(v_cond, "Var"), function () { return (v_cond.f_sigil() == "@")}))) ? false : true)) ) { (function () { (v_cond = function () { var tmp = {v_array1: [v_cond],}; tmp.__proto__ = Lit$Array; return tmp }()); })() };(v_body = function () { var tmp = {v_block: v_self.v_body.f_stmts(),v_needs_return: 0,}; tmp.__proto__ = Perlito$Javascript$LexicalBlock; return tmp }());(v_sig = "v__");if ( f_bool(v_self.v_body.f_sig()) ) { (function () { (v_sig = v_self.v_body.f_sig().f_emit_javascript()); })() };return((f_string("(function (a_) { for (var i_ = 0; i_ < a_.length ; i_++) { ") + f_string((f_string("(function (") + f_string(v_sig) + f_string(") { "))) + f_string(v_body.f_emit_javascript()) + f_string(" })(a_[i_]) } })") + f_string("(") + f_string(v_cond.f_emit_javascript()) + f_string(")"))) } catch(err) { if ( err instanceof Error ) { throw(err) } else { return(err) } } 
  }
