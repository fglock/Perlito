...
mp6 [switches] [programfile]
  switches:
    ...
    -Ctarget        compile to target backend: go, js, lisp, parrot, perl5, python
        options:
...
if ( $backend eq 'python' ) {
    ...
    if ( $execute || $compile_to_bin ) {
        ...
        my $filename = "lib/MiniPerl6/Python/Runtime.py";
        warn "reading Python runtime: $filename\n" if $verbose;
        open FILE, $filename
          or die "Cannot read Python runtime: $filename";
        local $/ = undef;
        my $lib = <FILE>;
        print OUT $lib, "\n";
