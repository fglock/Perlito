...
(my  $List_v = []);
    ...
    $List_a
        ...
        sub add_comp_unit {
            my $List_parse = $_[0];
for my $comp_unit ( @{$List_parse || []} ) {     if (Main::bool(Main::isa($comp_unit, 'CompUnit'))) {
            if (Main::bool($verbose)) {
                    warn('parsed comp_unit: \'', $comp_unit->name(), '\'')
            }
;
for my $stmt ( @{(($comp_unit->body()) || []) || []} ) {     if (Main::bool(($expand_use && Main::isa($stmt, 'Use')))) {
...
;
                if (Main::bool(($backend eq 'python'))) {
                        ...
                        Main::say('__all__ = []');
                        Main::say('');
for my $c ( @{(($comp_units) || []) || []} ) {     Main::say($c->emit_python()) }
                }
;
                if (Main::bool(($backend eq 'ruby'))) {
                        ...
                        Main::say('');
for my $c ( @{(($comp_units) || []) || []} ) {     Main::say($c->emit_ruby()) }
                }
;
                if (Main::bool(($backend eq 'parrot'))) {
                        Main::say('# Do not edit this file - Generated by ', $_V6_COMPILER_NAME, ' ', $_V6_COMPILER_VERSION);
for my $c ( @{(($comp_units) || []) || []} ) {     Main::say($c->emit_parrot()) }
                }
