...
class Perlito6::Ruby::AnonSub {
    ...
    method emit_ruby_indented( $level ) {
        ...
        my $block = Perlito6::Ruby::LexicalBlock.new( 
                block => @.block,
                needs_return => 1 );
        my @s;
    ...
    method emit_ruby_indented( $level ) {
        ...
        if $.needs_return && $last_statement {
            ...
            if $last_statement.isa( 'If' ) {
                ...
                my $body_block      = 
                    Perlito6::Ruby::LexicalBlock.new( block => ($last_statement.body.stmts), needs_return => 1 );
                my $otherwise_block = 
                    Perlito6::Ruby::LexicalBlock.new( block => ($last_statement.otherwise.stmts), needs_return => 1 );
...
class CompUnit {
    ...
    method emit_ruby_indented( $level ) {
        my @s;
        my $block = Perlito6::Ruby::LexicalBlock.new( block => @.body );
...
class If {
    ...
    method emit_ruby_indented( $level ) {
        ...
        my $body_block = Perlito6::Ruby::LexicalBlock.new( block => @.body.stmts );
        if $body_block.has_my_decl() {
        ...
        my $s = Ruby::tab($level) ~   'if ' ~ Ruby::to_bool(' && ', [$.cond]) ~ "\n" 
            ~ $body_block.emit_ruby_indented( $level + 1 );
        if ( $has_otherwise ) {
            my $otherwise_block = Perlito6::Ruby::LexicalBlock.new( block => @.otherwise.stmts );
...
class While {
    method emit_ruby { self.emit_ruby_indented(0) }
    method emit_ruby_indented( $level ) {
        my $body_block = Perlito6::Ruby::LexicalBlock.new( block => @.body );
        if $body_block.has_my_decl() {
            $body_block = Do.new( block => @.body );
        }
        if $.init && $.continue {
...
class Sub {
    ...
    method emit_ruby_indented( $level ) {
        ...
        if ( $.name eq '' ) {
            ...
            Perlito6::Ruby::LexicalBlock::push_stmt_ruby( 
                    Perlito6::Ruby::AnonSub.new( 
                        name  => $label, 
                        block => @.block,
                        sig   => $.sig, 
...
class Do {
    ...
    method emit_ruby_indented( $level ) {
        ...
        push @s,    (Perlito6::Ruby::LexicalBlock.new( block => $block, needs_return => 0 )).emit_ruby_indented($level+1);
...
Perlito6::Ruby::Emit - Code generator for Perlito-in-Ruby
