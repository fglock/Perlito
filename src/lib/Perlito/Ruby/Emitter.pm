...
class Perlito::Ruby::AnonSub {
    ...
    method emit_ruby_indented( $level ) {
        ...
        my $block = Perlito::Ruby::LexicalBlock.new( 
                block => @.block,
                needs_return => 1 );
        my @s;
...
class Perlito::Ruby::LexicalBlock {
    has @.block;
    has $.needs_return;
    has $.top_level;
...
class Sub {
    ...
    method emit_ruby_indented( $level ) {
        ...
        if ( $.name eq '' ) {
            ...
            Perlito::Ruby::LexicalBlock::push_stmt_ruby( 
                    Perlito::Ruby::AnonSub.new( 
                        name  => $label, 
                        block => @.block,
                        sig   => $.sig, 
...
Perlito::Ruby::Emit - Code generator for Perlito-in-Ruby
