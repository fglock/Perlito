...
";
    ...
    if $backend && @*ARGS {
        my $prelude_filename;
        if $backend eq 'lisp' {
            $prelude_filename = $perl6lib ~ '/Perlito5/Lisp/Prelude.pm';
        }
        if $backend eq 'js' {
            $prelude_filename = $perl6lib ~ '/Perlito5/Javascript/Prelude.pm';
        }
        if $backend eq 'go' {
            $prelude_filename = $perl6lib ~ '/Perlito5/Go/Prelude.pm';
        }
        if $prelude_filename {
            if $verbose {
        ...
        if $backend eq 'go' {
            ...
            my $filename = $perl6lib ~ '/Perlito5/Go/Runtime.go';
            if ( $verbose ) {
                warn "// now loading: ", $filename;
            }
            my $source = IO::slurp( $filename );
        ...
        if $backend eq 'lisp' {
            say ";; Do not edit this file - Generated by ", $_V6_COMPILER_NAME, " ", $_V6_COMPILER_VERSION;
            my $filename = $perl6lib ~ '/Perlito5/Lisp/Runtime.lisp';
            if ( $verbose ) {
                warn "// now loading: ", $filename;
            }
            my $source = IO::slurp( $filename );
