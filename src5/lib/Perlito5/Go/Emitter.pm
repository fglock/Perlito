...
class Perlito5::Go::LexicalBlock {
    has @.block;
    has $.needs_return;
    has $.top_level;
    method emit_go {
        ...
        if $.needs_return && $last_statement {
            if $last_statement.isa( 'If' ) {
                ...
                $body      = Perlito5::Go::LexicalBlock.new( block => $body, needs_return => 1, top_level => $.top_level );
                $otherwise = Perlito5::Go::LexicalBlock.new( block => $otherwise, needs_return => 1, top_level => $.top_level );
                $str = $str
...
class CompUnit {
    ...
    method emit_go {
        ...
        for @.body -> $decl {
            ...
            if $decl.isa( 'Method' ) {
                my $sig      = $decl.sig;
                my $block    = Perlito5::Go::LexicalBlock.new( block => $decl.block, needs_return => 1, top_level => 1 );
                $str = $str
              ~ '  // method ' ~ $decl.name ~ "\n"
...
class Apply {
    method emit_go {
        ...
        if $code eq 'go'            {
                                      return 'go func () { ' ~ "\n"
                                                ~ '        ' ~ (Perlito5::Go::LexicalBlock.new( block => (@.arguments[0]).block, needs_return => 0, top_level => 1 )).emit_go ~ "\n"
                                                ~ '    }()'
                                    }
...
class If {
    method emit_go {
        ...
        my $s = 'if tobool( ' ~ Call::emit_go_call( $cond, 'Bool' ) ~ ' ) { '
                    ~ (Perlito5::Go::LexicalBlock.new( block => @.body, needs_return => 0 )).emit_go
                ~ ' }';
        if !(@.otherwise) {
            return $s;
        }
        return $s
                ~ ' else { '
                    ~ (Perlito5::Go::LexicalBlock.new( block => @.otherwise, needs_return => 0 )).emit_go
                ~ ' }';
    }
}
...
class Method {
    method emit_go {
        ...
        'func ' ~ $.name ~ '(v Capture) *Any { '
              ~ '    ' ~ ($.sig).emit_go_bind ~ "\n"
              ~ '    p := make(chan *Any); go func () { ' ~ "\n"
              ~ '        ' ~ (Perlito5::Go::LexicalBlock.new( block => @.block, needs_return => 1, top_level => 1 )).emit_go
              ~ '; p <- nil }(); ' ~ "\n"
              ~ '    return <-p; ' ~ "\n"
        ~ ' }'
    }
...
class Do {
    method emit_go {
        '(func () *Any { '
          ~ (Perlito5::Go::LexicalBlock.new( block => @.block, needs_return => 1 )).emit_go
          ~ '; return u_undef() '
        ~ '})()'
    }
}
...
Perlito5::Go::Emit - Code generator for Perlito-in-Go
