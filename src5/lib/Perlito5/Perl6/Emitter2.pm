...
package Perlito5::Perl62;
{
    ...
    sub emit_perl62_block {
        my $block = $_[0];
        return [ 'block', 
                 map { defined($_) && $_->emit_perl62() } @$block
               ];
    }
}
...
package Perlito5::AST::CompUnit;
{
    ...
    sub emit_perl62_program {
        my $comp_units = $_[0];
        return  map { $_->emit_perl62() } @$comp_units;
    }
}
...
package Perlito5::AST::Call;
{
    sub emit_perl62 {
        ...
        if  ($meth eq 'postcircumfix:<( )>')  {
            ...
            {
                #  &subr(args)
                return [ apply => '(', $invocant, map { $_->emit_perl62() } @{$self->{arguments}} ];
            }
            $meth = '';
        }
...
{
    sub emit_perl62_args {
        my $self = $_[0];
        return () if !$self->{arguments};
        return map { $_->emit_perl62() } @{$self->{arguments}};
    }
    sub emit_perl62 {
        my $self = $_[0];   
        if (ref $self->{code}) {
        ...
        if ($self->{code} eq 'infix:<=>>')  { 
            return [ op => $self->{code}, 
                     Perlito5::AST::Lookup->autoquote($self->{arguments}[0])->emit_perl62(),
                     $self->{arguments}[1]->emit_perl62() ]
        }
        ...
        if ($self->{code} eq 'p5:s') {
            return 's!' . $self->{arguments}->[0]->{buf}   # emit_perl62() 
                 .  '!' . $self->{arguments}->[1]->{buf}   # emit_perl62()
                 .  '!' . $self->{arguments}->[2];
            ...
            if ($self->{arguments}->[0]->isa('Perlito5::AST::Val::Buf')) {
                ...
                for my $ast (@{$self->{arguments}[0]{arguments}}) {
                    if ($ast->isa('Perlito5::AST::Val::Buf')) {
                        ...
                        $s .= $ast->emit_perl62();  # variable name
                    }
                }
            }
        ...
        if ($self->{code} eq 'p5:tr') {
            return 'tr!' . $self->{arguments}->[0]->{buf}   # emit_perl62() 
                 .   '!' . $self->{arguments}->[1]->{buf}   # emit_perl62()
                 .   '!';
        }
        ...
        if ($code eq 'map' || $code eq 'grep' || $code eq 'sort') {    
            if ( $self->{special_arg} ) {
                ...
                return [ op => 'prefix:<' . $code . '>',
                         [ 'block', map { $_->emit_perl62() } @{$self->{special_arg}{stmts}} ],
                         $self->emit_perl62_args() ]
            }
...
package Perlito5::AST::If;
{
    sub emit_perl62 {
        ...
        if ($self->{body} && ref($self->{body}) ne 'Perlito5::AST::Lit::Block') {
            return [ stmt_modifier => $self->{body}->emit_perl62(),
                                      [ stmt => 'if', $self->{cond}->emit_perl62() ] ];
        }
        if ($self->{otherwise} && ref($self->{otherwise}) ne 'Perlito5::AST::Lit::Block') {
            return [ stmt_modifier => $self->{otherwise}->emit_perl62(),
        ...
        push @out, [ stmt => [ keyword => 'else' ],
                     Perlito5::Perl62::emit_perl62_block($otherwise->stmts)
                   ];
        return @out;
    }
}
...
package Perlito5::AST::While;
{
    sub emit_perl62 {
        ...
        if ($self->{body} && ref($self->{body}) ne 'Perlito5::AST::Lit::Block') {
            return @out,
                   [ stmt_modifier => $self->{body}->emit_perl62(),
                                      [ stmt => [ keyword => 'while' ], $self->{cond}->emit_perl62() ] ];
        }
...
package Perlito5::AST::For;
{
    sub emit_perl62 {
        ...
        if ($self->{body} && ref($self->{body}) ne 'Perlito5::AST::Lit::Block') {
            return @out,
                   [ stmt_modifier => $self->{body}->emit_perl62(),
                                      [ stmt => 'for', $self->{cond}->emit_perl62() ] ];
        }
        ...
        if (ref($self->{cond}) eq 'ARRAY') {
            ...
            $cond = [ paren_semicolon => '(', 
                      ( $self->{cond}[0] ? $self->{cond}[0]->emit_perl62() : [] ),
                      ( $self->{cond}[1] ? $self->{cond}[1]->emit_perl62() : [] ),
                      ( $self->{cond}[2] ? $self->{cond}[2]->emit_perl62() : [] ),
                    ];
        }
        else {
...
Perlito5::Perl62::Emit - Code generator for Perlito5-in-Perl5

=head1 SYNOPSIS

    $program->emit_perl62()  # generated Perl5 code
