...
class Perlito5::Java::LexicalBlock {
    has @.block;
    has $.needs_return;
    has $.top_level;
    method emit_java {
        ...
        if $.needs_return && $last_statement {
            if $last_statement.isa( 'If' ) {
                ...
                $body      = Perlito5::Java::LexicalBlock.new( block => $body, needs_return => 1, top_level => $.top_level );
                $otherwise = Perlito5::Java::LexicalBlock.new( block => $otherwise, needs_return => 1, top_level => $.top_level );
                $str = $str
...
class CompUnit {
    ...
    method emit_java {
        ...
        for @.body -> $decl {
            ...
            if $decl.isa( 'Method' ) {
                my $sig      = $decl.sig;
                my $block    = Perlito5::Java::LexicalBlock.new( block => $decl.block, needs_return => 1, top_level => 1 );
                $str = $str
              ~ '  // method ' ~ $decl.name ~ "\n"
...
class Apply {
    method emit_java {
        ...
        if $code eq 'go'            {
                                      return 'go func () { ' ~ "\n"
                                                ~ '        ' ~ (Perlito5::Java::LexicalBlock.new( block => (@.arguments[0]).block, needs_return => 0, top_level => 1 )).emit_java ~ "\n"
                                                ~ '    }()'
                                    }
...
class If {
    method emit_java {
        ...
        my $s = 'if tobool( ' ~ Call::emit_java_call( $cond, 'Bool' ) ~ ' ) { '
                    ~ (Perlito5::Java::LexicalBlock.new( block => @.body, needs_return => 0 )).emit_java
                ~ ' }';
        if !(@.otherwise) {
            return $s;
        }
        return $s
                ~ ' else { '
                    ~ (Perlito5::Java::LexicalBlock.new( block => @.otherwise, needs_return => 0 )).emit_java
                ~ ' }';
    }
}
...
class Method {
    method emit_java {
        ...
        'func ' ~ $.name ~ '(v Capture) *Any { '
              ~ '    ' ~ ($.sig).emit_java_bind ~ "\n"
              ~ '    p := make(chan *Any); go func () { ' ~ "\n"
              ~ '        ' ~ (Perlito5::Java::LexicalBlock.new( block => @.block, needs_return => 1, top_level => 1 )).emit_java
              ~ '; p <- nil }(); ' ~ "\n"
              ~ '    return <-p; ' ~ "\n"
        ~ ' }'
    }
...
class Do {
    method emit_java {
        '(func () *Any { '
          ~ (Perlito5::Java::LexicalBlock.new( block => @.block, needs_return => 1 )).emit_java
          ~ '; return u_undef() '
        ~ '})()'
    }
}
...
Perlito5::Java::Emit - Code generator for Perlito-in-Java
