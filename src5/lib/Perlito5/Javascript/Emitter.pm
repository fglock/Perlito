    ...
    sub emit_javascript_indented {
        ...
        if ($.method eq 'new') {
            ...
            return
                ...
                .       'return ' . $invocant . '.new(' . join(', ', @.arguments.>>emit_javascript) . '); '
                .   '} '
        ...
        if ($.hyper) {
            return
                    '(function (a_) { '
                        ...
                        . 'if ( a_ == null ) { return out; }; '
                        . 'for(var i = 0; i < a_.length; i++) { '
                            . 'out.push( a_[i].' . Javascript::escape_function( $meth ) . '(' . join(', ', @.arguments.>>emit_javascript) . ') ) '
                        . '}; '
                        . 'return out;'
                    . ' })(' . $invocant . ')'
        }
        ...
        if  ($meth eq 'postcircumfix:<( )>')  {
            return '(' . $invocant . ')(' . join(', ', @.arguments.>>emit_javascript) . ')';
        }
        ...
        return Javascript::tab($level) 
            ...
            . '? ' . $invocant . '.__proto__.' . Javascript::escape_function( $meth ) . '.call(' . join(',', @args) . ') '
            . ': ' . $invocant . '.' . Javascript::escape_function( $meth ) . '(' . join(', ', @.arguments.>>emit_javascript) . ')'
            . ')';
    }
}
...
class Apply {
    ...
    sub emit_javascript_indented {
        ...
        if ($code eq 'infix:<=>>') {
            return Javascript::tab($level) . join(', ', @.arguments.>>emit_javascript)
        }
        if (exists %op_infix_js{$code}) {
            return Javascript::tab($level) . '(' 
                . join( $op_infix_js{$code}, @.arguments.>>emit_javascript)
                . ')'
        }
        ...
        if ($code eq 'make')       { return Javascript::tab($level) . '(v_MATCH.v_capture = ' . join(', ', @.arguments.>>emit_javascript) . ')' }
        if ($code eq 'defined')    { return Javascript::tab($level) . '('  . join(' ', @.arguments.>>emit_javascript)    . ' != null)' }
        if ($code eq 'substr') {
        ...
        if ($code eq 'shift')      {
            if ( @.arguments ) {
                return 'shift(' . join(', ', @.arguments.>>emit_javascript) . ')'
            }
            return 'shift(List__)'
        }
        ...
        if ($code eq 'prefix:<!>') { return '( ' . Javascript::escape_function('bool') . '(' . join(' ', @.arguments.>>emit_javascript)    . ') ? false : true)' }
        if ($code eq 'prefix:<?>') { return '( ' . Javascript::escape_function('bool') . '(' . join(' ', @.arguments.>>emit_javascript)    . ') ? true : false)' }
        if ($code eq 'prefix:<$>') { return Javascript::escape_function('scalar') . '(' . join(' ', @.arguments.>>emit_javascript)    . ')' }
        if ($code eq 'prefix:<@>') { return '(' . join(' ', @.arguments.>>emit_javascript)    . ')' }
        if ($code eq 'prefix:<%>') { return '(' . join(' ', @.arguments.>>emit_javascript)    . ').' . Javascript::escape_function('hash') . '()' }

        if ($code eq 'prefix:<\\>') { 
            # XXX currently a no-op
            return join(' ', @.arguments.>>emit_javascript) 
        }

        if ($code eq 'postfix:<++>') { return '('   . join(' ', @.arguments.>>emit_javascript)  . ')++' }
        if ($code eq 'postfix:<-->') { return '('   . join(' ', @.arguments.>>emit_javascript)  . ')--' }
        if ($code eq 'prefix:<++>')  { return '++(' . join(' ', @.arguments.>>emit_javascript)  . ')' }
        if ($code eq 'prefix:<-->')  { return '--(' . join(' ', @.arguments.>>emit_javascript)  . ')' }

        if ($code eq 'infix:<x>')  { return 'str_replicate(' . join(', ', @.arguments.>>emit_javascript)  . ')' }

        if ($code eq 'list:<.>')
        { 
            return '(' . Javascript::escape_function('string') . '('
                . join( ') + ' . Javascript::escape_function('string') . '(',
                        @.arguments.>>emit_javascript()
                      )
                . '))' 
        }

        if ($code eq 'infix:<+>')  { return Javascript::escape_function('add') . '('  . join(', ', @.arguments.>>emit_javascript)  . ')' }
        ...
        if ($.namespace) {
            ...
            return Javascript::tab($level) . $code . '(' . join(', ', @.arguments.>>emit_javascript) . ')';
        }
        my @args = 'CallSub';
        push @args, $_->emit_javascript
            for @.arguments;
