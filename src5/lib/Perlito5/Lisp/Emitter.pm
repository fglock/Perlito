...
class Perlito5::Lisp::LexicalBlock {
    has @.block;
    method emit_lisp {
        my @block;
        for @.block {
    ...
    method emit_lisp {
        ...
        for @.body -> $decl {
            ...
            if $decl.isa( 'Method' ) {
                ...
                my $block    = Perlito5::Lisp::LexicalBlock.new( block => $decl.block );
                $str = $str
                    ~ ';; method ' ~ $decl.name() ~ "\n"
...
class If {
    method emit_lisp {
        my $block1 = Perlito5::Lisp::LexicalBlock.new( block => $.body.stmts );
        if $.otherwise {
            my $block2 = Perlito5::Lisp::LexicalBlock.new( block => $.otherwise.stmts );
...
class For {
    method emit_lisp {
        my $cond = $.cond;
        my $block = Perlito5::Lisp::LexicalBlock.new( block => @.body );
        if   $cond.isa( 'Var' )
          && $cond.sigil eq '@'
        {
...
class While {
    method emit_lisp {
        ...
        ~       (Perlito5::Lisp::LexicalBlock.new( block => @body )).emit_lisp()
        ~    '))';
    }
}
...
class Sub {
    method emit_lisp {
        my $sig = $.sig;
        my $pos = $sig.positional;
        my $block = Perlito5::Lisp::LexicalBlock.new( block => @.block );
        my $str;
        if @$pos {
            for @$pos -> $field {
...
Perlito5::Lisp::Emit - Code generator for Perlito-in-Lisp (SBCL)
