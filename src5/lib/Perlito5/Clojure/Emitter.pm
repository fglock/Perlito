...
class Perlito5::Clojure::LexicalBlock {
    has @.block;
    method emit_clojure {
        if !(@.block) {
            return 'nil';
...
';
            ...
            if $decl.isa( 'Method' ) {
                ...
                my $block    = Perlito5::Clojure::LexicalBlock.new( block => $decl.block );
                $str = $str ~
';; method ' ~ $decl.name() ~ '
...
class If {
    method emit_clojure {
        my $block1 = Perlito5::Clojure::LexicalBlock.new( block => @.body );
        my $block2 = Perlito5::Clojure::LexicalBlock.new( block => @.otherwise );
...
class For {
    method emit_clojure {
        my $cond = $.cond;
        my $block = Perlito5::Clojure::LexicalBlock.new( block => @.body );
        if   $cond.isa( 'Var' )
          && $cond.sigil eq '@'
        {
...
class Sub {
    method emit_clojure {
        my $sig = $.sig;
        my $pos = $sig.positional;
        my $block = Perlito5::Clojure::LexicalBlock.new( block => @.block );
        my $str;
        if @$pos {
            for @$pos -> $field {
...
Perlito5::Clojure::Emit - Code generator for Perlito-in-Clojure
