...
(v_n = 0);(Hash_context["@_"] = v_args);(function (a_) { for (var i_ = 0; i_ < a_.length ; i_++) { (function (v_name) { (Hash_context[v_name] = v_args[v_n].f_eval(v_env));(v_n = f_add(v_n, 1)) })(a_[i_]) } })(List_param_name);(v_env1 = (function () { var a = []; a.push(Hash_context); (function(a_) { for (var i_ = 0; i_ < a_.length ; i_++) { a.push(a_[i_]) }})((v_env));  return a })());(function (a_) { for (var i_ = 0; i_ < a_.length ; i_++) { (function (v_stmt) { (v_r = v_stmt.f_eval(v_env1)) })(a_[i_]) } })(v_self.v_block);throw(v_r) } catch(err) { if ( err instanceof Error ) { throw(err) } else { return(err) } }  },}; tmp.__proto__ = EvalFunction; return tmp }());if ( f_bool(v_self.v_name) ) { (function () { (v_env[0][v_self.v_name] = v_sub); })() } else { (function () { null })() };throw(v_sub) } catch(err) { if ( err instanceof Error ) { throw(err) } else { return(err) } } 
  }
